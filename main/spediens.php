...
$_SESSION['sess_title'] = $LDEditTitle . '::' . isset($LDSubmitNews);
...
while (list($x, $v) = $helper->CustomEach($aSubMenuItem)) {
	$sTemp = '';
	ob_start();
...
if (($cfg['bname'] == "msie") && ($cfg['bversion'] > 4)) {
	...
	if ($cfg['icons'] != 'no_icon') {
		$smarty2->assign('sIconImg', '<img ' . createComIcon($root_path, 'uhr.gif', '0') . '>');
	}
