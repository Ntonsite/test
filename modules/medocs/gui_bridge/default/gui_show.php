...
?>><a href="javascript:gethelp('admission_how2new.php')"><img <?php echo createLDImgSrc($root_path, 'hilfe-r.gif', '0') ?>  <?php if ($cfg['dhtml']) {
	echo 'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)>';
}
?></a><a href="<?php
...
<td bgcolor="#ffffee" ><FONT SIZE=-1  FACE="Arial"><?php if ($sex == "m") {
	echo $LDMale;
} elseif ($sex == "f") {
	echo $LDFemale;
}
...
<p>
<a href="
<?php if ($_COOKIE['ck_login_logged' . $sid]) {
	echo 'startframe.php' . URL_APPEND;
} else {
