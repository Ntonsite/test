		...
		case 'save':
			...
			if ($update) {
				...
				$sql = "UPDATE $dbtable SET
												...
												op_start='$op_start',
												op_end='$op_end',
			                                    anasthetist='$anasthetist',
												scrub_nurse='$scrub_nurse',
			                                    assistant='$assistant',
			                                    anaesthesia_type='" . $_POST['anaesthesia_type'] . "',
			                                    postorder='" . $_POST['postorder'] . "',
			                                    indication='$indication',
			                                    procedure_or='$procedure_or',
												op_room='$op_room',
				...
				$sql = "INSERT INTO $dbtable
											...
											VALUES (
												...
												'$op_start',
												'$op_end',
			                                    '" . htmlspecialchars($anasthetist) . "',

												'$scrub_nurse',
			                                    '$assistant',
			                                    '" . $_POST['anaesthesia_type'] . "',
			                                    '" . $_POST['postorder'] . "',
			                                    '" . htmlspecialchars($indication) . "',
			                                    '" . htmlspecialchars($procedure_or) . "',
			                                    '$op_room',
		...
		?>
			        function chkForm(d) {
			            if (d.op_date.value == "") {
			                ...
			                d.operator.focus();
			                return false;
			              }else if(d.anaesthesia_type.value == "") {
			                alert("<?php echo $LDPlsEnterAnaesthesiaType; ?>");
			                d.anaesthesia_type.focus();
			                return false;
			                ...
			                d.therapy.focus();
			                return false;
			            } else if (d.indication.value == "") {
			                alert("<?php echo $LDPlsEnterIndication; ?>");
			                d.indication.focus();
			                return false;
			            } else if (d.procedure_or.value == "") {
			                alert("<?php echo $LDPlsEnterProcedure; ?>");
			                d.procedure_or.focus();
			                return false;
			            } else if (d.notes.value == "") {
			                alert("<?php echo $LDPlsEnterNotes; ?>");
			                d.notes.focus();
			                return false;
			            }else if (d.postorder.value == "") {
			                alert("<?php echo $LDPlsEnterOrder; ?>");
			                d.postorder.focus();
			                return false;
			            } else if (d.class_s.value == "") {
			             ...
			             }else if (d.anasthetist.value == "") {
			                alert("<?php echo $LDPlsEnteranasthetist; ?>");
			                d.anasthetist.focus();
			                return false;
...
# Buffer page output
	...
	if ($rows || $err_data) {
			...
			?>
			                        ...
			                        <table border=0 cellpadding=3 >
			                            ...
			                            <form method="post" action="op-doku-start.php" name="opdoc" <?php if ($mode != 'saveok') {
				echo 'onSubmit="return chkForm(this)"';
			}
			?>>
			                                <tr <?php if ($mode == 'saveok') {
				echo "bgcolor=#ffffff";
			}
			?>>
	...
	echo ' [';
			...
			?>
			                                        ...
			                                        <font size=2 face="arial" color=red>&nbsp; &nbsp;<?php if ($err_operator) {
				echo 'color=#cc0000';
			}
			?> <?php echo $LDOperator ?>:
			        <?php
	...
	echo '<b>' . $full_en . '</b>';
			...
			?>>
			                                    <td background="<?php echo $bg_img; ?>"><FONT <?php if ($err_name) {
				echo 'color=#cc0000';
			}
			?>><?php echo $LDLastName ?>:
			                                    </td>
	...
	echo '<b>' . $result['name_last'] . '</b>';
			...
			?>>
			                                    <td background="<?php echo $bg_img; ?>"><FONT <?php if ($err_vorname) {
				echo 'color=#cc0000';
			}
			?>><?php echo $LDName ?>:
			                                    </td>
	...
	echo '<b>' . ucwords($result['name_first']) . '</b>';
			...
			?>>
			                                    <td background="<?php echo $bg_img; ?>"><FONT <?php if ($err_gebdatum) {
				echo 'color=#cc0000';
			}
			?>><?php echo $LDBday ?>:
			                                    </td>
	...
	if ($result['kasse'] == "kasse") {
		...
		?>
                                ...
                                <tr>
                                    <td background="<?php echo $bg_img; ?>"><FONT  color=red>
                                        <?php echo $LDTypesAnaesthesia; ?>

                                    </td>
                                    <td>
                                        <?php
if ($mode == 'saveok') {
			echo '<font color="#800000">' . $opdoc['anaesthesia_type'] . ' &nbsp;</font>';
		} else {
			echo '<select name="anaesthesia_type">';
			echo '<option value="' . $opdoc['anaesthesia_type'] . '">' . $opdoc['anaesthesia_type'] . '</option>';
			echo '<option value="local">local</option>';
			echo '<option value="spinal">spinal</option>';
		...
		?>>
                                    <td background="<?php echo $bg_img; ?>"><FONT  color=red><?php if ($err_diagnosis) {
			echo '*';
		}
		?><?php echo $LDDiagnosis ?>:
                                    </td>
                                    <td>
        <?php
echo createElement('diagnosis', $diagnosis, 60, 100);
		...
		?>>
                                    <td background="<?php echo $bg_img; ?>"><FONT  color=red> <?php if ($err_localize) {
			echo '*';
		}
		?><?php echo $LDLocalization ?>:
                                    </td>
                                    <td>
...
echo createElement('localize', $localize, 60, 100);
		...
		?>>
                                    <td background="<?php echo $bg_img; ?>"><FONT  color=red><?php if ($err_therapy) {
			echo '*';
		}
		?><?php echo $LDTherapy ?>:
                                    </td>
...
echo createElement('therapy', $therapy, 60, 100);
		...
		?>>
                                    <td background="<?php echo $bg_img; ?>"><FONT  color=red><?php if ($err_indication) {
			echo '*';
		}
		?><?php echo $LDIndication ?>:
                                    </td>
                                    <td>


                                        <?php
echo createElement('indication', $indication, 60, 100);
		?>
                                    </td>
                                </tr >
		...
		?>>
                                    <td background="<?php echo $bg_img; ?>"><FONT  color=red><?php if ($err_procedure_or) {
			echo '*';
		}
		?><?php echo $LDProcedure ?>:
                                    </td>
                                    <td>


                                        <?php
echo createElement('procedure_or', $procedure_or, 60, 100);
		?>
                                    </td>
                                </tr >
                               ...
                               <tr><td background="<?php echo $bg_img; ?>"><FONT color=red><?php echo $LDSpecials ?></td>
                                <?php
if ($mode == 'saveok') {

			echo '<td> <font color="#800000">' . $opdoc['special'] . ' &nbsp;</font></td>';
		} else {
			echo '<td><textarea name="notes" cols="40" rows="5" id="notes" >' . $opdoc['special'] . '</textarea></td>';
		}
		?>
                               ...
                               <tr><td background="<?php echo $bg_img; ?>"><FONT color=red><?php echo $LDPostOrders ?></td>
                                <?php
if ($mode == 'saveok') {

			echo '<td> <font color="#800000">' . nl2br($opdoc['postorder']) . ' &nbsp;</font></td>';
		} else {
			echo '<td><textarea name="postorder" cols="40" rows="5" id="postorder" >' . $opdoc['postorder'] . '</textarea></td>';
		}
		?>
		...
		?>>
                                    <td background="<?php echo $bg_img; ?>"><FONT  color=red><?php if ($err_klas) {
			echo '*';
		}
		?><?php echo $LDClassification ?>:
                                    </td>
...
if ($mode == 'saveok') {
			...
			?>
                                            <select name="class_s">
                                                <option value="<?php echo $opdoc[class_s] ?>"><?php echo $opdoc[class_s] ?></option>
...
if ($rows || $err_data) {
		?>
                        ...
                        <p>
                            <FONT color=red><?php if ($err_op_start) {
			echo '*';
		}
		?>
                            <?php
...
echo $LDAnasthetist . ':';
		...
		echo createElement('anasthetist', $anasthetist, 14, 14);
		echo '<p>';
...
echo $LDOpRoom . ':';
		...
		?>
            ...
            <?php endif;?>
                            ...
                            <input type="hidden" name="mode" value="<?php if ($mode == 'saveok') {
			echo 'update';
		} else {
			echo 'save';
		}
		?>">
                            ...
                            <input type="hidden" name="update" value="<?php if ($mode == 'update') {
			echo '1';
		}
		?>">
                            <input type="hidden" name="pn" value="<?php if ($mode == 'match' && $rows == 1) {
			echo $result['encounter_nr'];
		} else {
			echo $pn;
		}
		?>">
