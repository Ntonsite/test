...
require $root_path . 'language/en/lang_en_finance.php';
...
# Title in toolbar
$room_nr = @$ORoom['room_nr'] ? $ORoom['room_nr'] : "";
$smarty->assign('sToolbarTitle', $LDOR . ' :: ' . $room_nr);
...
# Window bar title
$smarty->assign('sWindowTitle', $LDOR . ' :: ' . $room_nr);
...
if (!empty($ORoom['ward_id'])) {
	if (defined('SHOW_COMBINE_WARDIDNAME') && SHOW_COMBINE_WARDIDNAME) {
		...
		if (defined('SHOW_FULL_WARDNAME') && SHOW_FULL_WARDNAME) {
			$buffer = $ORoom['wardname'];
		} else {
			$buffer = $ORoom['ward_id'];
		}
...
if (isset($ORoom['LD_var']) && !empty($ORoom['LD_var'])) {
	echo $ORoom['LD_var'];
} else {
	echo $ORoom['deptname'] ?? "";
}
