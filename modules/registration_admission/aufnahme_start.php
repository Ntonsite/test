...
if (empty($pid)) {
	$pid = @$_SESSION['sess_pid'] ? $_SESSION['sess_pid'] : "";
}
...
//											$weberp_obj = new weberp($webERPServerURL,$weberpuser,$weberppassword,$weberpDebugLevel);
						...
						// update NHIF Serial
						$encounter_obj->InsertNHIFFields($encounter_nr);
						...
						if (@$ambulance_fee && $ambulance_fee != '') {
							$bill_obj->new_reg($encounter_nr, $ambulance_fee, $user);
						}

						if (@$consultation_fee && $consultation_fee != '') {
							$bill_obj->new_reg($encounter_nr, $consultation_fee, $user);
						}
	...
	if (empty($encounter_nr) || $encounter_class_nr == 1) {
		# Load all  wards info
		$ward_obj = new Ward;
		$items = 'nr,name';
...
if (!isset($pid) || !$pid) {
	...
	if (is_object($encounter_classes)) {
		...
		while ($result = $encounter_classes->FetchRow()) {
			...
			if ($encounter_nr) {
				# If admitted, freeze encounter class
    ...
    $InsranceResult='';
    $insuranceID='';
    $sqlInsurance="SELECT insurance_ID FROM care_person WHERE pid=".$pid;
    $InsranceResult=$db->Execute($sqlInsurance);
    $insuranceID=$InsranceResult->FetchRow();
    if ($insuranceID['insurance_ID']<1) {
    	$cash=true;
    }else{
    	$cash=false;
    }
	...
	}elseif($encounter_class_nr == 1 || $inOutPatient == 1){
	...
	*/
	...
	if (!$GLOBAL_CONFIG['patient_service_att_dr_hide'] && is_object($att_dr_service)) {
		...
		while ($buffer = $att_dr_service->FetchRow()) {
			...
			$sc_att_dr_class_nr = $sc_att_dr_class_nr ?? "";
	...
	$LDRegistration = $LDRegistration ?? "Registration";
