        ...
        $.ajax("<?php echo $nhif_base; ?>/breeze/Verification/AuthorizeCard?CardNo=" + cardno + "&VisitTypeID=" + visitType + "&ReferralNo=" + referralNo,
            ...
            if (data.CardStatus === 'Invalid') {
                ...
                <?php if ($current_encounter > 0): ?>
                    data.encounter_nr = "<?php echo $current_encounter ?>"
                <?php endif;?>
                ...
                $.ajax("updatePatientNHIFDetails.php", {
                        type: "POST",
                        data: data,
                        timeout: 10000
                }).done(function (data) {
...
if (@$_COOKIE[" ck_login_logged" . $sid]) {
	echo
		"news/start_page.php?sid=" . $sid . "&lang=" . $lang;
} else {
	echo $breakfile .
...
} elseif (empty($death_date) || $death_date == $dbf_nodate || $death_date == '1980-01-01') {
