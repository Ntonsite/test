    ...
    <table width=100% border=0 cellspacing=0 cellpadding=0>
        ...
        <tr>
            ...
            <td bgcolor="<?php echo $cfg['top_bgcolor']; ?>" align="right">
                <a href="javascript:gethelp('admission_show.php','<?php echo $from ?>')"><img <?php echo createLDImgSrc($root_path, 'hilfe-r.gif', '0') ?>  <?php if ($cfg['dhtml']) {
...
if ($current_encounter) {
		echo $LDEncounterClosed;
	} else {
		echo $LDPatientIsDischarged;
	}
...
if (file_exists($root_path . 'cache/barcodes/en_' . $encounter_nr . '.png')) {
	echo '<img src="' . $root_path . 'cache/barcodes/en_' . $encounter_nr . '.png" border=0 width=180 height=35>';
} else {
...
if ($death_date && $death_date != '0000-00-00' && $death_date != '0001-01-01' && $death_date != '1980-01-01') {
...
while ($buffer = $care_service->FetchRow()) {
	...
	if ($sc_care_start && $sc_care_start != '0000-00-00') {
		echo ' [ ' . formatDate2Local($sc_care_start, $date_format) . ' ] ' . $LDTo . ' [ ' . formatDate2Local($sc_care_end, $date_format) . ' ]';
	}
