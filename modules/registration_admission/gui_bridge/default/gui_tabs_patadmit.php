...
if (!isset($notabs) || !$notabs) {
	...
	$pbBuffer = '<a href="aufnahme_pass.php' . URL_APPEND . '&target=search"><img ' . createLDImgSrc($root_path, $img, '0') . ' alt="' . $LDSearch . '" title="' . $LDSearch . '"';
	if ($cfg['dhtml']) {
		$pbBuffer .= 'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
	}
	...
	$pbBuffer = '<a href="aufnahme_pass.php' . URL_APPEND . '&target=archiv"><img ' . createLDImgSrc($root_path, $img, '0') . ' alt="' . $LDArchive . '" title="' . $LDArchive . '" ';
	if ($cfg['dhtml']) {
		$pbBuffer .= 'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
	}
...
//	$pbBuffer='<a href="patient_register.php'.URL_APPEND.'&target=entry"><img '.createLDImgSrc($root_path,'register_gray.gif','0').' alt="'.$LDRegisterNewPerson.'"  title="'.$LDRegisterNewPerson.'" ';
	$pbBuffer = '<a href="patient_register_pass.php' . URL_APPEND . '&target=entry"><img ' . createLDImgSrc($root_path, $sNewPersonButton, '0') . ' alt="' . $LDRegisterNewPerson . '"  title="' . $LDRegisterNewPerson . '" ';
	if ($cfg['dhtml']) {
		$pbBuffer .= 'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
	}
...
if (!empty($subtitle)) {
	$smarty->assign('sSubTitle', ":: $subtitle");
}
