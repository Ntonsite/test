...
function createTR($input_name, $ld_text, $input_val, $colspan = 2, $input_size = 55) {
    ...
    <tr>
        ...
        <td colspan=<?php echo $colspan; ?> bgcolor="#eeeeee"><input name="<?php echo $input_name; ?>" type="text" size="<?php echo $input_size; ?>" value="<?php if (isset($input_val)) {
		echo $input_val;
	}
	?>">
        </td>
...
if (!isset($pid)) {
	...
	?>
            ...
            <tr>
                ...
                <td bgcolor="#eeeeee">
                    <input name="date_start" id="datepicker" type="text" size=10 maxlength=10  value="<?php if (!empty($date_start)) {
		echo @formatDate2Local($date_start, $date_format);
	}
...
$dfbuffer = "LD_" . strtr($date_format, ".-/", "phs");
	...
	?>] </font>
                ...
                <td bgcolor="#eeeeee">
            <nobr>&nbsp;<?php echo $LDTo ?>:
                <input name="date_end" type="text" id="datepicker2" size=10 maxlength=10 value="<?php if (!empty($date_end)) {
		echo @formatDate2Local($date_end, $date_format);
	}
	?>"  onBlur="IsValidDate(this, '<?php echo $date_format ?>')" onKeyUp="setDate(this, '<?php echo $date_format ?>', '<?php echo $lang ?>')">
	...
	?>
            ...
            <tr>
                ...
                <td bgcolor="#eeeeee" colspan=2>
                    <input name="date_birth" id="datepicker3" type="text" size="15" maxlength=10 value="<?php if (!empty($date_birth)) {
...
$dfbuffer = "LD_" . strtr($date_format, ".-/", "phs");
	...
	?>]</font>
            ...
            <tr>
                ...
                <td colspan=2 bgcolor="#eeeeee"><input name="sex" type="radio" value="m" <?php if ($sex == 'm') {
		echo 'checked';
	}
	?>><?php echo $LDMale ?>&nbsp;&nbsp;
...
# Create  encounter classes radiobuttons
	while ($result = $encounter_classes->FetchRow()) {
		?>
                        <input name="encounter_class_nr" type="radio"  value="<?php echo $result['class_nr']; ?>"  <?php if ($encounter_class_nr == $result['class_nr']) {
			echo 'checked';
		}
		?>>
                        <?php
...
//d.r from merotech: Remove Diagnoses from Archive Form
	...
	$referrer_dr = $referrer_dr ?? "";
	$referrer_recom_therapy = $referrer_recom_therapy ?? "";
	$referrer_notes = $referrer_notes ?? "";
	...
	?>
            <tr>
                ...
                <td bgcolor="#eeeeee" colspan=2>
                    <input name="date_discharge" id="datepicker" type="text" size="15" maxlength=10 value="<?php if (!empty($date_discharge)) {
		echo @formatDate2Local($date_discharge, $date_format);
	}
	?>"
                           onFocus="this.select();"  onBlur="IsValidDate(this, '<?php echo $date_format ?>')" onKeyUp="setDate(this, '<?php echo $date_format ?>', '<?php echo $lang ?>')">
