...
$prescrSubCat = $_GET['prescrSubCat'];
...
$smarty->assign('notDiagnosed', $notDiagnosed);
...
//$smarty->assign('sHealthFund',$sinsname);
else {
	$smarty->assign('sToolbarTitle', "$page_title ($sTitleNr)");
}

$smarty->assign('sHealthFund', "$page_title ($sinsname)");
...
if ($mode == 'show' /* && !isset($externalcall) */) {
	...
	if ($rows) {
		...
		if (!empty($externalcall)) {
			...
			if (!isset($printout) && !isset($_GET['ShowOnlyPharmacy'])) {
				...
				$smarty->assign('sPromptLink', '<a href="' . $thisfile . URL_APPEND . '&disablebuttons=' . $disablebuttons . '&pid=' . $_SESSION['sess_pid'] . '&target=' . $target . '&mode=new&help_site=' . $help_site . '&externalcall=' . $externalcall . '&prescrServ=' . $prescrServ . '&prescrSubCat=' . $prescrSubCat . '&backpath=' . urlencode($backpath) . '"><img ' . createComIcon($root_path, 'createnew_tz.gif', '0') . ' ></a>');
			}
		} else {
	...
	if ($prescrServ == "serv") {
		...
		if ($prescrServ == "proc") {
			...
			if ($show == 'druglistnhif') {
				$activated_tab = 'Drug-list-NHIF';
...
if (empty($externalcall)) {
    ...
    if (@$height_weight_url) {
	    $bottomLink='javascript:history.back()'; 
    }else{
