...
if (is_object($encounter_classes)) {
	while ($ec_row = $encounter_classes->FetchRow()) {
...
if ($row['remind'] && $row['appt_status'] == 'pending') {
		if ($row['remind_email']) {
			echo '<img ' . createComIcon($root_path, 'email.gif', '0', '', TRUE) . '> ';
		}

		if ($row['remind_mail']) {
			echo '<img ' . createComIcon($root_path, 'print.gif', '0', '', TRUE) . '> ';
		}

		if ($row['remind_phone']) {
			echo '<img ' . createComIcon($root_path, 'violet_phone_2.gif', '0', '', TRUE) . '> ';
		}
...
$buf = $enc_class[$row['encounter_class_nr']]['LD_var'];
	if (isset($$buf) && !empty($$buf)) {
		echo $$buf;
	} else {
		echo $enc_class[$row['encounter_class_nr']]['name'];
	}
