...
if (!isset($mode)) {
	$mode = 'show';
} elseif ($mode == 'create' || $mode == 'update' || $mode == 'delete' || $mode == 'stop') {
...
if ($parent_admit) {
	...
	if ($ShowOnlyPharmacy) {
		...
		$sql = "SELECT pr.*, e.encounter_class_nr
            ...
            AND service.is_labtest=0 AND " . $SQLCrit . " ORDER BY pr.prescribe_date DESC";
		...
		/*
			          ...
			          ORDER BY pr.prescribe_date ASC";
		*/
...
$showHist = '';
$patientNr = @$_GET['pn'] ? $_GET['pn'] : $_SESSION['sess_en'];
...
if ($prescribewithoutdiagnosis == 1) {
	$notDiagnosed = false;
} else {
	$notDiagnosed = true;
	$patientSQL = "SELECT prescribe_without_diagnosis FROM care_person WHERE pid = $pid";
	$patientResult = $db->Execute($patientSQL);
	if ($patientResult->RecordCount() > 0) {
		$patientRow = $patientResult->FetchRow();
