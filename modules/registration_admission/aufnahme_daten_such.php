...
$PageName = @$_COOKIE['PageName'] ? $_COOKIE['PageName'] : "";
if (@$PageName && $PageName != "Billing") {
...
if ($mode == 'search' || $mode == 'paginate') {
	...
	if ($linecount) {
		...
		while ($zeile = $ergebnis->FetchRow()) {
			...
			case 'f':$smarty->assign('sSex', '<img ' . $img_female . '>');
				break;
			case 'm':$smarty->assign('sSex', '<img ' . $img_male . '>');
				break;
			default:$smarty->assign('sSex', '&nbsp;');
				break;
			}
			...
			if ($zeile['death_date'] && $zeile['death_date'] != $dbf_nodate) {
				$smarty->assign('sCrossIcon', '<img ' . createComIcon($root_path, 'blackcross_sm.gif', '0', 'absmiddle') . '>');
			} else {
				$smarty->assign('sCrossIcon', '');
			}
			...
			if ($task == 'newprescription') {
				...
				$sTarget = "<a href=\"aufnahme_pass.php" . URL_APPEND . "&from=such&encounter_nr=$full_en&target=admitdata\">";
			}
 ...
 */
$smarty->assign('sPostText', '<a href="aufnahme_pass.php' . URL_APPEND . '&target=archiv">' . $LDAdmWantArchive . '</a>');
