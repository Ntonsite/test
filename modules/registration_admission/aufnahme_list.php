...
$thisfile = $thisfile ?? basename($_SERVER['PHP_SELF']);
...
# default, ascending alphabetic
	...
	if ($mode == 'paginate') {
		...
		$select = "SELECT p.selian_pid,UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first,p.date_birth,p.addr_zip, p.sex,e.encounter_nr,e.encounter_class_nr,e.is_discharged,e.encounter_date FROM ";
