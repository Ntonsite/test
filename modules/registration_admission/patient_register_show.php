...
if ($_COOKIE["ck_login_logged" . $sid]) {
	...
	$breakfile = "patient.php" . URL_APPEND . "&target=entry";
}
...
$smarty->assign('pbUpdateData', "<a href=\"patient_register_pass.php" . URL_APPEND . "&pid=$pid&target=update&update=1\"><img " . createLDImgSrc($root_path, 'update_data.gif', '0', 'absmiddle') . "></a>");
...
} elseif (!$death_date || $death_date == $dbf_nodate || $death_date == '1980-01-01') {

	$smarty->assign('pbAdmitInpatient', "<a href=\"$admissionfile&pid=$pid&origin=patreg_reg&target=admitip&encounter_class_nr=1\"><img " . createLDImgSrc($root_path, 'admit_inpatient.gif', '0', 'absmiddle') . "></a>");
	$smarty->assign('pbAdmitOutpatient', "<a href=\"$admissionfile&pid=$pid&origin=patreg_reg&target=admitop&encounter_class_nr=2\"><img " . createLDImgSrc($root_path, 'admit_outpatient.gif', '0', 'absmiddle') . "></a>");
}
...
# Assign help links
$smarty->assign('sSearchLink', '<img ' . createComIcon($root_path, 'varrow.gif', '0') . '> <a href="patient_register_pass.php' . URL_APPEND . '&target=search">' . $LDPatientSearch . '</a>');
$smarty->assign('sArchiveLink', '<img ' . createComIcon($root_path, 'varrow.gif', '0') . '> <a href="patient_register_pass.php' . URL_APPEND . '&target=archiv&newdata=1&from=entry">' . $LDArchive . '</a>');

$sCancel = "<a href=";
if ($_COOKIE['ck_login_logged' . $sid]) {
	$sCancel .= $breakfile;
} else {
	$sCancel .= 'aufnahme_pass.php';
}
