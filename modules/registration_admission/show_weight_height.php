...
# set your default unit of weight msrmnt type, default 6 = kilogram
if (!isset($ht_unit_nr) || !$ht_unit_nr) {
	$ht_unit_nr = 7;
}
# set your default unit of height msrmnt type, default 7 = centimeter
if (!isset($hc_unit_nr) || !$hc_unit_nr) {
	$hc_unit_nr = 7;
}
# set your default unit of head circumfernce msrmnt type, default 7 = centimeter
...
$unit_ids = array();
while (list($x, $v) = $helper->CustomEach($unit_types)) {
	$unit_ids[$v['nr']] = $v['id'];
}
reset($unit_types);
if (@$unit_rates) {
	while (list($x, $v) = $helper->CustomEach($unit_rates)) {
		$unit_ids[$v['nr']] = $v['id'];
	}
	reset($unit_rates);
}

while (list($x, $v) = $helper->CustomEach($unit_bp)) {
	$unit_ids[$v['nr']] = $v['id'];
}
reset($unit_bp);

while (list($x, $v) = $helper->CustomEach($unit_temp)) {
	$unit_ids[$v['nr']] = $v['id'];
}
reset($unit_temp);

if (@$unit_sat) {
	while (list($x, $v) = $helper->CustomEach($unit_sat)) {
		$unit_ids[$v['nr']] = $v['id'];
	}
	reset($unit_sat);
}
...
if (@$mode && $mode == 'new') {
	//resolve for mode = new to mode = create
	$mode = 'create';
}
...
# Set to no redirect
		...
		if ($weight) {
			$msr_time=isset($msr_time) ? $msr_time : "";
			$mode = $modetmp;
			$_POST['value'] = $weight;
...
$height_weight_url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
