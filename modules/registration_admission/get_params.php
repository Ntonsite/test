...
extract($_GET);
$districtid = @$_GET['districtid']?$_GET['districtid']:0;
...
if (($dir == 'district') && ($id != '')) {
    ...
    while ($rw = $vh->FetchRow()) {
        ...
        print '<option '. $selectedDistrict .' value="' . $rw[0] . '" id="-1">' . $rw[1] . '</option>';
    }
    print '</select >';
}
...
if (($dir == 'ward') && ($id != '')) {
    ...
    while ($rw = $vh->FetchRow()) {
        ...
        print '<option '.$selectedWard.' value="' . $rw[0] . '" id="-1">' . $rw[1] . '</option>';
    }
    print '</select >';
}
?>
