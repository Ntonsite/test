...
*/
		...
		*/
		...
		if (($vct[9] == 1) && ($patient['encounter_nr'] != '')) {
			if ($kct < 1) {
				$smarty->assign('sFlagDiag', ' style="background-color:yellow; color:#000000;" ');
				$smarty->assign('sFlagDiag2', ' style="background-color:gold; color:#000000;" ');
		...
		$sBuffer = '';
		if (isset($$patient['selian_pid']) && !empty($$patient['selian_pid'])) {
			$sBuffer = $sBuffer . $$patient['selian_pid'];
		} else {
			$sBuffer = $sBuffer . $patient['selian_pid'];
		}
		...
		if ($patient['insurance_ID'] == '0') {
			...
			$sql_HealthFundName = "SELECT name,id FROM care_tz_company WHERE id=" . $patient['insurance_ID'];
			$result_HealthFundName = $db->Execute($sql_HealthFundName);
			$HealthFundName = $result_HealthFundName->FetchRow();
			$fundname = $HealthFundName['name'];

		}

		

		$smarty->assign('sInsuranceType', $fundname);
