...
if ($rows) {
	...
	while ($patient = $opat_obj->FetchRow()) {
		if (@$sEncNrBuffer && $patient['encounter_nr'] == $sEncNrBuffer) {
			continue;
		} else {
...
//        print_r($patient).'<br><hr>';
		...
		if ($edit) {
			...
			if ($glob_obj->getConfigValue("restrict_unbilled_items") === "1") {
				if ($enc_obj->getEncouterBillNo($patient['encounter_nr']) > 0 && $patient['insurance_ID'] == '0' || $patient['insurance_ID'] > 0) {
		...
		if ($edit) {
			...
			*/
			...
			if ($dept_nr == 3) {
				$smarty->assign('sOPRoom', '<a href="' . $root_path . 'main/op-doku.php' . '"><img ' . createComIcon($root_path, 'statbel.gif', '0', '', TRUE) . ' align="absmiddle" alt="' . $LDShowPatData . '"></a>');
			}
