...
if ($printout) {
    ...
    <html><body>
            ...
            <DIV align="center">
                <h2>Doctor &nbsp;<?php echo $_GET['doctor'] . '\'s'; ?>&nbsp;Patients Attendance List<?php echo ' From: ' . @formatDate2Local($startdate, "dd/mm/yyyy") . ' To: ' . @formatDate2Local($enddate, "dd/mm/yyyy"); ?></h1>
            ...
            <table class="printout" border="1" cellspacing="0" cellpadding="0" align="center" bgcolor=#ffffdd>
                ...
                $end=$_GET['end'];                              
                                     ...
                                     $sql_count_patients="SELECT  cen.encounter_nr,ce.encounter_date as visit_date,cp.selian_pid, cp.name_first,cp.name_last,cp.sex,cp.insurance_ID, cen.nr,cen.notes,dx.ICD_10_description,dx.ICD_10_code,dx.doctor_name, cen.personell_name,cen.date as tarehe FROM care_encounter_notes cen INNER JOIN care_encounter ce ON ce.encounter_nr=cen.encounter_nr INNER JOIN care_person cp ON cp.pid=ce.pid INNER JOIN care_tz_diagnosis dx ON dx.encounter_nr=ce.encounter_nr WHERE cen.personell_name='".$doctor."' AND dx.doctor_name=cen.personell_name AND dx.diagnosis_type='final' AND date BETWEEN '".$start."' AND '".$end."' AND FROM_UNIXTIME(dx.timestamp,'%Y-%m-%d')=cen.date GROUP BY cen.encounter_nr ORDER BY date";
                                     ...
                                     while ($rowp=$result_count_patient->FetchRow()) {
                                        ...
                                        echo '<tr><td>'.$serial.'</td><td>'.date('d/m/Y',strtotime($rowp['visit_date'])).'</td><td>'.$rowp['name_first'].' '.$rowp['name_last'].'</td><td>'.$rowp['sex'].'</td><td>'.$insurance.'</td><td>'.$rowp['selian_pid'].'</td><td>'.$rowp['ICD_10_description'].'('.$rowp['ICD_10_code'].')'.'</td><td>'.$rowp['notes'].'</td></tr>';
    ...
    <HTML>
        <HEAD>
            ...
            <script language="javascript" >
                ...
                function printOut()
                {
                    urlholder = "./docs_presc_report_patients.php?doctor=<?php echo $doctor; ?>&printout=TRUE&start=<?php echo $startdate; ?>&end=<?php echo $enddate; ?>";
