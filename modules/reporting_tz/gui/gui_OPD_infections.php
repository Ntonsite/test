...
<DIV align="center">
	...
	if ($debug) {
		echo "elements in the array: " . sizeof($res_array) . "<br>";
	}
...
while (list($u, $v) = each($res_array)) {
	...
	$sql_no_u_age = "SELECT count(*) AS return_underage FROM $tmp_tbl_OPD_diagnostic
					   WHERE icd_10_code='$code' AND diagnosis_type='final' AND UNIX_TIMESTAMP(date_birth) > (UNIX_TIMESTAMP(DATE_SUB(CURDATE(), INTERVAL 5 year)))  AND timestamp>='$start' and timestamp<='$end'";


	$db_ptr_no_u_age = $db->Execute($sql_no_u_age);
	$db_row_no_u_age = mysqli_fetch_array($db_ptr_no_u_age); //israel 1
	...
	$db_row_no = mysqli_fetch_array($db_ptr_no); //israel 2
