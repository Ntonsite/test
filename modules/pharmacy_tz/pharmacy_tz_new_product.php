...
if ($debug==TRUE) {
    ...
    print_debug_info("Item number", $selian_item_number);
...
//isset($debug)) ? $db->debug = TRUE : $db->debug = FALSE;
...
$short_8 = $short[7];
$long_8 = $long[7];
$is_insured_8 = $is_insurance[7];
$short_9 = $short[8];
$long_9 = $long[8];
$is_insured_9 = $is_insurance[8];
$short_10 = $short[9];
$long_10 = $long[9];
$is_insured_10 = $is_insurance[9];
$short_11 = $short[10];
$long_11 = $long[10];
$is_insured_11 = $is_insurance[10];
$short_12 = $short[11];
$long_12 = $long[11];
$is_insured_12 = $is_insurance[11];
...
if (!empty($mode)) {
    ...
    if (($mode != "show") && ($mode != "edit") && ($mode != "erase") && (!$GO_BACK_TO_SEARCH)) { // show or edit are external calls of this site
        if ($product_obj->check_form_variable($selian_item_number)) {
            if ($debug)
                echo "ERROR: <b>item Number is empty</b>";
            $ERROR = TRUE;
            print_r($_GET);
    ...
    if ($mode == "show" || $mode == "edit" || $mode == "erase" || $mode == "update") {
        if (!empty($item_id) && empty($formular_sent)) {
            ...
            $selians_item_price_7 = $product_obj->get_selians_item_alt_price($item_id, 7);
            $selians_item_price_8 = $product_obj->get_selians_item_alt_price($item_id, 8);
            $selians_item_price_9 = $product_obj->get_selians_item_alt_price($item_id, 9);
            $selians_item_price_10 = $product_obj->get_selians_item_alt_price($item_id, 10);
...
if (!empty($mode) && !$ERROR) {
    ...
    if (empty($selians_item_price_7))
        $selians_item_price_7 = "0";
    if (empty($selians_item_price_8))
        $selians_item_price_8 = "0";
    if (empty($selians_item_price_9))
        $selians_item_price_9 = "0";
    if (empty($selians_item_price_10))
        $selians_item_price_10 = "0";
    if (empty($selians_item_price_11))
        $selians_item_price_11 = "0";
    ...
    $db_buffer['unit_price_7'] = $selians_item_price_7;
    $db_buffer['unit_price_8'] = $selians_item_price_8;
    $db_buffer['unit_price_9'] = $selians_item_price_9;
    $db_buffer['unit_price_10'] = $selians_item_price_10;
    ...
    if ($mode == "insert") {
        ...
        if ($product_obj->item_number_exists($selian_item_number)) {
            ...
            // insert item NHIF Schmes

            // $nhifSchemes = @($_GET['nhif_schemes'])?$_GET['nhif_schemes']:[];

            // if (@$nhif_schemes) {
            //     foreach ($nhif_schemes as $scheme) {
            //         if (@$scheme) {
            //             $sql = "INSERT INTO care_tz_drugsandservices_nhifschemes (scheme_id, item_id) VALUES($scheme, '$selian_item_number')";
            //             $db->Execute($sql); 
            //         }
    ...
    if ($mode == "update") {
        ...
        if ($product_obj->item_number_exists($selian_item_number)) {
            ...
            // if (@$nhifSchemes) {
            //     $deleteSQL = "DELETE FROM care_tz_drugsandservices_nhifschemes WHERE item_id = $selian_item_number";
            //     $db->Execute($deleteSQL); 

            //     foreach ($nhifSchemes as $scheme) {
            //         if (@$scheme) {
                        
            //             $insertSQL = "INSERT INTO care_tz_drugsandservices_nhifschemes (scheme_id, item_id) VALUES($scheme, '$selian_item_number')";
...
if ($selian_item_number) {
   
    // $itemSql = "SELECT * FROM care_tz_drugsandservices_nhifschemes WHERE item_id = '$selian_item_number' ";
    // $result = $db->Execute($itemSql);
    // if (@$result && $result->RecordCount() > 0 ) {
    //   $schemeItems = $result->GetArray();
    // }
}
