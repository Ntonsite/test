...
function insertValue(id) {
    if (id == "") {
        ...
        data=document.getElementById("jina"+id).value;
        
        xmlhttp.open("GET","pharmacy_tz_min_level.php?id="+id+"&data="+data,true);
        xmlhttp.send();
    }
}
   ...
   href="pharmacy_tz_product_catalog.php" ><img src="../../gui/img/control/default/en/en_close2.gif" border=0 width="103" height="24" alt="" style="filter:alpha(opacity=70)" onMouseover="hilite(this,1)" onMouseOut="hilite(this,0)"></a>  </td>
	...
	<tr>
      ...
      <td height="321" valign=top bgcolor=#ffffff> <ul>
          <p> <br>
          <form action="pharmacy_tz_search.php" method="get" name="suchform" id="suchform">
          ...
          <table width="70%" border="0" bgcolor=#ffffdd align="center"  style='border:1px dotted #cccccc;'>
            ...
            <td nowrap class="b r" width="10%">Restrict Over Dose</td>
              ...
              <td nowrap class="b r" width="8%">Unit Cost</td>
              ...
              <td nowrap class="b" width="5%"><?php echo $LDMinLevel; ?></td>
            </tr>
            <?php
              echo $http_buffer; ?>
          </table>
...
<script type="text/javascript">
  function updateDrugRow(item_id, columnName, id) {
    var columnValue = $("#"+id+item_id).val()
    $.get("updateNHIFRow.php", {columnValue: columnValue, columnName: columnName, columnId: item_id },
      function(result) {
    });
  }

  function deleteNHIFScheme(scheme_id) {
    if (confirm("Are you sure you want to remove this scheme?"))
    {
     $.get("deleteSchemeRow.php", {scheme_id: scheme_id},
      function(result) {
        $("#scheme"+scheme_id).remove();
      });
    }
  }

  function addNHIFScheme(scheme_id) {
    $("#schemepopover"+scheme_id).popover('show');
  }

   function showPricesPopover(item_id) {
    $("#pricespopover"+item_id).popover({
      "trigger": 'click',
       "html": true,
       "container": 'body',
       "content": function(){
        var div_id =  "tmp-id-" + $.now();
        return details_in_popup("getItemPrices.php?item_id="+item_id, div_id);
      }
    });
  }
  ...
  function closePopover(id) {
    $("#schemepopover"+id).popover('hide');
  }

  function saveNHIFScheme(scheme_id) {
    var scheme_value = $("#scheme_value"+scheme_id).val();

    $.get("addSchemeRow.php", {scheme_id: scheme_id, scheme_value: scheme_value},
    function(result) {
      $("#schemediv"+scheme_id).append("<span id='scheme"+result.scheme_id+"' >"+scheme_value+"<i class='material-icons ic' onclick='deleteNHIFScheme("+result.scheme_id+")'>delete</i></span><br>");

      $("#schemepopover"+scheme_id).popover('hide');

    });
  }


  function updateDrugItemPrice(item_id, priceIndex) {
    var itemValue = $("#itempricevalue"+priceIndex+item_id).val();

    $.get("updateDrugItemPrice.php", {item_id: item_id, priceIndex: priceIndex, itemValue:itemValue},
    function(result) {
     $("#itempricevalue"+priceIndex+item_id).attr('value', result.amount);
    });
