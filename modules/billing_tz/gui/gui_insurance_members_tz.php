...
<script language="javascript" src="../../js/check_insurance_form.js"></script>
	...
	$insurance_tz->ShowInsurancesDropDown('company_id', $company_id ?? 0);
} else {
...
if (strlen($keyword) > 0) {
	...
	if ($result) {
		while ($row = $result->FetchRow()) {
			echo '<option value="' . $row['pid'] . '">' . $row['selian_pid'] . ' - ' . $row['name_last'] . ', ' . ucwords($row['name_first']) . ' (' . $row['date_birth'] . ')</option>';
		}
	}
