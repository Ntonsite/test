 ...
 *
 * See the file "copy_nototatice.txt" for the licence notice
 */
...
if ($task == "insert") {
	...
	while (list($x, $v) = each($_POST)) {
		...
		if (strstr($x, "modepres_")) {
			...
			if ($_POST['modepres_' . $prescriptions_nr] == 'bill') {
				...
				$bill_obj->StorePrescriptionItemToBill($pid, $prescriptions_nr, $new_bill_number, $_POST['insurance_' . $prescriptions_nr], $_GET['unit_price'], $_POST['bankid']);
			...
			if ($_POST['modelab_' . $labtest_nr] == 'bill') {
				...
				$bill_obj->StoreLaboratoryItemToBill($pid, $labtest_nr, $batch_nr, $new_bill_number, $_POST['insurance_' . $labtest_nr], $_GET['unit_price'], $_POST['bankid']);
			...
			if ($_POST['moderad_' . $radtest_nr] == 'bill') {
				...
				$bill_obj->StoreRadiologyItemToBill($pid, $radtest_nr, $new_bill_number, $_POST['insurance_' . $radtest_nr], $_GET['unit_price'], $_POST['bankid']);
	...
	if ($billcounter) {
		...
		if ($clear_bill) {
			...
			$bill_obj->StoreAdvancePaymentToBill($pid, $new_bill_number, 0, $balance, $notes, $description, $_POST['bankid']);
