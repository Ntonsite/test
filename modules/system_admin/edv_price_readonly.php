...
if ($_REQUEST['change'] == true) {
	$sql = 'UPDATE care_config_global SET `value` = \'' . $_REQUEST['mark_readonly'] . '\', `modify_time` = NOW() WHERE CONVERT(`care_config_global`.`type` USING utf8) = \'price_readonly\' LIMIT 1;';
	$db->Execute($sql);
}

$glob_obj->getConfig('price_readonly');
if (empty($GLOBAL_CONFIG['price_readonly'])) {
	$sql = 'INSERT INTO care_config_global (`type`, `value`, `notes`, `status`, `history`, `modify_id`, `modify_time`, `create_id`, `create_time`) '
		. 'VALUES (\'price_readonly\', \'0\', NULL, \'\', \'\', \'\', NOW(), \'\', NOW());';
	$db->Execute($sql);
}
?>
                    ...
                    <td  valign="top" align="middle" height="35">
                        <table cellspacing="0"  class="titlebar" border=0>
                            <tr valign=top  class="titlebar" >
                                <td bgcolor="#99ccff" >
                                    &nbsp;&nbsp;<font color="#330066">PRICE READ ONLY</font>
                                </td>
...
if (empty($_REQUEST['mark_readonly'])) {
	$_REQUEST['mark_readonly'] = '';
}
switch ($_REQUEST['mark_readonly']) {
case '1':
	$message = 'Success!';
	break;
...
?>
                                ...
                                <tr>
                                    <td class="adm_item" align="right"><FONT  color="#0000cc"><b><?php echo 'READ ONLY'; ?></b></FONT></td>
                                    <td  align="left">




                                        <select name="mark_readonly">
                                            <?php
$price_readonly = $glob_obj->getConfigValue('price_readonly');
if (@$price_readonly && $price_readonly == 1) {
