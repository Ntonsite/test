...
if (@$mode && $mode != '') {
	...
	if ((@$mode && $mode == 'save' && empty($error)) || ($mode == 'update' && empty($errordescr))) {
		...
		while (list($x, $v) = $helper->CustomEach($_POST)) {
			if (empty(preg_match('/_a_/', $x))) {
				if (empty(preg_match('/Sys/', $x))) {
					continue;
				}
			}
			if ($_POST[$x] != '') {
		...
		if ($p_areas != '') {
			if ($mode == 'save') {
				...
				$sql = "UPDATE care_user_roles SET description = '" . addslashes($description) . "',"
					. " permission='" . $p_areas . "', modify_id='" .$_SESSION['sess_user_name']. "'  WHERE role_id='$role_id'";
...
?>
    ...
    <?php
if ((@$mode && $mode == "") and ($remark != 'fromlist')) {
	$gtime = date('H.i');
	if ($gtime < '9.00') {
		echo $LDGoodMorning;
	}
...
while (list($x, $v) = $helper->CustomEach($area_opt)) {

	echo '<tr  bgcolor="white">';

	if (preg_match('/title/', $x)) {
		// If title print it out
