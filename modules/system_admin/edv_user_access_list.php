...
$sql = 'SELECT * FROM care_users cu, care_user_roles cur WHERE '
        . 'cu.role_id = cur.role_id ORDER BY login_id';
...
while ($zeile = $ergebnis->FetchRow()) {
    ...
    echo '</td>
					<td> ' . formatDate2Local($zeile[16], $date_format) . ' / ' . convertTimeToLocal($zeile['s_time']) . ' </td>';
    ...
    echo "
					<td>
					<a href=edv_user_access_edit.php?sid=" . $sid . "&lang=" . $lang . "&mode=edit&userid=" . str_replace(' ', '+', $zeile['login_id']) . "&role_id=" . $zeile['role_id'] . " title=\"$LDChange\"> $LDInitChange</a> \n
    ...
    echo "</a> \n
			<a href=edv_user_access_delete.php?sid=" . $sid . "&lang=" . $lang . "&itemname=" . str_replace(' ', '+', $zeile['login_id']) . " title=\"$LDDelete\">	$LDInitDelete</a>\n
			<a href=edv_change_user_role.php?sid=" . $sid . "&lang=" . $lang . "&itemname=" . str_replace(' ', '+', $zeile['login_id']) . " title=\"$LDChangeRole\">$LDInitChangeRole</a>
...
if ($ck_edvzugang_src == "listpass")
    echo "edv-accessplan-list-pass.php";
else
    echo "edv_user_access_edit.php";
?>" >
