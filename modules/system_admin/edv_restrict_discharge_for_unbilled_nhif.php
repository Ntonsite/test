...
if ($_REQUEST['change'] == true) {
	$sql = 'UPDATE care_config_global SET `value` = \'' . $_REQUEST['restrict_discharge_nhif'] . '\', `modify_time` = NOW() WHERE CONVERT(`care_config_global`.`type` USING utf8) = \'restrict_discharge_for_unbilled_nhif\' LIMIT 1;';
	$db->Execute($sql);
    $message = 'Updated';
}

$glob_obj->getConfig('restrict_discharge_for_unbilled_nhif');
if (empty($GLOBAL_CONFIG['restrict_discharge_for_unbilled_nhif'])) {
	$sql = 'INSERT INTO care_config_global (`type`, `value`, `notes`, `status`, `history`, `modify_id`, `modify_time`, `create_id`, `create_time`) '
		. 'VALUES (\'restrict_discharge_for_unbilled_nhif\', \'0\', NULL, \'\', \'\', \'\', NOW(), \'\', NOW());';
	$db->Execute($sql);
}
?>
                    ...
                    <td  valign="top" align="middle" height="35">
                        <table cellspacing="0"  class="titlebar" border=0>
                            <tr valign=top  class="titlebar" >
                                <td bgcolor="#99ccff" >
                                    &nbsp;&nbsp;<font color="#330066">Restrict Discharge UnBilled NHIF Patients</font>
                                </td>
                        ...
                        <form  method="post" name="form">
                            <table border=0 cellspacing=1 cellpadding=5>
                                <tr>
                                    ...
                                    <td class="adm_item" align="right"><FONT  color="#0000cc"><b><?php echo 'Restrict discharge for Unbilled NHIF?'; ?></b></FONT></td>
                                    <td  align="left">

                                        <select name="restrict_discharge_nhif">
                                            <?php
$restrict_discharge_nhif = $glob_obj->getConfigValue('restrict_discharge_for_unbilled_nhif');
if ($restrict_discharge_nhif == 1) {
