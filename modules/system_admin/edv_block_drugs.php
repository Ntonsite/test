...
if ($_REQUEST['change'] == true) {
	$sql = 'UPDATE care_config_global SET `value` = \'' . $_REQUEST['markos'] . '\', `modify_time` = NOW() WHERE CONVERT(`care_config_global`.`type` USING utf8) = \'auto_out_of_stock\' LIMIT 1;';
	$db->Execute($sql);
}

$glob_obj->getConfig('auto_out_of_stock');
if (empty($GLOBAL_CONFIG['auto_out_of_stock'])) {
	$sql = 'INSERT INTO care_config_global (`type`, `value`, `notes`, `status`, `history`, `modify_id`, `modify_time`, `create_id`, `create_time`) '
		. 'VALUES (\'auto_out_of_stock\', \'0\', NULL, \'\', \'\', \'\', NOW(), \'\', NOW());';
	$db->Execute($sql);
}
?>
                    ...
                    <td  valign="top" align="middle" height="35">
                        <table cellspacing="0"  class="titlebar" border=0>
                            <tr valign=top  class="titlebar" >
                                <td bgcolor="#99ccff" >
                                    &nbsp;&nbsp;<font color="#330066">Enable Doctor to see stock status</font>
                                </td>
...
if (empty($_REQUEST['markos'])) {
	$_REQUEST['markos'] = '';
}
switch ($_REQUEST['markos']) {
case '1':
	$message = 'Success!';
	break;
...
?>
                                ...
                                <tr>
                                    ...
                                    <td  align="left">
                                        ...
                                        <select name="markos">
                                            <?php
$auto_out_of_stock = $glob_obj->getConfigValue('auto_out_of_stock');
