...
if (@$from && $from == 'set') {
	$returnfile = 'edv_system_format_currency_set.php' . URL_APPEND . '&from=add';
} else {
...
if ((@$mode && $mode == 'save') && @$short_name && @$long_name && @$info) {
...
if (empty($item_no)) {
	$smarty->assign('sOnLoadJs', 'onLoad="document.c_form.short_name.focus()"');
}
...
<?php
if ((@$mode && $mode == 'save') && @$new_currency_ok) {
	echo '<img ' . createMascot($root_path, 'mascot1_r.gif', '0', 'absmiddle') . '> ' . $saved_msg . '<p>';
}

if (@$item_no) {
	echo $LDPlsEnterUpdate;
} else {
	echo $LDPlsAddCurrency;
}
...
<?php if ($item_no) {
	$save_button = 'update.gif';
} else {
	$save_button = 'savedisc.gif';
}
?>
...
<?php if ($item_no): ?>
            ...
            <input type="hidden" name="validator" value="<?php for ($i = 0; $i < sizeof($LDDateFormats); $i++) {
	echo $LDDateFormats[$i] . "_";
}
?>">
    </form>
    <p><br><p>
