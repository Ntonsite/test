...
if ($_REQUEST['change'] == true) {
	$sql = 'UPDATE care_config_global SET `value` = \'' . $_REQUEST['ShowPriceOnPr'] . '\', `modify_time` = NOW() WHERE CONVERT(`care_config_global`.`type` USING utf8) = \'show_price_on_prescription\' LIMIT 1;';
    $db->Execute($sql);
}

$glob_obj->getConfig('show_price_on_prescription');
if (empty($GLOBAL_CONFIG['show_price_on_prescription'])) {
	$sql = 'INSERT INTO care_config_global (`type`, `value`, `notes`, `status`, `history`, `modify_id`, `modify_time`, `create_id`, `create_time`) '
		. 'VALUES (\'show_price_on_prescription\', \'0\', NULL, \'\', \'\', \'\', NOW(), \'\', NOW());';
	$db->Execute($sql);
}
?>
                    ...
                    <td  valign="top" align="middle" height="35">
                        <table cellspacing="0"  class="titlebar" border=0>
                            <tr valign=top  class="titlebar" >
                                <td bgcolor="#99ccff" >
                                    &nbsp;&nbsp;<font color="#330066">SHOW PRICE ON PRESCRIPTION</font>
                                </td>
...
if (empty($_REQUEST['ShowPriceOnPr'])) {
	$_REQUEST['ShowPriceOnPr'] = '';
}
switch ($_REQUEST['ShowPriceOnPr']) {
case '1':
	$message = 'Success!';
	break;
...
?>
                                ...
                                <tr>
                                    <td class="adm_item" align="right"><FONT  color="#0000cc"><b><?php echo 'Enable Price'; ?></b></FONT></td>
                                    <td  align="left">
                                        ...
                                        <select name="ShowPriceOnPr">
                                            <?php
$show_price_on_pr=$glob_obj->getConfigValue('show_price_on_prescription');

if (@$show_price_on_pr && $show_price_on_pr == 1) {
