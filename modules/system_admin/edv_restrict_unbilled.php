...
if ($_REQUEST['change'] == true) {
	$sql = 'UPDATE care_config_global SET `value` = \'' . $_REQUEST['restrict_unbilled_items'] . '\', `modify_time` = NOW() WHERE CONVERT(`care_config_global`.`type` USING utf8) = \'restrict_unbilled_items\' LIMIT 1;';
	$db->Execute($sql);
}

$glob_obj->getConfig('restrict_unbilled_items');
if (empty($GLOBAL_CONFIG['restrict_unbilled_items'])) {
	$sql = 'INSERT INTO care_config_global (`type`, `value`, `notes`, `status`, `history`, `modify_id`, `modify_time`, `create_id`, `create_time`) '
		. 'VALUES (\'restrict_unbilled_items\', \'0\', NULL, \'\', \'\', \'\', NOW(), \'\', NOW());';
	$db->Execute($sql);
}
?>
                    ...
                    <td  valign="top" align="middle" height="35">
                        <table cellspacing="0"  class="titlebar" border=0>
                            <tr valign=top  class="titlebar" >
                                <td bgcolor="#99ccff" >
                                    &nbsp;&nbsp;<font color="#330066">Restrict Viewing/Access to Not Billed Items Configuration</font>
                                </td>
                        ...
                        <form  method="post" name="form">
                            <table border=0 cellspacing=1 cellpadding=5>
                                <tr>
                                    <td class="adm_item" align="right"><FONT  color="#0000cc"><b><?php echo 'Enable the Restriction to not Billed Items?'; ?></b></FONT></td>
                                    <td  align="left">

                                        <select name="restrict_unbilled_items">
                                            <?php
$restrict_unbilled_items = $glob_obj->getConfigValue('restrict_unbilled_items');
if ($restrict_unbilled_items == 1) {
