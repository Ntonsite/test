...
if (@$_REQUEST['change'] && $_REQUEST['change'] == true) {
	$sql = 'UPDATE care_config_global SET `value` = \'' . $_REQUEST['transmit_to_weberp_enabled'] . '\', `modify_time` = NOW() WHERE CONVERT(`care_config_global`.`type` USING utf8) = \'transmit_to_weberp_enabled\' LIMIT 1;';
	$db->Execute($sql);
}

$glob_obj->getConfig('transmit%');
if (empty($GLOBAL_CONFIG['transmit_to_weberp_enabled'])) {
	$sql = 'INSERT INTO care_config_global (`type`, `value`, `notes`, `status`, `history`, `modify_id`, `modify_time`, `create_id`, `create_time`) VALUES (\'transmit_to_weberp_enabled\', \'0\', NULL, \'\', \'\', \'\', NOW(), \'\', \'0000-00-00 00:00:00\');';
	$db->Execute($sql);
}
?>
                            ...
                            <table border=0 cellspacing=1 cellpadding=5>
                                <tr>
                                    ...
                                    <td  align="left">
                                        ...
                                        <select name="transmit_to_weberp_enabled">
<?php
$weberpenabled = $glob_obj->getConfigValue('transmit_to_weberp_enabled');
if (@$weberpenabled) {
