...
if ($_REQUEST['change'] == true) {
	$sql = 'UPDATE care_config_global SET `value` = \'' . $_REQUEST['enable'] . '\', `modify_time` = NOW() WHERE CONVERT(`care_config_global`.`type` USING utf8) = \'nurse_chart_deduct_stock\' LIMIT 1;';
	$db->Execute($sql);
}

$glob_obj->getConfig('nurse_chart_deduct_stock');
if (empty($GLOBAL_CONFIG['nurse_chart_deduct_stock'])) {
	$sql = 'INSERT INTO care_config_global (`type`, `value`, `notes`, `status`, `history`, `modify_id`, `modify_time`, `create_id`, `create_time`) '
		. 'VALUES (\'nurse_chart_deduct_stock\', \'0\', NULL, \'\', \'\', \'\', NOW(), \'\', NOW());';
	$db->Execute($sql);
}
?>
                    ...
                    <td  valign="top" align="middle" height="35">
                        <table cellspacing="0"  class="titlebar" border=0>
                            <tr valign=top  class="titlebar" >
                                <td bgcolor="#99ccff" >
                                    &nbsp;&nbsp;<font color="#330066">Enable nurse Chart to Deduct stock</font>
                                </td>
...
?>
                                ...
                                <tr>
                                    ...
                                    <td  align="left">
                                        <select name="enable">
                                            <?php
$auto_out_of_stock = $glob_obj->getConfigValue('nurse_chart_deduct_stock');
