...
if (isset($mode) && !empty($mode)) {
	if ($mode == 'save') {
		...
		$postname = $_POST['name'];
		if (@$postname) {
			$postname = str_replace("%", "1", $postname);
			$postname = str_replace("#", "2", $postname);
		}

		$_POST['id'] = "_" . cleanString(strtolower($postname)) . '__' . strtolower($_POST['group_id']);
		if (@$_POST['id']) {
			$_POST['id'] = str_replace("%", "1", $_POST['id']);
			$_POST['id'] = str_replace("#", "2", $_POST['id']);
		}
...
?>></img>
                    ...
                    <a href="<?php echo $_SESSION['testGroupAdminURL'] ?>" ><img <?php echo createLDImgSrc($root_path, 'close2.gif', '0') ?>  <?php if ($cfg['dhtml']) {
	echo 'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)>';
}
?>></img>
                    </a>
