...
if ($stored_request['batch_nr']) {
	$sTitle = $sTitle . "  (" . $stored_request['batch_nr'] . " " . $stored_request['room_nr'] . " " . $stored_request['dept'] . ")";
}
...
<!--
    ...
    function chkForm(d)
    {
        ...
        if (d.result.value == "" || d.result.value == " ")
            return false;
...
if (($stored_request['result'] != '') && $stored_request['status'] != 'done') {
	...
	$TP_send_doctor = '';
	if ($edit_form || $read_form) {
		$TP_send_doctor_x = stripslashes($stored_request['send_doctor']);
	} else {
		$TP_send_doctor_x = '';
	}
	...
	if ($stored_request['result']) {
		$TP_result = stripslashes($stored_request['result']);
	} else {
		$TP_result = '';
	}
	...
	if ($stored_request['result_date'] != DBF_NODATE) {
		$TP_report_date .= formatDate2Local($stored_request['result_date'], $date_format) . '"';
	} else {
		$TP_report_date .= formatDate2Local(date('Y-m-d'), $date_format) . '"';
	}
	...
	if ($stored_request['result_doctor']) {
		$TP_result_doctor = stripslashes($stored_request['result_doctor']);
	} else {
		$TP_result_doctor = '';
	}
