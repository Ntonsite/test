...
if (empty($mode)) /* Get the pending test requests with no specimen/sample collected */ {
...
$sqlconf="SELECT value FROM care_config_global WHERE type='restrict_unbilled_items'";
$resultconf=$db->Execute($sqlconf);
if ($confrow=$resultconf->FetchRow()) {
	$confValue=$confrow['value'];
    

}

if ($confValue=="1" && $patient['encounter_class_nr'] == "2" && $patient['insurance_ID'] === '0') {
...
$_SESSION['batch_nr'] = $batch_nr;
$_SESSION['labPn'] = $pn;
$_SESSION['lab_form_title'] = $formtitle;
