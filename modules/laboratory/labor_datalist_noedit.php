...
$datetime2 = date_create($patient['date_birth']); 
...
$dobDiff=abs($days);
	...
	case ($dobDiff >= 361) and ($dobDiff <= 5040):
		...
		break;
	case $dobDiff > 5040:
		$p_hi_bound = @$pName->fields['lo_bound'] ? $pName->fields['lo_bound'] : "";
		if ($patient['sex'] == 'm' || $patient['sex']=='M') {
			if (@$pName->fields['hi_bound'] && $paramValue > $pName->fields['hi_bound']) {
				$txt .= '<img ' . createComIcon($root_path, 'arrow_red_up_sm.gif', '0', '', TRUE) . '> <font color="red">' . htmlspecialchars($paramValue) . '</font>';
			} elseif ($paramValue < $p_hi_bound) {
				$txt .= '<img ' . createComIcon($root_path, 'arrow_red_dwn_sm.gif', '0', '', TRUE) . '> <font color="red">' . htmlspecialchars($paramValue) . '</font>';
			} else {
				$txt .= htmlspecialchars($paramValue);
			}
		} elseif ($patient['sex'] == 'f' || $patient['sex']=='F' ) {
...
if ($encounter = $enc_obj->getBasic4Data($encounter_nr)) {
	...
	$fileRecs = $lab_obj->getFileResults($encounter_nr);

	if ($rows = $lab_obj->LastRecordCount()) {
		$fileRecords = array();
		while ($fileRec = $fileRecs->FetchRow()) {
			$fileRec['date'] = date('d/m/Y H:i', strtotime($fileRec['test_date'] . ' ' . $fileRec['test_time']));
			$fileRecords[] = $fileRec;
		}
	}
...
//        urlholder = "<?php echo $root_path ?>modules/pdfmaker/laboratory/report_all.php<?php echo URL_REDIRECT_APPEND; ?>&encounter_nr=" + enc + "&skipme=" + skipme + "&userId=" + userId;
        ...
        urlholder = "<?php echo $root_path ?>modules/laboratory/labor_test_findings_chemlabor_print.php<?php echo URL_REDIRECT_APPEND; ?>&encounter_nr=" + enc + "&skipme=" + skipme + "&userId=" + userId;
        if (!skipme) {
            alert('Please Select Results to Print!');
        } else {
            var openwin = window.open(urlholder, '<?php echo $LDOpenReport; ?>', "width=750,height=500,menubar=no,resizable=yes,scrollbars=yes");

        }
    }

    function openFileResults(path) {
        window.open(path, '<?php echo $LDOpenReport; ?>', "width=750,height=500,menubar=no,resizable=yes,scrollbars=yes")
    }
//  Script End -->
</script>
...
if ($records) {
	...
	$cache = '
		...
		';
	while (list($x, $v) = $helper->customEach($tdate)) {
		$cache .= '
	...
	while (list($x, $v) = $helper->customEach($ttime)) {
		$cache .= '
...
//echo $requestData;
	while (list($groupId, $paramEnc) = $helper->customEach($requestData)) {
		...
		while (list($paramId, $encounterNr) = $helper->customEach($paramEnc)) {
			$pName = $lab_obj->TestParamsDetails($paramId);

			$p_name = @$pName->fields['name'] ? $pName->fields['name'] : "";
			$p_lo_bound = @$pName->fields['lo_bound'] ? $pName->fields['lo_bound'] : "";
			$p_hi_bound = @$pName->fields['hi_bound'] ? $pName->fields['hi_bound'] : "";
			$p_msr_unit = @$pName->fields['msr_unit'] ? $pName->fields['msr_unit'] : "";

			$cache .= "<tr>";
			$cache .= "<td class=\"" . $class . "\">" . $p_name . "</td>";
			$cache .= "<td class=\"" . $class . "\">" . $p_lo_bound . " - " . $p_hi_bound . "</td>";
			$cache .= "<td class=\"" . $class . "\">" . $p_msr_unit . "</td>";
			...
			$p_id = @$pName->fields['id'] ? $pName->fields['id'] : "";
			$cache .= "<td align=\"right\" colspan=\"" . ($cols - $columns + 1) . "\" class=\"" . $class . "\"><input type=\"checkbox\" name=tk value=\"" . $p_id . "\"></td></tr>";
...
$disp="";
$disp.="<br><b>Comment from Laboratory</b>";
$disp.='<div>';
$disp.='<pre>';
$result_labcomment=$db->Execute($labcommentSQL);
while ($rows_labc=$result_labcomment->FetchRow()) {
	$disp.=$rows_labc['lab_comment'].'.'.'<p>';
...
$smarty->assign('slabcomments', $disp);




$smarty->assign('sLabFileRecords', $fileBtns);
