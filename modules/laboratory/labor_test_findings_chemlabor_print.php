...
if ($records) {
	...
	$cache .= '<p style="font-size:20px">' . $main_facility_name . '</p>';
	$cache .= $main_address . '<br>';
	$cache .= $main_email . '<br>';
	$cache .= "PHONE:".$main_phone. '<br>';
...
//display the values
	...
	while (list($groupId, $paramEnc) = each($requestData)) {
		...
		while (list($paramId, $encounterNr) = each($paramEnc)) {
			...
			$cache .= "<td class=\"" . $class . "\">" . $pName->fields['lo_bound'] . " - " . $pName->fields['hi_bound'] . "</td>";
...
//            $class == 'wardlistrow1' ? $class = 'wardlistrow2' : $class = 'wardlistrow1';
	...
	//echo 'ggggggggggggggggggggggggggggggg '.$bchnr;

	$sqlTestBy="SELECT history FROM care_test_request_chemlabor WHERE batch_nr=".$bchnr;
	$history_result=$db->Execute($sqlTestBy);
	if ($row_history=$history_result->FetchRow()) {
		$personelName=$row_history['history'];

		$namearray=explode('=', $personelName);
		$personelName=$namearray[1];
