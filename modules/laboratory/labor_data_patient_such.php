        ...
        <tr bgcolor="#DDE1EC" >
            ...
            <td valign=top><p><br>
                <ul>
                    ...
                    <FORM action="<?php echo $thisfile; ?>" method="get" name="sform" onSubmit="return checkForm(sform.keyword)">
...
/* 	for($i=0;$i<sizeof($LDfieldname);$i++) {
	...
	while ($zeile = $encounter->FetchRow()) {
		...
		if ($lab_obj->IsMissingLabResult($zeile['batch_nr']) == 1) {
			echo ' <img ' . createComIcon($root_path, 'check2.gif', '0', '', TRUE) . '></img>';
		}
# all results are in the list
		elseif ($lab_obj->IsMissingLabResult($zeile['batch_nr']) == -1) {
			echo ' <img ' . createComIcon($root_path, 'check-r.gif', '0', '', TRUE) . '></img>';
		}
# not all results are written to the file
		else {
			echo ' <img ' . createComIcon($root_path, 'exclama.gif', '0', '', TRUE) . '></img>';
		}
# not even one results are written down to file
