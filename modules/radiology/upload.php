...
$encounter = '';
//Check if patient is currently admitted if not reject upload
require_once $root_path . 'include/care_api_classes/class_encounter.php';
$enc_obj = new Encounter;

if (!$enc_obj->isPIDCurrentlyAdmitted($pid)) {
	echo '<script>'
...
if (!isset($encounter_nr)) {
	//Get current encounter number from pid
	require_once $root_path . 'include/care_api_classes/class_mini_dental.php';
	$oc = new dental;
	$encounter = $oc->GetEncounterFromPid($pid);
} else {
	...
	for ($i = 0; $i < $maxpic; $i++) {
		...
		if ($img->isValidUploadedImage($_FILES[$picfile])) {
			...
			$data = array('pid' => $pid,
				...
				'img_type' => @$imageTypes[1] ? $imageTypes[1] : 'dicom',
				'notes' => $_POST['notes'],
...
if ($mode == 'show' && $nr) {
	...
	$files = &$img->FilesListArray($imgpath);
