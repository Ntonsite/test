...
$smarty->assign('LDBloodGroup', 'Blood group');
if (@$blood_group) {
	$buf = 'LD' . $blood_group;
...
if ($mode == 'new') {
	?>
                            ...
                            <td bgcolor="#ffffee"  colspan=2><input type="text" name="encounter_nr" size=11 maxlength=11 value="<?php echo $encounter; ?>">
                            </td>
                        </tr>
                        <tr>
...
if ($mode == 'show') {
	if ($rows) {
		...
		if (!$pop_only) {
			$img_torso = createComIcon($root_path, 'torso.gif', '0');
		}
...
$i = 1;
		...
		while (list($x, $v) = each($files)) {
			...
			$type = explode('.', $v);
			$imageType = @$type[1] ? $type[1] : 'dicom';
			$jpgHolder = 'src="../../gui/img/common/default/fileman.gif" border=0 width="18" height="18"';
			echo '
		 		<tr>
			...
			if (!$pop_only) {
				if ($imageType == 'dcm') {
					echo '
                    <td class="v12" align=center>
                    <a href="dicom_launch_single.php' . URL_APPEND . '&pid=' . $pid . '&img_nr=' . $nr . '&fn=' . $v . '" title="' . $LDViewInFrame . '"><img ' . $img_torso . '></a>
                    </td>';
				} else {
					echo '
                    <td class="v12" align=center>
                    <a href="#" onclick="openFile(\'' . $imgpath . '/' . $v . '\')"  title="' . $LDViewInFrame . '"><img ' . $jpgHolder . '></a>
                    </td>';
				}
			}
