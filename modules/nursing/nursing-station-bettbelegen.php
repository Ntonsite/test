...
<!--
    ...
    function belegen(anum) {
        if ((anum == "lock") && (!confirm("<?php echo $LDConfirmLock ?>")))
            return;
...
<table width=100% border=0 cellpadding="5" cellspacing=0 >
    ...
    <tr>
        <td>
            ...
            <a href="javascript:belegen1('lock')" title="<?php echo $LDClk2LockBed ?>"><img <?php echo createComIcon($root_path, 'delete2.gif', '0', 'absmiddle', TRUE) ?> alt="<?php echo $LDClk2LockBed ?>"> <?php echo $LDLockThisBed ?></a>
            <p>
            <ul>
                ...
                if ($result = $db->Execute($sql)) {
                    ...
                    if ($rows == 1) {
                        ...
                        while ($row = $ergebnis->FetchRow()) {
                            if (eregi($row['patnum'], $bed_occ['bed_patient']))
                                continue;
                            else {
                                $new_zeile[] = $row;
                                $pat_count++;
