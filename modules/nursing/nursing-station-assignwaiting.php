...
if ($encounter['death_date'] && $encounter['death_date'] != DBF_NODATE && $encounter['death_date'] != '1980-01-01') {
	...
	for ($i = $ward_info['room_nr_start']; $i <= $ward_info['room_nr_end']; $i++) {
		...
		for ($j = 1; $j <= $room_info['nr_of_beds']; $j++) {
			...
			if ($is_patient && ($bed['encounter_nr'] != "")) {
				...
				if (isset($sln) && $sln) {
					$smarty->assign('sFamilyName', eregi_replace($sln, '<span style="background:yellow">' . ucfirst($sln) . '</span>', ucfirst($bed['name_last'])));
				} else {
					$smarty->assign('sFamilyName', ucfirst($bed['name_last']));
				}
				...
				if (isset($sfn) && $sfn) {
					$smarty->assign('sName', eregi_replace($sfn, '<span style="background:yellow">' . ucfirst($sln) . '</span>', ucfirst($bed['name_first'])));
				} else {
					$smarty->assign('sName', ucfirst($bed['name_first']));
				}
			...
			$sBuffer = '';
			if ($bed['insurance_class_nr'] != 2) {
				$sBuffer = $sBuffer . '<font color="#ff0000">';
			}

			if (isset($$bed['insurance_LDvar']) && !empty($$bed['insurance_LDvar'])) {
				$sBuffer = $sBuffer . $$bed['insurance_LDvar'];
			} else {
				$sBuffer = $sBuffer . $bed['insurance_name'];
			}
