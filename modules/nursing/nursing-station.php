...
function treatmentsheet(pn) {

<?php
/* if ($edit) */{
	echo '
        urlholder="nursing-station-treatment-sheet.php' . URL_REDIRECT_APPEND;
	echo '&pn=" + pn + "';
	echo "&pday=$pday&pmonth=$pmonth&pyear=$pyear&edit=$edit&station=" . $ward_info['name'];
	echo '";';
...
//                $sBuffer = '<a href="javascript:popPic(\'' . $bed['pid'] . '\')">';
				...
				// Else show the image link to assign bed to patient
...
//                print_r($dateArr);
				if ($dateArr['hour'] < 10) {
					//Add leading zero to hours if < 10
					$dateArr['hour'] = '0' . $dateArr['hour'];
				}

				if ($dateArr['minute'] < 10) {
			...
			if ($edit) {
				if (($is_patient) && !empty($bed['encounter_nr'])) {
					...
					if ($insurance_name == "NHIF" && $_SESSION['transfer_nhif_patients'] == 1) {
						...
						$transferlink = '<a href="javascript:Transfer(\'' . $bed['encounter_nr'] . '\')"><img ' . createComIcon($root_path, 'xchange.gif', '0', '', TRUE) . ' alt="' . $LDTransferPatient . '"></a>';
						$transferlink .= '<i onClick="transferNHIFPatient(\'' . $bed['encounter_nr'] . '\')" class="material-icons icon-blue">keyboard_tab</i>';

						$smarty->assign('sTransferIcon', $transferlink);
					} else {
					...
					$smarty->assign('sTreatmentSheet', '<a href="javascript:treatmentsheet(\'' . $bed['encounter_nr'] . '\')"><img ' . createComIcon($root_path, 'monitor.gif', '0', '', TRUE) . ' alt="' . $LDShowPatData . '"></a>');
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <button type="button" class="close" onclick="closeDischargedPatientsModal()" aria-label="Close">
...
$today = date('Y-m-d');
$patients = array();
$sql = "SELECT * FROM care_encounter WHERE is_discharged = 1 AND discharge_date = '$today'";
$result = $db->Execute($sql);
if (@$result && $result->RecordCount() > 0) {
	$patients = $result->GetArray();
}
foreach ($patients as $key => $patient) {
	...
	if (@$patientRow) {
		$patients[$key]['p_name'] = $patientRow['name_last'] . " " . $patientRow['name_first'] . " " . $patientRow['name2'] . " " . $patientRow['name3'];
		$patients[$key]['birth_date'] = date('d/m/Y', strtotime($patientRow['date_birth']));
	}
}
?>
            ...
            <table class="table datatable2 table-striped table-bordered" style="width:100%">
                ...
                <tbody>
                    ...
                    <?php foreach ($patients as $patient): ?>
                        <tr id="patient_<?php echo $patient['encounter_nr'] ?>">
                            <td><?php echo $patient['p_name'] ?></td>
                            <td><?php echo $patient['birth_date'] ?></td>
                            <td><?php echo $patient['pid'] ?></td>
                            <td><?php echo date('d/m/Y', strtotime($patient['encounter_date'])) ?></td>
                            <td><?php echo date('H:i:s', strtotime($patient['discharge_time'])) ?></td>
                            <td>
                                <button class='btn btn-sm btn-info' onclick="unDischargePatient(<?php echo $patient['encounter_nr'] ?>, '<?php echo $patient['p_name'] ?>')">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-sm btn-secondary" onclick="closeDischargedPatientsModal()" >Close</button>
      </div>
    </div>
  </div>
</div>
