...
if ($user_origin == 'lab') {
	...
	if ($user_origin == 'bill') {
		...
		$breakfile = $root_path . "modules/laboratory/labor_test_request_aftersave.php" . URL_REDIRECT_APPEND . "&edit=$edit&saved=insert&pn=$pn&station=$station&user_origin=$user_origin&status=$status&target=chemlabor&noresize=$noresize&batch_nr=$batch_nr";
...
$p_obj = $person_obj->getValueByList('membership_nr', $_SESSION['sess_pid']);
$card_no = $p_obj['membership_nr'];
...
$coreObj = new Core;
// echo "<pre>"; print_r($paramlist); echo "</pre>"; die;
switch ($mode) {
case 'save':
	...
	if (prepareTestElements()) {
		...
		if ($diag_obj->insertDataFromInternalArray()) {
			...
			$insertedSQL = "SELECT MAX(batch_nr) as batch_nr FROM care_test_request_chemlabor";
			$insertedResult = $db->Execute($insertedSQL);
			$insertedRow = $insertedResult->FetchRow();
			$batch_nr = $insertedRow['batch_nr'];
			...
			foreach ($singleParam as $key => $value) {
				...
				$item = $pres_obj->GetItemIDByNumber($diag_obj->getItemNrByParamID($tmpParam[0]));
				$parsedParamList['item_id'] = $item['item_id'];
				$parsedParamList['nhif_item_code'] = $item['nhif_item_code'];
				$parsedParamList['nhif_approval_number']=$_POST['Approval_'.$item['item_id']];
...
//echo 'z'.$diag_obj->getItemNrByParamName($tmpParam[0]).'z';
				...
				$sqlInner2 = "SELECT * from care_tz_laboratory_param WHERE name='" . $testname . "'";
				...
				$pres_obj->insert_prescription($pn, $item_id, 1,$_POST['Approval_'.$item['item_id']]);
//               echo 'x'.$item_id.'x';
			...
			/*
				                 * Open here the request form for printing or editing
			*/
//                header("location:" . $breakfile);
			//
			//Alert the user data has been saved and Redirect
			//                echo '<script>'
...
<script language="javascript">
    ...
    function setM(m, editable,item_id,isNHIF,restricted,nhif_item_code) {
    	eval("marker=document.images." + m);
        eval("element=document." + form_name + "." + m);
        if (editable == 'no') {
              //start NHIF stuff
            if (isNHIF&&restricted==1) {
              	var nhif_approval_no = prompt('Please Enter NHIF approval Number Below');
              	var valid_approval = verify_nhif_approval(card_no, nhif_approval_no, nhif_item_code);

              	if (valid_approval) {
              		if (marker.src != mFilled.src){
                       marker.src = mFilled.src;
                       element.value = 1;
                       document.getElementById("Approval_"+item_id).value=nhif_approval_no;
                    }

                }else{
                	 alert("Invalid Approval Number: "+nhif_approval_no+" For CardNumber:" +card_no+" Please Enter Correct Approval Number");
              	...
              	if (marker.src != mFilled.src) {
                     marker.src = mFilled.src;
                     element.value = 1;
                      //document.getElementById(item_id).value=10;
                } else {
                     marker.src = mBlank.src;
                     element.value = 0;
...
// var card_no = <?php //echo $card_no ?>;
// function setM(m, editable, restricted, item_code) {
//         eval("marker=document.images." + m);
//         eval("element=document." + form_name + "." + m);
//         if (editable == 'no') {
//             if (marker.src != mFilled.src) {
// 				if(restricted == 1 && isNHIF) {
// 					var nhif_approval_no = prompt('The item with item code ' + item_code + ' is restricted for card number ' + card_no + ', please enter NHIF approval number below');
// 					var valid_approval = verify_nhif_approval(card_no, nhif_approval_no, item_code);
// 					if (valid_approval) {
//                     	console.log('Valid');
// 						marker.src = mFilled.src;
//                 		element.value = 1;
// 					} else {
//                     	console.log('Invalid');
// 						alert('Invalid NHIF approval number');
// 					}
// 				} else {
// 					marker.src = mFilled.src;
//                 	element.value = 1;
...
} elseif (!$read_form && empty($no_proc_assist)) {
	?>
 ...
 */
$insurance_id=$person_obj->insurance($h_pid);
$insuranceNameSql="SELECT name FROM care_tz_company WHERE id='$insurance_id' AND name LIKE '%NHIF%'" ;
$insuranceNameResult=$db->Execute($insuranceNameSql);
if ($insuranceNameResult->RecordCount() && @$insuranceNameResult) {
	$isNHIF=true;
}
...
for ($i = 0; $i <= $max_row; $i++) {
	...
	for ($j = 0; $j <= $column; $j++) {
		...
		if (@$LD_Elements[$j][$i]['type'] && $LD_Elements[$j][$i]['type'] == 'top') {
			...
			//echo "<pre>";print_r($LD_Elements);echo "</pre>";
			if (@$LD_Elements[$j][$i]['value'] && $LD_Elements[$j][$i]['value']) {
				echo '<td>';
				if ($edit) {
					...
					if (isset($stored_param[$LD_Elements[$j][$i]['id']]) && !empty($stored_param[$LD_Elements[$j][$i]['id']])) {
						echo '<input type="hidden" name="' . $LD_Elements[$j][$i]['id'] . '" value="1">
                                                                    <a href="javascript:setM(\'' . $LD_Elements[$j][$i]['id'] . '\', \'' . $editable . '\', \'' . $LD_Elements[$j][$i]['item_id'] . '\', \'' . $isNHIF . '\',\'' . $LD_Elements[$j][$i]['restricted'] . '\',\'' . $LD_Elements[$j][$i]['item_code'] . '\')">';
					} else {
						echo '<input type="hidden" name="' . $LD_Elements[$j][$i]['id'] . '" value=""><input type="hidden" name="' .'Approval_'. $LD_Elements[$j][$i]['item_id'] . '" id="' .'Approval_'. $LD_Elements[$j][$i]['item_id'] . '" value="">
                                                                    <a href="javascript:setM(\'' . $LD_Elements[$j][$i]['id'] . '\', \'' . $editable . '\', \'' . $LD_Elements[$j][$i]['item_id'] . '\', \'' . $isNHIF . '\',\'' . $LD_Elements[$j][$i]['restricted'] . '\',\'' . $LD_Elements[$j][$i]['item_code'] . '\')">';
					}
				}
				if (isset($stored_param[$LD_Elements[$j][$i]['id']]) && !empty($stored_param[$LD_Elements[$j][$i]['id']])) {
                            ...
                            <table border=0 cellpadding=0 cellspacing=0 width=745 bgcolor="<?php echo $bgc1 ?>">
                                <tr>
                                    ...
                                    <td colspan=6>
                                        ...
                                        <textarea rows="2" cols="85" name="notes" form="form_test_request"><?php if (@$stored_request['notes']) {
