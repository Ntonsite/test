...
<!--
    ...
    function getinfo(patientID) {
        urlholder = "nursing-station.php?route=validroute&patient=" + patientID + "&user=<?php echo $aufnahme_user . '"' ?>;patientwin = window.open(urlholder, patientID, "menubar=no,resizable=yes,scrollbars=yes");  patientwin.moveTo(0, 0);patientwin.resizeTo(screen.availWidth, screen.availHeight);"
    }
                    ...
                    <table style="border:1px solid #A7A7A7;" cellpadding="1" cellspacing="0">
                        ...
                        <tr>
                            <td  colspan="3" align="center" valign="top" bgcolor="#696969">
                                <div id="dpDiv" style="width:99%; overflow:hidden; height:<?php echo $height ?>; background-color:#A7A7A7; padding:2px;">
...
if ($dept_nr == '43') {
	...
	$diagnosisUrl = $root_path . 'modules/diagnostics_tz/icd10_search.php?sid=' . $sid . '&encounter=' . $pn . '&reference=quicklist&quicklist=160';
	echo '<input style="width:220px; overflow:hidden; border:1px solid white; margin:0px 0px 7px 6px;" type="button" onClick="chooseDiagnosisType(\'' . $diagUrl . '\')" value="' . $LDDentaldiagnoses . '">';
} else {

	$diagUrl = $root_path . 'modules/diagnostics_tz/icd10_search.php?sid=' . $sid . '&encounter=' . $pn . '&lang=en&ntid=false&externalcall=true&target=search&1=1&ispopup=true&backpath_diag=' . urlencode($_SERVER["PHP_SELF"] . URL_APPEND . '&pn=' . $pn);

	echo '<input style="width:220px; overflow:hidden; border:1px solid white; margin:0px 0px 7px 6px;" type="button" onClick="chooseDiagnosisType(\' ' . $diagUrl . '\')" value="' . $LDDiagnoses . '">';
}
...
if (@$showMealsTab) {
	echo '<br><input style="width:220px; overflow:hidden; border:1px solid white; margin:0px 0px 7px 6px;" type="button" onClick="window.location.href=\'' . $root_path . 'modules/registration_admission/show_prescription_doc.php' . URL_REDIRECT_APPEND . '$sid=' . $SID . '&pn=' . $pn . '&lang=en&ntid=false&externalcall=true&help_site=patient_charts&target=search&1=1&prescrServ=serv&prescrSubCat=meals&backpath=' . urlencode($_SERVER["PHP_SELF"] . URL_APPEND . '&pn=' . $pn . '&edit=1') . '\'" value="Meals">';
}



//op documentation
$sql_en="SELECT * FROM care_encounter WHERE encounter_nr=".$pn;
$result=$db->Execute($sql_en);
if ($row_en=$result->FetchRow()) {
	switch ($row_en['encounter_class_nr']) {
		case 2:
			$deptWard=$row_en['current_dept_nr'];
			break;
...
if (@$deptWard) {
	echo '<br><input style="width:220px; overflow:hidden; border:1px solid white; margin:0px 0px 7px 6px;" type="button" onClick="window.location.href=\'' . $root_path . 'modules/op_document/op-doku-start.php' . URL_REDIRECT_APPEND . 'lang=en&sid='.$SID.'&ntid=false&mode=select&pn=' . $pn . '&dept_nr='.$deptWard.'&target=entry' . '\'" value="OP Documentation">';
...
//modules/op_document/op-doku-start.php?sid=20f0290e6hqvdclvhjj7naomkq&lang=en&target=entry&pn=438413&dept_nr=88
            ...
            <tr>
                <td colspan="2" style="padding-top:15px;">
                    ...
                    <table width="100%"  border="0" align="center" cellpadding="10" cellspacing="1" id="extratable" style="">
                        ...
                        <tr>
                            <td width="15%" bgcolor="#D2DFD0" style="border-top:3px solid red;" valign="top"><strong>OP Documentation</strong></td>
                            <td width="85%" style="border-top:3px solid red;">
                                <?php include './op_doc.php';?>
                            </td>
                        </tr>
                        <tr>
