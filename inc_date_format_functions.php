...
function formatDate2Local($stdDate, $localFormat, $retTime = FALSE, $timeOnly = FALSE, $sepChars) {
	...
	if (preg_match('/0000/i', $stdDate)) {
		return strtr($localFormat, 'yYmMdDHis', '000000000');
	}
	// IF  std date is 0 return 0's in local format
	...
	for ($i = 0; $i < 3; $i++) {
		if ($localArray[$i] == 'yyyy') {
			$localArray[$i] = $stdArray[0] ?? date("Y");
		} elseif ($localArray[$i] == 'mm') {
			$localArray[$i] = $stdArray[1] ?? date('m');
		} elseif ($localArray[$i] == 'dd') {
			$localArray[$i] = $stdArray[2] ?? date('d');
		}
	...
	if ($retTime) {
		return implode($localSeparator, $localArray) . ' ' . $stdTime;
	} else {
...
#Children
	if (($age >= 0) && ($age < 6)) {
		...
		# Birthday is today
		if ((date("m") == $date[1]) && (date("d") >= $date[2])) {
			$age++;
		}
		$returnvalue = $age;
		if ($age > 1) {
