...
if (IS_TANZANIAN) {
	...
	if (!empty($top_rs) && isset($top_rs)) {
		...
		while ($categories = $top_rs->FetchRow()) {
			...
			$SQLStatementParam = "SELECT id,name,status, nr FROM care_tz_laboratory_param WHERE group_id = '" . $categories['id'] . "' AND status NOT IN ('deleted','hidden') ORDER BY sort_order, nr, sort_nr";
			...
			if (!empty($row_rs) && isset($row_rs)) {
				while ($parameters = $row_rs->FetchRow()) {
					...
					if (@$patientResult && $patientResult->RecordCount()) {
						...
						$schemeId=$patient['nhif_scheme_id'];
					}
					if ($insurance_ID > 0) {
					...
					if ($isNHIFMember) {
						...
						$drugSQL = "SELECT ds.item_id,ds.nhif_is_restricted,ds.nhif_item_code FROM care_tz_drugsandservices ds INNER JOIN care_tz_drugsandservices_nhifschemes dsn ON ds.nhif_item_code=dsn.ItemCode WHERE ds.item_number = '$itemNumber' AND ds.nhif_item_code > 0 AND dsn.SchemeID='$schemeId'";
