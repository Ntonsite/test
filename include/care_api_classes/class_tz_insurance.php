...
class Insurance_tz extends Core {
	...
	function InsertNewInsuranceCompany($dataarray) {
		...
		if ($dataarray['enable_member_expiry']) {
			$expiry = 1;
		} else {
			$expiry = 0;
		}
...
//------------------------------------------------------------------------------
	...
	function CheckContractValidity($company_id, $start_date, $end_date) {
		...
		$this->contract_array = $this->GetContractsForCompanyAsArray($company_id);
		while (list($x, $v) = $helper->customEach($this->contract_array)) {
	...
	function NewContractForm($company_id) {
		...
		if ($this->is_company_just_invoiced($company_id)) {
			...
			$v['Contract']['plan']=isset($v['Contract']['plan']) ? $v['Contract']['plan'] : "";
			...
			$this->ShowInsuranceTypesDropDown('plan', $v['Contract']['plan'],$a);
			echo '</td></tr>
					</table>';
		}
	}
	...
	function ShowInsuranceTypesDropDown($name, $selected, $FLAG) {
		...
		$this->pid = $arr_name_informations[1] ?? 0;
		...
		if (count($contractarray) > 0) {
			if (is_array($array)) {
				while (list($x, $v) = $helper->CustomEach($array)) {
					...
					if (!is_array($v['Contract']) and $v['Contract'] != -2) {
						//New Contract
						echo '<tr bgcolor="ffffaa"><td>' . $person['name_last'] . ', ' . $person['name_first'] . ' (Hospital file nr: ' . $person['selian_pid'] . ')</td><td>' . $LDAction . '</td><td>' . $LDPlanSubstraction . '</td></tr>
							...
							<td align="center">';
						if (!$this->is_company_just_invoiced($company_id)) {
							$this->ShowInsuranceTypesDropDown('plan_' . $v['PID'], $contractarray['plan'],$a);
						}
						...
						if ($v['Contract']['company_id'] != $selected_insurance) {
							// Is there another valid contract for this person?
...
// No its not, pid is member of this company
		...
		while (list($x, $v) = $helper->CustomEach($allcontracts)) {
			$result = $person_obj->getAllInfoObject($v['PID']);
			if ($result) {
				$person = $result->FetchRow();
			}
			// Just if there are results, there should be a FetchRow()...
...
";
		...
		$this->result_string .= "<option value=\"-2\" selected>-Select Credit Company or Cash-</option>";
		$this->result_string .= "<option value=\"0\">-(Cash-Patient)-</option>";
	...
	function allocatePrescriptionsToinsurance($bill_number, $prescriptions_nr, $insurance_payment, $insurance_id) {
		...
		($debug) ? $db->debug = TRUE : $db->debug = FALSE;
		$this->sql = "SELECT ID, amount, price FROM care_tz_billing_archive_elem where nr=$bill_number AND prescriptions_nr=$prescriptions_nr AND is_medicine=1";
		if ($this->result = $db->Execute($this->sql)) {
			while ($this->row = $this->result->FetchRow()) {
				$covered_price = $this->row['amount'] * $this->row['price'];
				$exact_id_of_bill_element = $this->row['ID'];
				$this->sql = "update care_tz_billing_archive_elem SET balanced_insurance='$covered_price', insurance_id=$insurance_id WHERE ID=$exact_id_of_bill_element";
	...
	function allocateLaboratoryItemsToinsurance($bill_number, $labtest_nr, $insurance_payment, $insurance_id) {
		...
		$debug = FALSE;
		($debug) ? $db->debug = TRUE : $db->debug = FALSE;
		$this->sql = "SELECT ID, amount, price FROM care_tz_billing_archive_elem where nr=$bill_number AND is_labtest=1";
		if ($this->result = $db->Execute($this->sql)) {
			while ($this->row = $this->result->FetchRow()) {
				$covered_price = $this->row['amount'] * $this->row['price'];
				$exact_id_of_bill_element = $this->row['ID'];
	...
	function allocateRadiologyItemsToinsurance($bill_number, $radtest_nr, $insurance_payment, $insurance_id) {
		...
		$this->sql = "SELECT ID, amount, price FROM care_tz_billing_archive_elem where nr=$bill_number AND is_radio_test=1";
		if ($this->result = $db->Execute($this->sql)) {
			while ($this->row = $this->result->FetchRow()) {
				$covered_price = $this->row['amount'] * $this->row['price'];
				$exact_id_of_bill_element = $this->row['ID'];
	...
	function GetNumberOfMembers($company_id, $year = 0) {
		...
		*/
		...
		// if no year is given, take the current year
