...
class OPRoom extends Core {
	...
	function ORInfo($nr = 0, $rec_key = FALSE) {
		...
		if ($rec_key) {
			$this->sql .= " WHERE o.nr=$nr AND o.type_nr=$this->OR_typenr";
		} else {
			$this->sql .= " WHERE o.room_nr=$nr AND o.type_nr=$this->OR_typenr";
		}
