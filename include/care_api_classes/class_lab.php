...
($this->dead_stat)";
	 ...
	 * @return mixed adodb record object or boolean
	 */
	function getLabResult($job_id, $grp_id = "", $enc_nr = '') {
		global $db;
		...
		$this->sql = "SELECT * FROM $this->tb_find_chemlab INNER JOIN $this->tb_find_chemlab_sub ";
		$this->sql .= "ON ($this->tb_find_chemlab.job_id = $this->tb_find_chemlab_sub.job_id) ";
		$this->sql .= "WHERE $this->tb_find_chemlab.encounter_nr='$this->enc_nr' AND $this->tb_find_chemlab.job_id='$job_id' AND $this->tb_find_chemlab.group_id='$grp_id' AND $this->tb_find_chemlab.status != 'hidden' ORDER BY $this->tb_find_chemlab_sub.sort_order";
		if ($this->result = $db->Execute($this->sql)) {
			if ($this->rec_count = $this->result->RecordCount()) {
				return $this->result;
			} else {
	...
	function getFileResults($enc_nr = '') {
		global $db;
		if (!$this->internResolveEncounterNr($enc_nr)) {
			return FALSE;
		}

		$this->sql = "SELECT file_path, test_date, test_time FROM $this->tb_find_chemlab";
		$this->sql .= " WHERE $this->tb_find_chemlab.encounter_nr='$this->enc_nr' AND file_path != '' AND $this->tb_find_chemlab.status NOT IN ($this->dead_stat) ORDER BY batch_nr DESC";
		if ($this->fileResult = $db->Execute($this->sql)) {
			if ($this->rec_count = $this->fileResult->RecordCount()) {
	...
	function GetTestsToDo($id = '') {
		...
		$sub = "_sub";
		$sql = "SELECT * FROM $this->tb_req_chemlab" . $sub . "   WHERE $cond AND $this->tb_req_chemlab" . $sub . ".deleted = 0 ORDER BY sort_order";
		// print_r($sql);die;
		if ($this->tparams = $db->Execute($sql)) {
			if ($this->rec_count = $this->tparams->RecordCount()) {
				return $this->tparams;
			} else {
				return FALSE;
...
($this->dead_stat) ORDER BY modify_time DESC";
	...
	function getLabBillNoByBatch($batch_nr, $param_id) {
		...
		if (!empty($batch_nr) && !empty($param_id)) {
			$this->sql = "SELECT bill_number FROM $this->tb_req_chemlab_sub INNER JOIN care_tz_billing_archive_elem ON $this->tb_req_chemlab_sub.bill_number=care_tz_billing_archive_elem.nr WHERE batch_nr='" . $batch_nr . "' AND paramater_name='" . $param_id . "' AND deleted = 0";
			if ($debug) {
				echo $this->sql . "<br>";
			}
