...
class Appointment extends Core {
	...
	/**
	 * Counts all appointment on a given date
	 * by department or doctor
	 */
	function appointment_count($date = '', $by = '', $val = '') {
//        print $date;
		global $db, $sql_LIKE;
		...
		if ($this->res['_ga'] = $db->Execute($this->sql)) {
			if ($this->count = $this->res['_ga']->RecordCount()) {
				//Get the limit number of appointments and compare
				//initialize the variable $app_no and $row
				$app_no = '';
				$row = array();
				switch ($by) {
				case '_DEPT':$this->sql = "SELECT max_appointments FROM care_department
                                                WHERE nr = $val";
					break;
				case '_DOC':
					break;
				}
				if ($this->res['nr'] = $db->Execute($this->sql)) {
					$row = $this->res['nr']->FetchRow();
				} else {
					echo $this->sql;
					return false;
				}
//                Compare the number of appointments
				if ($row['max_appointments'] != 0 && $this->count >= $row['max_appointments']) {
					return TRUE;
				} else {
					return FALSE;
				}
