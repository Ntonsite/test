...
class Person extends Core {
	...
	*/
	...
	function GetNewSelianFileNumber() {
		...
		//        $this->sql = "SELECT max(selian_pid) as maximum FROM $this->tb_person";
		//A case for Haydom to avoif big hospital file numbers
         
        $this->sql="SELECT MAX(pid) FROM care_person WHERE create_time>'2009-12-31 11:40:12' ";
        if($this->result = $db->Execute($this->sql)){
		...
		$this->sql = "SELECT selian_pid as maximum FROM $this->tb_person WHERE pid=".$MaxPid;
		if ($this->result = $db->Execute($this->sql)) {
			if ($this->row = $this->result->FetchRow()) {
...
#        global $_POST;
		...
		while (list($x, $v) = $helper->CustomEach($this->elems_array)) {
	...
	function insertDataFromArray($array) {
		...
		} elseif (@$ins_cat && $ins_cat == "gold") {
			$ins_index = ", insurance_gold";
		} elseif (@$ins_cat && $ins_cat == "friedkin") {
			$ins_index = ", insurance_friedkin";
		} elseif (@$ins_cat && $ins_cat == "selian") {
	...
	*/
	 ...
	 */
	function getPersonValue($item, $pid = "") {
		global $db;
...
//            return $this->sql;
	...
	*/
	 ...
	 */
	function FirstName() {
		return $this->getPersonValue('name_first');
	}
	 ...
	 */
	function LastName() {
		return $this->getPersonValue('name_last');
	}
	 ...
	 */
	function SecondName() {
		return $this->getPersonValue('name_2');
	}
	 ...
	 */
	function ThirdName() {
		return $this->getPersonValue('name_3');
	}
	 ...
	 */
	function MiddleName() {
		return $this->getPersonValue('name_middle');
	}
	 ...
	 */
	function MaidenName() {
		return $this->getPersonValue('name_maiden');
	}
	 ...
	 */
	function OtherName() {
		return $this->getPersonValue('name_others');
	}
	 ...
	 */
	function BirthDate() {
		return $this->getPersonValue('date_birth');
	}
	 ...
	 */
	function StreetNr() {
		return $this->getPersonValue('addr_str_nr');
	}
	 ...
	 */
	function StreetName() {
		return $this->getPersonValue('addr_str');
	}
	 ...
	 */
	function ZIPCode() {
		return $this->getPersonValue('addr_zip');
	}
	...
	function isValidAddress() {
		return $this->getPersonValue('addr_is_valid');
	}
	...
	function CityTownCode() {
		return $this->getPersonValue('addr_citytown_nr');
	}
	...
	function Citizenship() {
		return $this->getPersonValue('citizenship');
	}
	...
	function FirstPhoneAreaCode() {
		return $this->getPersonValue('phone_1_code');
	}
	...
	function FirstPhoneNumber() {
		return $this->getPersonValue('phone_1_nr');
	}
	...
	function SecondPhoneAreaCode() {
		return $this->getPersonValue('phone_2_code');
	}
	...
	function SecondPhoneNumber() {
		return $this->getPersonValue('phone_2_nr');
	}
	...
	function FirstCellphoneNumber() {
		return $this->getPersonValue('cellphone_1_nr');
	}
	...
	function SecondCellphoneNumber() {
		return $this->getPersonValue('cellphone_2_nr');
	}
	 ...
	 */
	function FaxNumber() {
		return $this->getPersonValue('fax');
	}
	...
	function EmailAddress() {
		return $this->getPersonValue('email');
	}
	 ...
	 */
	function Sex() {
		return $this->getPersonValue('sex');
	}
	 ...
	 */
	function Title() {
		return $this->getPersonValue('title');
	}
	...
	function PhotoFilename() {
		return $this->getPersonValue('photo_filename');
	}
	...
	function EthnicOrigin() {
		return $this->getPersonValue('ethnic_origin');
	}
	 ...
	 */
	function OrgID() {
		return $this->getPersonValue('org_id');
	}
	 ...
	 */
	function SSSNumber() {
		return $this->getPersonValue('sss_nr');
	}
	...
	function NationalIDNumber() {
		return $this->getPersonValue('nat_id_nr');
	}
	 ...
	 */
	function Religion() {
		return $this->getPersonValue('religion');
	}
	...
	function Insurance($pid) {
		return $this->getPersonValue('insurance_ID', $pid);
	}
	 ...
	 */
	function MotherPID() {
		return $this->getPersonValue('mother_pid');
	}
	 ...
	 */
	function FatherPID() {
		return $this->getPersonValue('father_pid');
	}
	 ...
	 */
	function DeathDate() {
		return $this->getPersonValue('death_date');
	}
	 ...
	 */
	function DeathCause() {
		return $this->getPersonValue('death_cause');
	}
	...
	function RecordStatus() {
		return $this->getPersonValue('status');
	}
	...
	function RecordHistory() {
		return $this->getPersonValue('history');
	}
	...
	function DeathEncounterNumber() {
		return $this->getPersonValue('death_encounter_nr');
	}
	...
	function CurrentEncounter($pid) {
		...
		$this->sql = "SELECT encounter_nr FROM $this->tb_enc WHERE pid='$pid' AND (is_discharged='' OR is_discharged=0) AND encounter_status <> 'cancelled'";
  //below part is removed because it return true even when status is null
...
# default, latest pid at top

			$sql2 = "	WHERE pid=$suchwort OR selian_pid='$suchwort' OR membership_nr='$suchwort'";
		} else {
				...
				if ($suchwort == '%' || $suchwort == '%%') {
					...
					if ($DOB == '') {
						if (defined('SHOW_FIRSTNAME_CONTROLLER') && SHOW_FIRSTNAME_CONTROLLER) {
							if ($fname) {
								$sql2 = " WHERE name_2 $sql_LIKE '" . strtr($suchwort, '+', ' ') . "%' OR name_last $sql_LIKE '" . strtr($suchwort, '+', ' ') . "%' OR name_first $sql_LIKE '" . strtr($suchwort, '+', ' ') . "%' OR membership_nr $sql_LIKE '" . strtr($suchwort, '+', ' ') . "%'";
							} else {
