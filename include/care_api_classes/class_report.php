...
class report extends core {
	...
	function _ColumnNames() {
		if ($this->debug) {
			echo "class_report::_ColumnNames(" . func_num_args() . " parameters)<br>";
		}

		if (func_num_args() > 2) {

			if ($this->debug) {
				echo "---> 4 parameters...<br>";
			}
	...
	function SetReportingTable($tbl = '') {
		...
		if ($this->debug) {
			echo "class_report::SetReportingTable($tbl)<br>";
		}
	...
	function SetReportingLink($tbl1, $tbl1_key, $tbl2, $tbl2_key) {
		...
		$this->debug = FALSE;
		if ($this->debug) {
			echo "class_report::SetReportingLink($tbl1,$tbl1_key, $tbl2,$tbl2_key)<br>";
		}
	...
	function RowCount($tbl) {
		global $db;
		if ($this->debug) {
			echo "class_report::RowCount($tbl)<br>";
		}
	...
	function GroupBy($table, $field) {
		global $db;
		if ($this->debug) {
			echo "class_report::GroupBy($table,$field)<br>";
		}
