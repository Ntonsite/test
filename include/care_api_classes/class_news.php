...
class News extends Core {
	...
	function getTitle($nr = 0) {
		...
		if ($this->buffer = $this->getItem($nr, 'title')) {
			return $this->buffer;
		}
	...
	function getPreface($nr = 0) {
		...
		if ($this->buffer = $this->getItem($nr, 'preface')) {
			return $this->buffer;
		}
	...
	function getBody($nr) {
		...
		if ($this->buffer = $this->getItem($nr, 'body')) {
			return $this->buffer;
		}
	...
	function getAuthor($nr) {
		...
		if ($this->buffer = $this->getItem($nr, 'author')) {
			return $this->buffer;
		}
	...
	function getPublishDate($nr) {
		...
		if ($this->buffer = $this->getItem($nr, 'publish_date')) {
			return $this->buffer;
		}
	...
	function getSubmitDate($nr) {
		...
		if ($this->buffer = $this->getItem($nr, 'submit_date')) {
			return $this->buffer;
		}
	...
	function getPicMime($nr) {
		...
		if ($this->buffer = $this->getItem($nr, 'pic_mime')) {
			return $this->buffer;
		}
	...
	function getArchiveList($dept_nr = 0, &$heads) {
		...
		if ($this->res['gal'] = $db->Execute($this->sql)) {
			if ($this->rec_count = $this->res['gal']->RecordCount()) {
				return $this->res['gal'];
			} else {
				return false;
			}
	...
	function saveNews($dept_nr = 0, &$news) {
		global $db, $lang, $dbtype; // $_SESSION;
		//$db->debug=FALSE;
		# Seed random gen
		srand();
		$imgfname = rand();
