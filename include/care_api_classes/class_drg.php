...
class DRG extends Encounter {
	...
	function searchGroup($key, $order = 'description') {
		global $db, $sql_LIKE;
		if (strlen($key) < 3) {
			$this->sql = "SELECT nr,code,description FROM $this->tb_drg WHERE code $sql_LIKE '$key%' OR description $sql_LIKE '$key%'";
		} else {
			$this->sql = "SELECT nr,code,description FROM $this->tb_drg WHERE code $sql_LIKE '%$key%' OR description $sql_LIKE '%$key%'";
		}
...
//	    global  $_SESSION;
		if (!$this->internResolveEncounterNr($enc_nr) || !$grp_nr) {
			return FALSE;
		}
