...
class Encounter extends Notes {
	...
	/**
	 * Table name for encounter prescriptions
	 * @var string
	 */
	var $tb_enc_prescr = 'care_encounter_prescription';
	...
	/**
	 * Table name for pharmacy locations
	 * @var string
	 */
	var $tb_locations = 'locations';
	...
	function getNewEncounterNr($ref_nr, $enc_class_nr) {
		...
		if ($ref_nr > 0) {
			...
			$this->sql = "SELECT MAX(encounter_nr) as encounter_nr FROM $this->tb_enc";
		}
...
/* echo $this->sql.'no sql'; */
	...
	*/
	...
	function GetPharmacy($enr = 0) {
		global $db;
		$this->sql = "SELECT pharmacy FROM $this->tb_enc WHERE encounter_nr=" . $enr;
		$this->result = $db->Execute($this->sql);
		if ($this->dispensing = $this->result->Fetchrow()) {
			return $this->dispensing['pharmacy'];
		}

	}

	function GetPharmacyName($enr = 0) {
		global $db;
		$this->sql = "SELECT locationname FROM locations WHERE loccode='" . $this->GetPharmacy($enr) . "'";
		$this->result = $db->Execute($this->sql);
		if ($this->dispensing = $this->result->Fetchrow()) {
			return $this->dispensing['locationname'];
		}
	...
	function _searchAdmissionBasicInfo($key, $enc_class = 0, $add_opt = '', $limit = FALSE, $len = 30, $so = 0) {
		...
		if ($enc_class == 2) {
			$this->sql = "SELECT DISTINCT p.selian_pid, rc.batch_nr,rc.send_date, e.encounter_nr, e.encounter_class_nr, p.pid, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex,p.blood_group
		...
		if (is_numeric($key)) {
			...
			if ($db->debug) {
				echo " -> we have to search in a text comparison<br>";
			}
//            $this->sql.=" WHERE (e.encounter_nr $sql_LIKE '$key%'
	 ...
	 *    echo $obj->rec_count);  # Prints the number of resulting rows
	 *    while($admission=$result->FetchRow()){
	 *        echo $admission['name_last'];  # Prints the patient's name
	 *      ......
	 *    }
	...
	function _InLocation($type_nr) {
		global $db;
		$sql = "SELECT nr FROM $this->tb_location WHERE encounter_nr=$this->enc_nr AND type_nr=$type_nr AND location_nr=$this->loc_nr AND (date_to='' OR date_to='0000-00-00' OR date_to='1980-01-01')";
		// echo $sql;
	...
	function assignInWard($enr, $loc_nr, $group_nr, $date, $time) {
		if ($this->_setLocation($enr, 2, $loc_nr, $group_nr, $date, $time)) {
			# loc. type 2 = ward
	...
	function assignInRoom($enr, $loc_nr, $group_nr, $date, $time) {
		...
		$presc_obj->insert_prescription($enr, $row[0], 'service',$a);
	...
	function assignInDept($enr, $loc_nr, $group_nr = "", $date = "", $time = "") {
		if ($this->_setLocation($enr, 1, $loc_nr, $group_nr, $date, $time)) {
			# loc. type 1 = department
			return $this->setCurrentDeptInDept($enr, $loc_nr);
		}
	}

	function InsertNHIFFields($encounter_nr) {
		global $db;
		$pid = 0;
		$isNHIFMember = 0;

		$sql = "SELECT pid from care_encounter WHERE encounter_nr = '$encounter_nr'";
		$patientResult = $db->Execute($sql);
		...
		if ($isNHIFMember) {
			...
			$snSQL = "SELECT max(nhif_serial_number) as nhif_serial_number  FROM care_encounter WHERE MONTH(nhif_serial_date)= '$thisMonth' AND YEAR(nhif_serial_date) = '$thisYear'";
			$snResult = $db->Execute($snSQL);

			if (@$snResult && $snResult->RecordCount()) {
				$nhifSerial = $snResult->FetchRow();
				$serialNumber = $nhifSerial['nhif_serial_number'];
			}

			$nextSerialNumber = $serialNumber + 1;

			$updateSerialSQL = "UPDATE care_encounter SET nhif_serial_number = '$nextSerialNumber', nhif_serial_date = '$today' WHERE encounter_nr = '$encounter_nr' ";
			$updatedResult = $db->Execute($updateSerialSQL);
			...
			if (@$patNHIFResult && $patNHIFResult->RecordCount() > 0) {
				...
				if (@$patJson) {
					...
					$nhif_full_name = $pData->FullName;
					$nhif_card_status = $pData->CardStatus;
					$nhif_authorization_status = $pData->AuthorizationStatus;
					$nhif_authorization_number = $pData->AuthorizationNo;
					$nhif_latest_authorization = $pData->LatestAuthorization;
...
//		global $_SESSION;
	...
	function saveDischargeNotesFromArray(&$data_array) {
		...
		if ($this->_insertNotesFromInternalArray(3)) {
			// 3 = discharge summary note type
			return true;
		} else {
			return FALSE;
		}
	...
	function getBasic4Data($enc_nr) {
		...
		$this->sql = "SELECT UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex,p.insurance_ID
...
//	    global $_SESSION;
	...
	function getEncouterBillNo($encounter_nr) {
		global $db;
		$debug = false;
		...
		if (!empty($encounter_nr)) {
			$this->sql = "SELECT $this->tb_enc_prescr.bill_number FROM $this->tb_enc_prescr INNER JOIN care_tz_drugsandservices INNER JOIN care_tz_billing_archive_elem ON $this->tb_enc_prescr.bill_number=care_tz_billing_archive_elem.nr  WHERE "
				. " encounter_nr='" . $encounter_nr . "' AND (article like '%cons%' OR care_tz_drugsandservices.item_number like '%cons%'  ) AND is_disabled=''";
			if ($debug) {
				echo $this->sql . "<br>";
			}
...
// end of function
	...
	/*
		     * A function to print encounter consultation notes
	*/
	function PrintPatientNotes($encounter_no) {

		$result = $this->getEncounterNotes($encounter_no);
		if (empty($result)) {
			echo '<td colspan="3" align="center"> No notes </td>';
		} else {
			$row = $result->FetchRow();
			echo '<td valign="top">' .
			'<b>' . date('d/M/Y', strtotime($row['date'])) . '</b></td>';
