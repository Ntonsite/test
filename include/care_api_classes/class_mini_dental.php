...
class dental extends Core {
	...
	*/
	 ...
	 *
	 */
	function PrintPatientNotes($pid, $enno, $note = '', $color1 = '', $color2 = '', $recs = '') {
		global $db;
//        $db->debug=true;
		$nipo = FALSE;
		$notename = '';
		...
		$this->sql = "SELECT date, notes, create_id, short_notes, encounter_nr, type_nr, nr,create_time FROM care_encounter_notes WHERE (encounter_nr IN (SELECT encounter_nr FROM care_encounter WHERE pid = '$pid')) " . $MORE . " ORDER BY create_time DESC";
		...
		while ($this->row = $this->result->FetchRow()) {
			...
			if ($dateDiff>2880) {
				...
				if ($_SESSION['sess_user_name'] == $this->row['create_id']) {
			...
			echo '<td valign="top">' . $this->row[7] . '</td>';
