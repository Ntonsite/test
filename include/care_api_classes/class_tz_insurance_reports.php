	...
	*/
	...
	function Display_ReportTable_Head($BGCOL = '#99ccff') {
		...
		*/
		...
		if ($this->debug) {
			echo "Display_ReportTable_Head($BGCOL='#99ccff') : " . func_num_args();
		}
		...
		if ($this->debug) {
			echo "Output error, maybe wrong parameters? <br>";
		}
...
//echo ($this->is_poor_people_company($company_id)) ? "ist poor people" : "nee, is keine";
				...
				// Debug output:
				if ($this->debug) {
					echo ($number_of_members > 0) ? "This company has members for the year $year" : "This company has no members for the year $year";
				}
				...
				if ($number_of_members > 0) {
					...
					if ($this->is_poor_people_company($company_id)) {
						...
						while (list($u, $v) = $helper->CustomEach($memberlist)) {
							$used_amount = $v[4];
							echo '<tr>';
