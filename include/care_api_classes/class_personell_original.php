...
class Personell extends Core {
	...
	*/
	...
	function getDoctorsOfDept($dept_nr = 0) {
		...
		// 1= dept (location), 17 = doctor (role)
	}
	/**
	 * Returns information of all nurses of a department.
	 *
	...
	function _getAllPersonell($loc_type_nr, $role_nr = 0, $dept_nr) {
		...
		$sql = "SELECT a.nr, a.personell_nr, ps.job_function_title, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex
	...
	function _getAllPersonellAllDepts($loc_type_nr, $role_nr = 0) {
		...
		$sql = "SELECT a.nr, a.personell_nr, ps.job_function_title, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex
				...
				WHERE a.role_nr=$role_nr
					...
					AND a.personell_nr=ps.nr
					AND ps.pid=p.pid
				ORDER BY UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first";
	...
	*/
	...
	function searchPersonellBasicInfo($key, $oitem = 'name_last', $odir = 'ASC', $limit = FALSE, $len = 30, $so = 0) {
		...
		$this->sql = "SELECT ps.nr, j.name, p.pid, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex
		...
		if (!empty($oitem)) {
			if ($oitem == 'nr' || $oitem == 'job_function_title') {
				$this->sql .= " ORDER BY ps.$oitem $odir";
			} else {
				$this->sql .= " ORDER BY p.$oitem $odir";
			}
