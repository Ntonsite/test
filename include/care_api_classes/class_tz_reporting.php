...
class selianreport extends report
{
	...
	function Display_OPD_Diagnostic($start, $end)
	{
		...
		//count all
		$sql_count = "SELECT diag.ICD_10_code, DATE_FORMAT( cp.date_birth, '%Y' ) AS TAREHE,  diag.ICD_10_description, count(*) AS TOTAL FROM care_tz_diagnosis AS diag INNER JOIN care_person AS cp   ON diag.PID=cp.pid WHERE $sql_timeframe $dx_type GROUP BY  diag.`ICD_10_code`";
		...
		for ($i = 0; $i <= $count; $i++) {
			...
			//under age count
			$sql_under_age = "SELECT diag.ICD_10_code,  diag.ICD_10_description, count(*) AS TOTAL_UNDER_AGE FROM care_tz_diagnosis AS diag INNER JOIN care_person AS cp   ON diag.PID=cp.pid WHERE (DATE_FORMAT( NOW( ) , '%Y' ) - DATE_FORMAT( cp.date_birth, '%Y' ) ) <5 $dx_type AND $sql_timeframe AND diag.ICD_10_code='$diagnostic_code'";
			$under_age_result = $db->Execute($sql_under_age);


			//over age count
			$sql_over_age = "SELECT diag.ICD_10_code,  diag.ICD_10_description, count(*) AS TOTAL_OVER_AGE FROM care_tz_diagnosis AS diag INNER JOIN care_person AS cp   ON diag.PID=cp.pid WHERE (DATE_FORMAT( NOW( ) , '%Y' ) - DATE_FORMAT( cp.date_birth, '%Y' ) ) >=5 AND $sql_timeframe $dx_type AND diag.ICD_10_code='" . $diagnostic_code . "'";
			$over_age_result = $db->Execute($sql_over_age);

			//Male count
			$sql_male = "SELECT diag.ICD_10_code,  diag.ICD_10_description, count(*) AS TOTAL_MALE FROM care_tz_diagnosis AS diag INNER JOIN care_person AS cp   ON diag.PID=cp.pid WHERE cp.sex='m' AND $sql_timeframe $dx_type  AND diag.`ICD_10_code`='" . $diagnostic_code . "'";
			$male_result = $db->Execute($sql_male);

			//Female count
			$sql_female = "SELECT diag.ICD_10_code,  diag.ICD_10_description, count(*) AS TOTAL_FEMALE FROM care_tz_diagnosis AS diag INNER JOIN care_person AS cp   ON diag.PID=cp.pid WHERE cp.sex='f' AND $sql_timeframe  $dx_type AND diag.`ICD_10_code`='" . $diagnostic_code . "'";
...
?>
	...
	function Display_OPD_Diagnostic_month($start, $end, $admission_nr, $insurance, $current_ward_nr, $dept_nr)
	{
		...
		if (isset($_POST['dxCode']) && !empty($_POST['dxCode']) && isset($_POST['dx']) && !empty($_POST['dx'])) {
			$dx = $_POST['dxCode'];
			$dxs = 'AND icd_10_code LIKE ' . '"%' . $dx . '%"';
		} elseif (!empty($_POST['dxCode'])) {
			$dx = $_POST['dxCode'];
			$dxs = 'AND icd_10_code LIKE ' . '"%' . $dx . '%"';
		} elseif (!empty($_POST['dx'])) {
			$dx = $_POST['dx'];
			$dxs = 'AND icd_10_description LIKE ' . '"%' . $dx . '%"';
		} else {
			$dxs = "";
			echo $dxs;
		}
		...
		//echo '</tr>';
			...
			for ($i = 0; $i <= $count; $i++) {
				...
				$sql_yr_to_under_5yrs_male = "SELECT diag.ICD_10_code,  diag.ICD_10_description, count(*) AS TOTAL_YR_TO_UNDER_5YRS_MALE FROM care_tz_diagnosis AS diag INNER JOIN care_person AS cp   ON diag.PID=cp.pid INNER JOIN care_encounter ce ON ce.encounter_nr=diag.encounter_nr WHERE timestampdiff(YEAR,DATE_FORMAT(cp.date_birth,'%Y-%m-%d'),DATE_FORMAT(NOW(),'%Y-%m-%d')) >=1 AND timestampdiff(YEAR,DATE_FORMAT(cp.date_birth,'%Y-%m-%d'),DATE_FORMAT(NOW(),'%Y-%m-%d')) <5  AND $sql_timeframe $dx_type $hf $idara $dxs AND cp.sex='m' AND diag.ICD_10_code='" . $diagnostic_code . "' ORDER BY series ASC";
				...
				$sql_5yrs_to_under_60yrs_male = "SELECT diag.ICD_10_code,  diag.ICD_10_description, count(*) AS TOTAL_5YRS_TO_UNDER_60YRS_MALE FROM care_tz_diagnosis AS diag INNER JOIN care_person AS cp   ON diag.PID=cp.pid INNER JOIN care_encounter ce ON ce.encounter_nr=diag.encounter_nr WHERE timestampdiff(YEAR,DATE_FORMAT(cp.date_birth,'%Y-%m-%d'),DATE_FORMAT(NOW(),'%Y-%m-%d')) >=5 AND timestampdiff(YEAR,DATE_FORMAT(cp.date_birth,'%Y-%m-%d'),DATE_FORMAT(NOW(),'%Y-%m-%d')) <60  AND $sql_timeframe $dx_type $hf $idara $dxs AND cp.sex='m' AND diag.ICD_10_code='" . $diagnostic_code . "' ORDER BY series ASC";
		...
		function DisplayOPDMtuhaRows($start, $end, $admission_nr, $insurance, $current_ward_nr, $dept_nr)
		{
			if (empty($start)) {
		...
		*/
		...
		function _get_amount_of($start_timeframe, $day, $filter, $total_summary)
		{
			...
			if ($return_value) {
				...
				// no return value given for this query...
				return "0";
...
AND unix_timestamp(care_encounter.encounter_date)>='" . $start_timeframe . "' AND unix_timestamp(care_encounter.encounter_date)<='" . $end_timeframe . "'
		...
		if ($admission == 2) {
			...
			$db_ptr = $db->Execute($sql_d);
			$sql_s = "CREATE TEMPORARY TABLE tmp_laboratories  (SELECT care_test_request_chemlabor.batch_nr,UNIX_TIMESTAMP(care_test_request_chemlabor.create_time) as send_date,care_test_request_chemlabor.status,
paramater_name as id
		...
		if ($admission == 2) {
			...
			$sql_d = "DROP TABLE IF EXISTS `tmp_laboratories`";
			$db_ptr = $db->Execute($sql_d);
			$sql_s = "CREATE TEMPORARY TABLE tmp_laboratories  (SELECT care_test_request_chemlabor.batch_nr, UNIX_TIMESTAMP( care_test_request_chemlabor.create_time ) AS send_date,
	  ...
	  $tbl2.pid,$tbl2.selian_pid,$tbl2.date_reg,$tbl2.name_first,$tbl2.name_2,$tbl2.name_last,$tbl2.date_birth,$tbl2.sex,$tbl2.is_first_reg FROM $tbl1 INNER JOIN $tbl2 ON $tbl1.$tbl1_key=$tbl2.$tbl2_key AND UNIX_TIMESTAMP(encounter_date)>='" . $start . "' AND UNIX_TIMESTAMP(encounter_date)<='" . $end . "' " . $sql_adm . ")";
		...
		function DisplayMonthlyLabTestsSummary($start_timeframe, $end_timeframe, $admission, $health_fund)
		{
			...
			$sql = "SELECT nr, name, id FROM care_tz_laboratory_param WHERE group_id!='-1' AND status NOT IN('hidden','deleted') ORDER BY name";
			...
			?>
				...
				?>
					<tr>
						...
						<td><?php echo $rows_items['partcode']; ?></td>
				...
				<tr bgcolor="lightblue">
					<td colspan="13" bgcolor="lightblue"></td>
				</tr>
				<tr bgcolor="lightblue">
					<td colspan="8"><B>
							<font size="3">SUB TOTAL <?php echo number_format($total_items) . ';'; ?>
						</B>&nbsp;<i>Receipt Number <?php echo $rows_particlars['nr'] . ';  '; ?>&nbsp;<i>BankRef: <?php echo $rows_particlars['bank_ref'] . ';  '; ?>CASHIER <?php echo $rows_particlars['User_Id'] . ';  '; ?> </i></font>
					</td>
				</tr>
				<tr bgcolor="red">
					<td colspan="13" bgcolor="red"></td>
				</tr>
          ...
          WHERE description='Advance' AND billelem.date_change BETWEEN '" . $date_from_timestamp . "' AND '" . $date_to_timestamp . "' " . $in_out_patient . " " . $and_PayType . " " . $fileNr . " ";
			...
			?>
				<tr>
					...
					<!--<td width="70" scope="col" font-size="8"> <?php //echo  $rows_dep['price'];                                    
																	?></td>-->
			...
			?>
				...
				<tr bgcolor="lightblue">
					<td colspan="8"><B>
							<font size="3">SUB TOTAL <?php echo number_format($total_items) . ';'; ?>
						</B>&nbsp;<i>Receipt Number <?php echo $rows_particlars['nr'] . ';  '; ?>CASHIER <?php echo $rows_particlars['User_Id'] . ';  '; ?> </i></font>
					</td>
				</tr>
				<tr bgcolor="red">
					<td colspan="13" bgcolor="red"></td>
			...
			?>
				...
				?>
					<tr>
						...
						<td><?php echo $rows_topup_items['description']; ?></td>
						<td><?php echo $class['purch_class']; ?></td>
				...
				<tr bgcolor="lightblue">
					<td colspan="8"><B>
							<font size="3">DEPOSITED <?php echo number_format($amt_deposited_topup) . ';  '; ?>
						</B>SUB TOTAL <?php echo number_format($total_topup_items) . ';'; ?><i>Rec# <?php echo $rows_topup['nr'] . ';'; ?> BankRef: <?php echo $rows_topup['bank_ref'] . ';'; ?>CASHIER <?php echo $rows_topup['User_Id']; ?> </i></font>
					</td>
					<td colspan="4"><B>
							<font size="3"><i>FINAL BILL(TOPUP)</i>:</font>
						</B></td>
					<td><span style="border-bottom:double black;">
							<font size="3"><?php echo number_format($rows_topup['price']); ?><i></i></font>
						</span></td>
				</tr>
				<tr bgcolor="red">
					<td colspan="13" bgcolor="red"></td>
          ...
          WHERE description='Refund' AND billelem.date_change BETWEEN '" . $date_from_timestamp . "' AND '" . $date_to_timestamp . "' " . $in_out_patient . " " . $and_PayType . " " . $fileNr . " ";
			...
			?>
				...
				?>
					<tr>
						<td colspan="8" bgcolor="grey"></td>
						<td><?php echo $rows_refund_items['description']; ?></td>
						<td><?php echo $class['purch_class']; ?></td>
				...
				<tr bgcolor="lightblue">
					<td colspan="8"><B>
							<font size="3">DEPOSITED <?php echo number_format($amt_deposited_refund); ?>&nbsp;
						</B>SUB TOTAL <?php echo number_format($total_refund_items) . ';'; ?>&nbsp;Rec#<?php echo $rows_refund['nr'] . '; '; ?>&nbsp;BankRef:<?php echo $rows_refund['bank_ref'] . '; '; ?>&nbsp;CASHIER <?php echo $rows_refund['User_Id'] . ';'; ?></font>
					</td>
					<td colspan="4"><B>
							<font size="3"><i>FINAL BILL(REFUND)</i>:</font>
						</B></td>
					<td><span style="border-bottom:double black;">
							<font size="3"><?php echo number_format($rows_refund['price']); ?><i></i></font>
						</span></td>
				</tr>
				<tr bgcolor="red">
					<td colspan="13" bgcolor="red"></td>
			...
			<?php if ($print) { ?>
				...
				<?php } else { ?>
                    ...
                    -->
						...
						<th width="89" scope="col"><B><span style="border-bottom:double black;">
									<font size="3"><?php echo number_format($total_all_exl_deposit + $total_items_deposit); ?></font>
								</span></B></th>
					</tr>
					<tr bgcolor="black">
						<td colspan="13" bgcolor="red"></td>
					...
					<table class="report" width="90%" border="0">
						<tr>
							<th align="left">
								<font size="3"><span style="border-bottom:double black;">MAIN SUMMARY:</span></font>
							</th>
							<th align="left">
								<font size="5">GRAND TOTAL:&nbsp;&nbsp;&nbsp;<span style="border-bottom:double black"><?php echo number_format($total_all_exl_deposit + $sum_deposits_and_topup) . '/='; ?></span></font><i>Excl.Refund</i>
							</th>
						...
						<tr align="left">
							<td>
								<font size="2">TOTAL DEPOSIT:&nbsp;&nbsp;&nbsp;<?php echo number_format($total_deposit); ?></font>
							</td>
						</tr>
						<tr>
							<td align="left">
								<font size="2">TOTAL TOPUP:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo number_format($total_topup); ?></span></font>
							</td>
						</tr>
						<tr>
							<td align="left">
								<font size="2">TOTAL REFUND:&nbsp;&nbsp;&nbsp;&nbsp;<?php echo number_format($total_refund); ?></font>
							</td>
						</tr>
						<tr>
							<td align="left">
								<font size="3"><B>NET DEPOSITS</B></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font size="3"><span style="border-bottom:double black;"><?php echo number_format($sum_deposits); ?></span></font>
							</td>
							<td><a href="./gui/downloads/detailed_revenue.csv"><img border=0 src="../../gui/img/common/default/savedisk.gif"></a></td>
						</tr>
		...
		function DailySummary($date_from, $date_to, $company, $admission, $dept = '', $insurance = '', $print = 0)
		{
			...
			?>
				...
				<table class="report" width="90%" border="1">
					...
					$sqlDateFrom = $dateFromArray[2].'-'.$dateFromArray[1].'-'.$dateFromArray[0].' 00:00:00';
					$sqlDateTo = $dateToArray[2].'-'.$dateToArray[1].'-'.$dateToArray[0].' 23:59:59';
                 
                    $timestampFrom=strtotime($sqlDateFrom);
                    $timestampTo=strtotime($sqlDateTo);
			...
			$this->sql = "SELECT 
						     DATE_FORMAT(FROM_UNIXTIME(date_change),'%Y-%m-%d')  as TAREHE, 
					         ifnull(SUM(CASE WHEN ds.purchasing_class='labtest' THEN bae.amount*bae.price END), 0) AS LAB,
					         ifnull(SUM(CASE WHEN ds.purchasing_class='drug_list' THEN bae.amount*bae.price END), 0) AS DRUGS, 
					         ifnull(SUM(CASE WHEN ds.purchasing_class='xray' THEN bae.amount*bae.price END), 0) AS XRAY,
					         ifnull(SUM(CASE WHEN ds.purchasing_class='dental' THEN bae.amount*bae.price END), 0) AS DENTAL,
					         ifnull(SUM(CASE WHEN ds.purchasing_class='eye-service' THEN bae.amount*bae.price END), 0) AS EYE,					         
					         ifnull(SUM(CASE WHEN ds.purchasing_class='minor_proc_op' THEN bae.amount*bae.price END), 0) AS MINOR_PROC,					         
					         ifnull(SUM(CASE WHEN ds.purchasing_class='surgical_op' THEN bae.amount*bae.price END), 0) AS SURG_OP,					         
					         ifnull(SUM(CASE WHEN ds.purchasing_class='service' THEN bae.amount*bae.price END), 0) AS CONS_SERV,
					         ifnull(SUM(CASE WHEN ds.purchasing_class='supplies' THEN bae.amount*bae.price END), 0) AS SUPPL,
					         ifnull(SUM(CASE WHEN ds.purchasing_class='supplies_laboratory' THEN bae.amount*bae.price END), 0) AS LAB_SUPP,
					         ifnull(SUM(CASE WHEN ds.purchasing_class='ortho_op' THEN bae.amount*bae.price END), 0) AS ORTH_OP, 
					         ifnull(SUM(CASE WHEN ds.purchasing_class='obgyne_op' THEN bae.amount*bae.price END), 0) AS OBGYNE_OP   
						...
						$resultDailySummary = $db->Execute($this->sql);
						// $rowHeaders=$resultDailySummary->fetchRow();
                        ...
                        while ($row = $resultDailySummary->fetchRow()) {
                        	$j++;
                        	if ($j==1) {
                        		echo "<thead>";
									echo "<tr>";
                        	...
                        	$i = -1;
                         	...
                         	echo "<td>".number_format($rowsTotal)."</td>";
                         	echo "</tr>";

                        }
                        $columnTotal[0] = "TOTAL";
                        echo "<tr>";
                        for ($i=0; $i < count($columnTotal) ; $i++){
                        	echo "<td>".number_format($columnTotal[$i]) ."</td>";
                        }
                        $columnTotal[0] = 0;
                        echo '<td class="bg-muted">'.number_format(array_sum($columnTotal)) .'</td>';
		...
		function services($date_from, $date_to, $company, $admission, $dept = '', $insurance = '', $print = 0)
		{
			...
			function cons_and_services($date_from, $date_to, $company, $admission, $dept = '', $insurance = '', $print = 0)
			{
				...
				<table class="report" width="90%" border="1">
					...
					<tr>
						...
						<td><?php echo number_format($total_drugs) ?></td>
						<td><?php echo number_format($total_xray); ?></td>
						<td><?php echo number_format($total_dental); ?></td>
						<td><?php echo number_format($total_eye_service); ?></td>
						<td><?php echo number_format($total_minor_proc_op); ?></td>
						<td><?php echo number_format($total_surgical_op); ?></td>
						<td><?php echo number_format($total_service); ?></td>
						<td><?php echo number_format($total_supplies); ?></td>
						<td><?php echo number_format($total_lab_supplies); ?></td>
			...
			function MtuhaDiagnosisBlocks($nr, $companies)
			{
				/*
			              ...
			              +255767809660
		*/
				global $db, $LDDateFrom, $LDDateTo, $root_path, $LDShow, $date_format, $dx, $dxCode;
					...
					if ($_POST['date_from'] <= $_POST['date_to']) {
						if (isset($_POST['admission_id']) || !empty($_POST['admission_id'])) {
							...
							if (!$error) {
								...
								//will continue from here adding dx name and icd 10 in the query

								// echo $_POST['dxCode'];
								// echo $_POST['dx'];

								if (isset($_POST['dxCode']) && !empty($_POST['dxCode'])) {
									$code = 'AND ICD_10_code LIKE ' . "'" . '%' . $_POST['dxCode'] . '%' . "'";

									//echo ' ya kwanza'.$dx;
								}

								if (isset($_POST['dx']) && !empty($_POST['dx'])) {
									$desc = 'AND ICD_10_description LIKE ' . "'" . '%' . $_POST['dx'] . '%' . "'";
								}
              ...
              WHERE timestamp BETWEEN '" . $DateStampFrom . "' AND '" . $DateStampTo . "' AND diagnosis_type='final' " . $dept_ward . " " . $cash_credit . "" . $dx . " ) ";
			...
			?>
				<form name="form1" method="POST" action="" onsubmit="return inputvalue()">
					<table width="100%" border="0" align="center">
						<tr>
							<td><?php echo $LDDateFrom; ?><input name="date_from" id="datepicker" type="text" size=10 maxlength=10 value="<?php echo $_POST['date_from'] ?>" placeholder="DateFrom">
 ...
 -->
								...
								<?php echo $LDDateTo; ?>
								<input name="date_to" type="text" id="datepicker1" size=10 maxlength=10 value="<?php echo $_POST['date_to'] ?>" placeholder="DateTo"><BR>
								<input type="text" name="dx" placeholder="diagnosis name"> <b>OR</b><BR>
								<input type="text" name="dxCode" placeholder="icd10 code"><BR>
					...
					if ($_POST['date_from'] <= $_POST['date_to']) {
						if (isset($_POST['admission_id']) || !empty($_POST['admission_id'])) {
							...
							if (!$error) {
								...
								$total_male_female = 0;
								while ($rows_group = $result_group->FetchRow()) {
									//first while loop
									...
									$j = 0;
									while ($rows_values = $result_values_group->FetchRow()) {
 ...
 -->
			...
			function drug_consumption($MysqlDateFrom, $MysqlDateTo, $insurance, $admission)
			{
				...
				$gc = "SELECT type, value FROM care_config_global WHERE
		 type='transmit_to_weberp_enabled'";

				$gc_rs = $db->execute($gc);

				if ($gc_row = $gc_rs->FetchRow()) {
					$GLOBAL_CONFIG['transmit_to_weberp_enabled'] = $gc_row['value'];
				}
           ...
           WHERE bae.prescriptions_nr=cep.nr AND erpmaster.stockid=cep.partcode";
            ...
            <table border=" 1" class="report" style="width: 98%;">
					...
					<tr style="background-color:lightgrey; font-weight: 24px;">
						<th><?php echo $HeaderArray[0] ?></th>
						<th><?php echo $HeaderArray[1]; ?></th>
						<th><?php echo $HeaderArray[2]; ?></th>
						<th><?php echo $HeaderArray[3] ?></th>
						<th><?php echo $HeaderArray[4]; ?></th>
						<th><?php echo $HeaderArray[5]; ?></th>
						<th><?php echo $HeaderArray[6]; ?></th>
					</tr>
