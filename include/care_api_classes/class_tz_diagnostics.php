...
class Diagnostics extends Encounter {
	...
	function get_array_fuzzysearch_results($keyword) {
		...
		if ($keyword == "*") {
			...
			{
				$this->sql = "SELECT `" . $this->tbl_col_code . "`, `" . $this->tbl_col_content . "` FROM " . $this->tbl . " LIMIT 250";
			} else {
				$this->sql = "SELECT `" . $this->tbl_col_code . "`, `" . $this->tbl_col_content . "` FROM " . $this->tbl . " LIMIT 250";
			}
			...
			while ($this->row_elem = $this->rs_tbl->FetchRow()) {
				...
				if ($debug) {
					echo $this->row_elem['$this->tbl_col_content'];
				}
	...
	function GetDiagnosisGroupItems($grupID) {
		...
		($debug) ? $db->debug = TRUE : $db->debug = FALSE;
		if ($debug) {
			echo "calling: GetDiagnosisGroupItems<br>";
		}
	...
	function GetAllDiagnosisGroupItems() {
		...
		($debug) ? $db->debug = TRUE : $db->debug = FALSE;
		if ($debug) {
			echo "calling: GetAllDiagnosisGroupItems<br>";
		}
	...
	function insert_diagnosis_as_new($pid, $encounter_nr, $ICD10_code, $ICD10_description, $comment, $type, $doctor) {
		...
		$this->sql = "SELECT series, opd_series, opd_name, ipd_series, ipd_name  FROM care_icd10_en WHERE diagnosis_code ='" . $ICD10_code . "'";
		$diagQuery = $db->Execute($this->sql);
		while ($d_row = $diagQuery->FetchRow()) {
			$series = $d_row['series'];
			$opd_series = $d_row['opd_series'];
			$opd_name = $d_row['opd_name'];
	...
	*/
	...
	function Display_Quicklist_Elements($selected) {
		...
		if (!$counter) {
			echo '<option value="-1">Empty!</opion>';
		}
	...
	function Display_Quicklist_Elements_Items($id) {
		...
		if (!$counter) {
			echo '<option value="-1">' . $LDNothingFoundTryFuzzySearch . '</opion>';
		}
