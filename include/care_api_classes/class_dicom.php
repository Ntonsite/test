...
define('TOOLKIT_DIR', $dcmtk_path); // CHANGE THIS IF YOU HAVE DCMTK INSTALLED SOMEWHERE ELSE
// WINDOWS EXAMPLE
...
if (RUNNING_WINDOWS) {
	...
	define('BIN_DCMP2PGM', TOOLKIT_DIR . '/dcmp2pgm.exe');
} else {
	...
	define('BIN_DCMP2PGM', TOOLKIT_DIR . '/dcmp2pgm');
...
#print "$dump\n";
		...
		foreach (explode("\n", $dump) as $line) {
			...
			if (!$found) {
				// a couple of tags are not in [] preceded by =
				$t = preg_match_all("/\=(.*)\#/", $line, $matches);
				if (isset($matches[1][0])) {
					$found = 1;
					$val = $matches[1][0];
...
###
	...
	function dcm_show() {
		...
		$this->bmp_file = $this->file . '.bmp';

		$show_cmd = BIN_DCMP2PGM . " " . $this->file . " " . $this->bmp_file;
		$out = Execute($show_cmd);
		print_r($out);

		if (file_exists($this->bmp_file)) {
			$filesize = filesize($this->bmp_file);
		}
