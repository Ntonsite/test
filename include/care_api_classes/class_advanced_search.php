...
class advanced_search extends Core {
	...
	function get_equal_words($column, $table, $word_to_compare, $sharpeness, $givekey) {
		...
		while (list($u, $v) = $this->helper->customEach($res_array)) {
			if (!$word_to_compare) {
				...
				if ($percent == 100) {
					...
					if ($debug) {
						echo "Yepp...$s1 and $s2 are equal!<br>";
					}
					...
					// we have a selection:
					if ($debug) {
						echo "<br>$s1 and $s2 are more than $percent equal<br>";
					}
				...
				if ($debug) {
					echo "$all_record_array[$arr_index]--";
				}
		...
		if (!$word_to_compare) {
			...
			if ($debug) {
				echo "<br>Perfect-Hit:" . $PERFECT_HIT . "<br>";
			}

			if ($debug) {
				echo "<br>Hit-Index:" . $hit_index . "<br>";
			}
