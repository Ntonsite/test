...
class Core {
	...
	*/
	...
	function _prepSaveArray() {
		...
		while (list($x, $v) = $helper->CustomEach($this->ref_array)) {
	...
	function insertDataFromArray($array) {
		...
		$values = substr_replace($values, '', (strlen($values)) - 1);
         ...
         //echo 'core table= '.$this->coretable;
	...
	function getLastPk($tableName, $columnName) {
		global $db;
		$id = 0;
		$lastSQL = "SELECT MAX($columnName) as $columnName FROM $tableName";
		$lastResult = $db->Execute($lastSQL);
		if (@$lastResult && $lastResult->RecordCount()) {
			$lastRow = $lastResult->FetchRow();
			$id = $lastRow['batch_nr'];
		}
		return $id;
	}
	...
	function ConcatFieldString($fieldname, $str = '') {
		...
		case 'mysqli':return $fieldname . " " . $str;
			break;
	...
	function CheckNumber($string) {
		...
		*/
		...
		while (list($string_x, $string_v) = $helper->CustomEach($string_array)) {
			while (list($x, $v) = $helper->CustomEach($forbidden_chars_array)) {
