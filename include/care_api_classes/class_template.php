...
class Template {
	...
	function __construct($root = './', $path = '', $theme = 'default') {
		$this->tp_root = $root;
		if (empty($path)) {
			$this->tp_path = $root . $this->default_path;
		} else {
			$this->tp_path = $path;
		}
	...
	function load($tp_fn) {
		if (!empty($tp_fn)) {
			...
			if (file_exists($tpsrc . $tp_fn)) {
				$this->filename = $tpsrc . $tp_fn;
			} else {
				$this->filename = $this->tp_root . $this->default_path . $this->default_theme . '/' . $tp_fn;
			}
