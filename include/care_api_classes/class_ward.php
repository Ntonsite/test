	...
	*/
	...
	function _getWardOccupants($ward_nr, $date) {
		...
		$this->sql = "SELECT r.location_nr AS room_nr,
									...
									b.nr AS bed_loc_nr,
									p.pid,
									UPPER(p.name_last) as name_last,
							...
							FROM $this->tb_location AS r
									LEFT JOIN $this->tb_location AS b  ON 	(r.encounter_nr=b.encounter_nr
																								...
																								AND b.date_from<='$date'
										 														AND ('$date'<=b.date_to OR b.date_to ='$dbf_nodate' OR b.date_to = '1980-01-01')
																								)
							...
							WHERE  r.group_nr=$ward_nr
											...
											AND r.status NOT IN ('$pstat','closed',$this->dead_stat)
										 	AND ('$date'<=r.date_to OR r.date_to ='$dbf_nodate' OR r.date_to ='1980-01-01')
	...
	function updateWard($nr, &$data) {
		...
		if (isset($this->data_array['date_create'])) {
			unset($this->data_array['date_create']);
		}

		if (isset($this->data_array['create_id'])) {
	...
	function hasPatient($nr) {
		...
		$this->sql = "SELECT nr FROM $this->tb_location
						WHERE type_nr=4
							AND group_nr=$nr
							AND date_from NOT  IN  ('0000-00-00','','1980-01-01')
							AND date_to  IN  ('0000-00-00','','1980-01-01')
	...
	function createWaitingInpatientList($ward_nr = 0) {
		global $db;
		if ($ward_nr) {
			$cond = "AND current_ward_nr='$ward_nr'";
		} else {
			$cond = '';
		}

		//if(empty($key)) return false;
		$this->sql = "SELECT e.encounter_nr, e.encounter_class_nr, e.current_ward_nr, p.pid, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth, p.sex,w.ward_id
