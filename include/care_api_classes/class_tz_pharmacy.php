...
class Product extends Core {
	...
	function _updatePricingDataFromArray(&$array, $field_nr = '', $isnum = TRUE) {
		...
		if ($isnum) {
			if (empty($field_nr) || ($isnum && !is_numeric($field_nr))) {
				return FALSE;
			} else
			if (empty($field_nr)) {
				return FALSE;
	...
	function get_array_search_results($keyword, $result_filter) {
		...
		if ($debug) {
			echo "get_array_search_results($keyword, $result_filter)";
		}
		...
		if ($debug) {
			echo "class_tz_diagnostics::get_array_select_results starts here<br>";
		}
		...
		if (true) {
			...
			if (@$result_filter) {
				$sql .= " AND purchasing_class = '$result_filter'";
			}
			return $db->Execute($sql);

		} else {
			$this->sql = "SELECT `item_id`, `partcode`, `item_description`,`not_in_use`,`min_level`, nhif_item_code FROM " . $this->tbl_product_items . " WHERE `purchasing_class`='" . $keyword . "'";
			...
			if ($debug) {
				echo "class_tz_diagnostics::get_array_select_results -> TMP-table is created<br>";
			}

			$this->arr_keywords = explode(" ", $keyword);

			if ($debug) {
				echo "class_tz_diagnostics::get_array_select_results -> keyword >" . $keyword . "< is slpitted into " . count($this->arr_keywords) . " words<br>";
			}
			...
			while ($this->row_elem = $this->rs_tbl->FetchRow()) {
				if ($debug) {
					echo "class_tz_diagnostics::get_array_select_results -> reading to the description >>" . $this->row_elem[$this->tbl_col_content] . "<<<br>";
				}

				if ($debug) {
					echo $this->row_elem['item_description'];
				}
				...
				while (list($keyword_index, $keyword_value) = each($this->arr_keywords)) {
					...
					while (list($content_index, $content_value) = each($this->arr_tbl_col_content)) {
						...
						if (strlen($content_value) > 3 && strlen($keyword_value) > 3) {
							if ($debug) {
								echo "compare:" . $keyword_value . "<->" . $content_value . ":";
							}
							...
							if (strcmp($content_value, $keyword_value) == 0) {
								...
								if ($debug) {
									echo "<b>WOUW</b>:" . $keyword_value . " with " . $content_value . " gives a nice factor of:" . $this->nice_factor . "<br>";
								}
								...
								if ($debug) {
									echo "<br> metaphone value of $keyword_value is $m1::metaphone value of $content_value is $m2<br>";
								}
								...
								if ($debug) {
									echo 'Nice: ' . $this->nice . '= -1/2 * ' . $levenshtein . ' + ' . strlen($keyword_value) . '<br>';
								}

								// Step 2: Percent value of levensthein comparison:
								$this->nice_factor = 100 / strlen($keyword_value) * $this->nice;
								if ($debug) {
									echo 'DEBUG: current nice factor of this run is:' . $this->nice_factor . '<br>';
								}
								...
								if ($this->nice_factor) {
									array_push($this->arr_nice_factor, $this->nice_factor);
									if ($debug) {
										echo "<u>" . $keyword_value . "</u> with <u>" . $content_value . "</u> gives a nice factor of:" . $this->nice_factor . "<br>";
									}
...
//this selects items from specific cateogry
	...
	function get_items_nhif_item_code($item_id) {
		global $db;
		$this->sql = "SELECT nhif_item_code FROM care_tz_drugsandservices WHERE item_id='" . $item_id . "'";
		$this->rs = $db->Execute($this->sql);
		if ($this->rs->RecordCount()) {
			$this->elem = $this->rs->FetchRow();
			return $this->elem[0];
	...
	function get_selians_item_alt_price($item_id, $NUMBER) {
		...
		*/
		global $db;
		if (($NUMBER < 0 || $NUMBER > 12) || empty($item_id)) {
			return FALSE;
		}

		$field = array('unit_price', 'unit_price_1', 'unit_price_2', 'unit_price_3', 'unit_price_4', 'unit_price_5', 'unit_price_6', 'unit_price_7', 'unit_price_8', 'unit_price_9', 'unit_price_10', 'unit_price_11');
	...
	function getProductNHIFSchemes($item_id) {
		global $db;
		$items = [];
		$sql = "SELECT * FROM care_tz_drugsandservices_nhifschemes WHERE item_id = '$item_id'";
	...
	function getAllPricesColumns($item_id) {
		global $db;
		$return_string = ""; // local variable of this method declared here

		$this->sql = "SELECT unit_price, unit_price_1, unit_price_2, unit_price_3, unit_price_4, unit_price_5, unit_price_6, unit_price_7, unit_price_8, unit_price_9, unit_price_10, unit_price_11, unit_price_12, unit_price_13, unit_price_14, unit_price_15, unit_price_16, unit_price_17, unit_price_18, unit_price_19, unit_price_20 FROM care_tz_drugsandservices WHERE item_id='" . $item_id . "'";
		$this->rs = $db->Execute($this->sql);
		$prices = array();
		if ($this->rs->RecordCount()) {
			$prices = $this->rs->FetchRow();
		}
		return $prices;
	}

	// function doctorQualification(){
	// 	global $db;
	// 	$qualification=array();
	// 	$this->sql="SELECT * FROM `care_role_person`";
	// 	$result=$db->Execute($this->sql);
	// 	while($result->RecordCount()>0){
	// 		$qualification=$result->FetchRow();
