...
class Stock_tz extends Product {
	...
	function get_order_array_search_results($keyword, $supplier) {
		...
		if ($debug) {
			echo "class_tz_diagnostics::get_order_array_select_results starts here<br>";
		}
		...
		if ($keyword == "*") {
			...
			if ($debug) {
				echo "class_tz_diagnostics::get_order_array_select_results -> TMP-table is created<br>";
			}

			$this->arr_keywords = explode(" ", $keyword);

			if ($debug) {
				echo "class_tz_diagnostics::get_order_array_select_results -> keyword >" . $keyword . "< is splitted into " . count($this->arr_keywords) . " words<br>";
			}
			...
			if ($this->rs_tbl = $db->Execute($this->sql)) {
				while ($this->row_elem = $this->rs_tbl->FetchRow()) {
					if ($debug) {
						echo "class_tz_diagnostics::get_order_array_select_results -> reading to the description >>" . $this->row_elem[$this->tbl_col_content] . "<<<br>";
					}

					if ($debug) {
						echo $this->row_elem['Supplier_item_name'];
					}
						...
						while (list($content_index, $content_value) = each($this->arr_tbl_col_content)) {
							...
							if (strlen($content_value) > 3 && strlen($keyword_value) > 3) {
								...
								if (strcmp($content_value, $keyword_value) == 0) {
									...
									if ($debug) {
										echo 'DEBUG: ' . $this->nice_factor . '<br>';
									}
									...
									if ($this->nice_factor > 90) {
										$this->nice_factor = 500;
									}

									if ($this->nice_factor >= 75) {
										$this->nice_factor = 200;
									}
	...
	function get_all_stock_amounts($orderby, $purchasing_class = 'all') {
		...
		if ($debug) {
			echo 'get_all_stock_amounts(' . $orderby . ', ' . $purchasing_class . '); ';
		}
	...
	function get_stock_amounts_of_drugsandservices_id($drugsandservices_id) {
		...
		$debug = FALSE;
		if ($debug) {
			echo 'get_stock_amounts_of_drugsandservicesid(' . $drugsandservices_id . '); ';
		}

		($debug) ? $db->debug = TRUE : $db->debug = FALSE;
		if (!is_numeric($drugsandservices_id) || !is_numeric(CURRENT_STOCK_ID)) {
			return false;
		}
	...
	function transfer_insert_into_stock($id) {
		...
		$debug = false;
		if ($debug) {
			echo 'transfer_insert_into_stock(' . $id . '); ';
		}
	...
	function transfer_update($id, $newdrugid, $amount) {
		...
		$debug = false;
		if ($debug) {
			echo 'transfer_update(' . $id . ', ' . $newdrugid . ', ' . $amount . '); ';
		}
	...
	function create_new_transfer($Drugsandservices_id, $Transfer_from, $Transfer_to) {
		...
		$debug = false;
		if ($debug) {
			echo 'create_new_transfer(' . $Drugsandservices_id . ', ' . $Transfer_from . ', ' . $Transfer_to . '); ';
		}
	...
	function stock_update($id, $description, $type, $trigger) {
		...
		$debug = false;
		if ($debug) {
			echo 'stock_update(' . $id . ', ' . $description . ', ' . $type . ', ' . $trigger . '); ';
		}
