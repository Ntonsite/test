...
class Image extends Core {
	 ...
	 * @var string
	 */
	var $mimefilter = 'jpg,jpeg,gif,png,bmp,dcm,pdf,doc,docx,docm,dotx,dotm,docb,xls,xlt,xlm,xlsx,xlsm,xltx,xltm,csv';
	...
	function saveUploadedImage($img, $fpath = '', $fname = '') {
		if (empty($fpath)) {
			$fpath = $this->def_root_path;
		}
		if (empty($fname)) {
			return copy($img['tmp_name'], $fpath . $img['name']);
		} else {
			return copy($img['tmp_name'], $fpath . $fname);
		}
	}
	...
	function DicomImages($key = '') {
		...
		$this->sql = "SELECT i.nr, i.encounter_nr, i.upload_date, i.max_nr, LENGTH(i.notes) AS note_len,
						p.pid, UPPER(p.name_last) as name_last, CONCAT(p.name_first,' ', p.name_2) AS name_first, p.date_birth
		...
		$this->sql .= "  i.pid=p.pid AND i.status NOT IN ('cancelled',$this->dead_stat) ORDER BY p.name_last";
		//echo $this->sql;
	...
	function saveImgDiagnosticData($data) {
		...
		$this->data_array = $data;
		if ($insertedRow = $this->insertDataFromInternalArray()) {
