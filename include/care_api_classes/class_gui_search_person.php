...
class GuiSearchPerson {
	...
	function display($skey = '') {
		...
		$this->thisfile = $filename ?? basename($_SERVER['PHP_SELF']);
		$this->searchkey = $skey;
		$this->mode = $mode;
		...
		$pagen = new Paginator($pgx, $this->thisfile, $_SESSION['sess_searchkey'], $root_path, 'asc');
		...
		if (empty($GLOBAL_CONFIG['pagin_person_search_max_block_rows'])) {
			...
			$pagen->setMaxCount($max_block_rows ?? 0);
		} else {
			...
			if ($ergebnis) {
				if ($linecount == 1) {
					if (($this->auto_show_bynumeric && $person->is_nr) || $this->auto_show_byalphanumeric) {
						...
						header("location:" . $this->targetfile . "?sid=" . $sid . "&lang=" . $lang . "&pid=" . $zeile['pid'] . "&edit=1&status=" . $status . "&user_origin=" . $user_origin . "&noresize=1&mode=$mode&moderad=new");
						exit;
					}
				}
				...
				$show_sqlquery=isset($show_sqlquery)? $show_sqlquery : "";
				if (@$show_sqlquery) {
					echo $sql;
				}
		...
		if ($linecount) {
			...
			while ($zeile = $ergebnis->FetchRow()) {
				...
				if ($zeile['status'] == '' || $zeile['status'] == 'normal') {
					...
					#
					if ($zeile['death_date'] && $zeile['death_date'] != $dbf_nodate && $zeile['death_date'] != '1980-01-01' ) {
