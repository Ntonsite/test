...
class GuiInputPerson {
	 ...
	 */
	function createTR($error_handler, $input_name, $ld_text, $input_val, $colspan = 1, $input_size = 35, $red = FALSE, $inputId = "") {
		echo '<tr>
		...
		if ($input_name == 'membership_nr') {
			echo '<td  style="position: relative;" class="reg_input" colspan=' . $colspan . '><input name="' . $input_name . '" type="text" id="membershipnr" size="' . $input_size . '" value="' . $input_val . '" > </td>';
			echo '<td>
            <div style="position: absolute; top: 40%; margin-left: 200px;">
              <input type="text" placeholder="NHIF Card No" id="nhifcardnr" style="display: block; width: 100%; padding: .375rem .75rem; font-size: 1rem; line-height: 1.5; color: #495057; background-color: #fff; background-clip: padding-box; border: 1px solid #ced4da; border-radius: .25rem; transition: border-color .15s ease-in-out,box-shadow .15s ease-in-out;"  /><br>
              <button type="button" disabled id="membershipbtn" class="btn btn-primary btn-sm" onclick="verify_card()">Get NHIF Card Details</button><br><br>

              <p><strong>Full Name:</strong> <span id="nhif_patient_fullname"></span></p>
              <p><strong>Card Status:</strong> <span id="nhif_patient_cardstatus"></span></p>
              <p><strong>Authorization:</strong> <span id="nhif_patient_authorization"></span></p>
              <p><strong>Authorization Number:</strong> <span id="nhif_patient_authorizationnumber"></span></p>
              <p><strong>Remarks:</strong> <span id="nhif_patient_remarks"></span></p>
            </div>
          </td>';
		} else {
			echo '<td  class="reg_input" colspan=' . $colspan . '><input id="' . $inputId . '" name="' . $input_name . '" type="text" size="' . $input_size . '" value="' . $input_val . '" > </td>';
		}
...
//        extract($_POST);
		...
		$person_foto_path = @$GLOBAL_CONFIG['person_foto_path'] ? $GLOBAL_CONFIG['person_foto_path'] : "";
		$photo_path = (is_dir($root_path . $person_foto_path)) ? $person_foto_path : $this->default_photo_path;
		...
		if (($mode == 'save') || ($mode == 'forcesave')) {
			...
			if (is_array($result_array = $search_obj->get_equal_words("name", "care_tz_religion", false, 65, 'nr')) && @$religion && empty($person_religion_hide)) {
...
//addr_citytown_nr='$addr_citytown_nr',
				...
				if (($update)) {
					...
					$sql = "UPDATE $dbtable SET
                                                 ...
                                                 name_2='" . addslashes($name_2 ?? "") . "',
                                                 name_3='" . addslashes($name_3 ?? "") . "',
                                                 name_middle='" . addslashes($name_middle ?? "") . "',
                                                 name_maiden='" . addslashes($name_maiden ?? "") . "',
                                                 name_others='" . addslashes($name_others ?? "") . "',
                                                 date_birth='" . formatDate2STD($date_birth, $date_format ?? "") . "',
                                                 blood_group='" . trim($blood_group ?? "") . "',
                                                 rh='" . addslashes($rh ?? "") . "',
                                                 sex='" . addslashes($sex ?? "") . "',
                                                 addr_str='" . addslashes($addr_str ?? "") . "',
                                                 addr_str_nr='" . addslashes($addr_str_nr ?? 0) . "',
                                                 addr_zip='" . addslashes($addr_zip ?? "") . "',
                                                 addr_citytown_nr='" . addslashes($addr_citytown_nr ?? 0) . "',
                                                 addr_citytown_name='" . addslashes($addr_citytown_name ?? "") . "',
                                                 phone_1_nr='" . addslashes($phone_1_nr ?? "") . "',
                                                 phone_2_nr='" . addslashes($phone_2_nr ?? "") . "',
                                                 cellphone_1_nr='" . addslashes($cellphone_1_nr ?? "") . "',
                                                 cellphone_2_nr='" . addslashes($cellphone_2_nr ?? "") . "',
                                                 fax='" . addslashes($fax ?? "") . "',
                                                 email='" . addslashes($email ?? "") . "',
                                                 citizenship ='" . addslashes($citizenship ?? "") . "',
                                                 civil_status='" . addslashes($civil_status ?? "") . "',
                                                 membership_nr='" . addslashes($membership_nr ?? "") . "',
                                                 sss_nr='',
                                                 nat_id_nr='',
                                                 religion='" . addslashes($religion ?? "") . "',
                                                 ethnic_orig='" . addslashes((int) $ethnic_orig ?? 0) . "',
                                                 insurance_ID='$insurance_ID',
                                                 national_id = '" . addslashes($national_id ?? "") . "',
                                                 employee_id = '$employee_id',
                                                 allergic = '" . addslashes($allergic ?? "") . "',
                                                 allergy = '" . addslashes($allergy ?? "") . "',
                                                 sub_insurance_id = '$sub_insurance_id',
                                                 prescribe_without_diagnosis = '$prescribe_without_diagnosis',
                 ...
                 ward='$ward',";
					...
					$updatedDesc = $person_obj->ConcatHistory("Updated " . date('Y-m-d H:i:s') . " " . $_SESSION['sess_user_name'] . " \n");
					$sql .= " history='" . $updatedDesc . "', modify_id='" . $_SESSION['sess_user_name'] . "' WHERE pid=$pid";
					//$db->debug=FALSE;
					...
					if ($ok) {
						...
						if (@$insurance_show) {
							...
							} elseif (@$insurance_nr && $insurance_nr && $insurance_firm_name && $insurance_class_nr) {
								$insure_data = array('insurance_nr' => $insurance_nr,
									'firm_id' => $insurance_firm_id,
									'pid' => $pid,
									'class_nr' => $insurance_class_nr,
					...
					if (empty($person_obj->InitPIDExists($GLOBAL_CONFIG['person_id_nr_init'] ?? ""))) {
					...
					if (empty($error_person_exists) || $mode == 'forcesave') {
						...
						if ($person_obj->insertDataFromInternalArray()) {
							...
							if (!$update) {
								...
								$lastIDSQL = "SELECT MAX(pid) as pid FROM care_person";
								if ($lastresult = $db->Execute($lastIDSQL)) {
									if ($lastresult->RecordCount()) {
										$pidRow = $lastresult->FetchRow();
										$pid = $pidRow['pid'];
									} else {
										$pid = 0;
									}
...
*/
		...
		*/
		...
		$theme_com_icon = $theme_com_icon ?? "blue_aqua";
...
return false;
		...
		?>
                ...
                var prescribe_without_diagnosis = $('input[name=prescribe_without_diagnosis]:checked').val();
...
require_once $root_path . 'include/care_api_classes/class_multi.php';
		...
		$val = explode('|', $v);
		$showPrescribeWithoutDiagnosis = 0;
		if (@$val[13] && $val[13] == 1) {
			$showPrescribeWithoutDiagnosis = 1;
		}
		if ($showPrescribeWithoutDiagnosis == 0): ?>
                  else if (!prescribe_without_diagnosis) {
                    alert('Please check whether to precribe without diagnosis');
                    return false;
...
<?php $thisfile = basename($_SERVER['PHP_SELF']);?>
...
//Check if Autogenerate file number is set in global config
		if ($glob_obj->getConfigValue('generate_hosp_fileno')) {
			//Returns 1 if set to true
			?>
...
if ($person_obj->IsHospitalFileNrMandatory()) {
		...
		$this->createTR($errornamefirst ?? "", 'name_first', $LDFirstName . ' *', $name_first ?? "", '', '', FALSE, 'FirstName');

		$this->createTR($errorname2 ?? "", 'name_2', $LDName2 . ' *', $name_2 ?? "", '', '', FALSE, 'MiddleName');

		$this->createTR($errornamelast ?? "", 'name_last', $LDLastName . ' *', $name_last ?? "", '', '', FALSE, "LastName");
...
echo '<SELECT name="name_maiden" onChange="list_popup(this, \'tribe\');">';
			...
			foreach ($tribe_array as $unit) {
				...
				if ((isset($name_maiden) && strtoupper($name_maiden) == strtoupper($unit[1]))) {
					$check = 'selected';
				} else {
					$check = '';
				}
		...
		?><?php echo $LDBday ?> *</font>:
                            </td>
                            <td class="reg_input">
                                <FONT SIZE=-1  FACE="Arial">
                                <input name="date_birth" id="DateOfBirth" type="text" size="15" maxlength=10 value="<?php
if (@$date_birth) {
			if (@$mode && $mode == 'save' || @$error || @$error_person_exists) {
				echo $date_birth;
			} else {
...
if (@$errorsex) {
		...
		echo $LDSex . ' *' . '</font>';
		?>:<td>
                                <input name="sex" type="radio" id="maleGender" value="m"  <?php if (@$sex && $sex == "m") {
			echo "checked";
		}
		?>><?php echo $LDMale ?>&nbsp;&nbsp;
                                <input name="sex" type="radio" id="femaleGender" value="f"  <?php if (@$sex && $sex == "f") {
			echo "checked";
		}
		?>>
...
echo $LDFemale;
		...
		# Clean blood group
		$blood_group = @$blood_group ? trim($blood_group) : "";
		?>
                            </td>
                        </tr>
                        <tr>
...
echo '<SELECT name="religion" onChange="list_popup(this,\'religion\');">';
		...
		?>
                        ...
                        <?php
if (empty($person_name_others_hide)) {
//      $this->createTR($errornameothers, 'name_others', $LDNameOthers,$name_others);
		...
		?>
                        ...
                        <?php echo $LDBloodGroup ?>:
                                                ...
                                                <td class="reg_input">
                                                        <FONT SIZE=-1  FACE="Arial">
                                                        <input name="blood_group" type="radio" value="A"  <?php if (@$blood_group && $blood_group == 'A') {
			echo 'checked';
		}
		?>><?php echo $LDA ?>&nbsp;&nbsp;
                                                        <input name="blood_group" type="radio" value="B"  <?php if (@$blood_group && $blood_group == 'B') {
			echo 'checked';
		}
		?>><?php echo $LDB ?>&nbsp;&nbsp;
                                                        <input name="blood_group" type="radio" value="AB"  <?php if (@$blood_group && $blood_group == 'AB') {
			echo 'checked';
		}
		?>><?php echo $LDAB ?>&nbsp;&nbsp;
                                                        <input name="blood_group" type="radio" value="O"  <?php if (@$blood_group && $blood_group == 'O') {
			echo 'checked';
		}
		?>><?php echo $LDO ?>
                                                </td>
		...
		?> <?php echo $LDCivilStatus ?></font>:
                                                </td>
                                                <td colspan=2 class="reg_input">
                                                        <FONT SIZE=-1  FACE="Arial"> <input name="civil_status" type="radio" value="single"  <?php if (@$civil_status && $civil_status == "single") {
			echo "checked";
		}
		?>><?php echo $LDSingle ?>&nbsp;&nbsp;
                                                        <input name="civil_status" type="radio" value="married"  <?php if (@$civil_status && $civil_status == "married") {
			echo "checked";
		}
		?>><?php echo $LDMarried ?>
                                                        <FONT SIZE=-1  FACE="Arial"> <input name="civil_status" type="radio" value="divorced"  <?php if (@$civil_status && $civil_status == "divorced") {
			echo "checked";
		}
		?>><?php echo $LDDivorced ?>&nbsp;&nbsp;
                                                        <input name="civil_status" type="radio" value="widowed"  <?php if (@$civil_status && $civil_status == "widowed") {
			echo "checked";
		}
		?>><?php echo $LDWidowed ?>
                                                        <FONT SIZE=-1  FACE="Arial"> <input name="civil_status" type="radio" value="separated"  <?php if (@$civil_status && $civil_status == "separated") {
			echo "checked";
		}
		?>><?php echo $LDSeparated ?>&nbsp;&nbsp;
                                                </td>
                                                </tr> -->
                        <tr>
                            <td class="reg_item">
...
echo $LDFirstTimePat . ' * </font>';
		?>:</td>
                            <td>
                                <input name="is_first_reg" type="radio" value="1"  <?php if (@$is_first_reg && $is_first_reg == "1") {
			echo "checked";
		}
		?>><?php echo $LDYes ?>&nbsp;&nbsp;
                                <input name="is_first_reg" type="radio" value="0"  <?php if (@$is_first_reg && $is_first_reg == "0") {
			echo "checked";
		}
		?>>
...
if (@$errormaiden) {
			...
			$sql = "SELECT DISTINCT region_id, region_name FROM care_tz_region region INNER JOIN care_tz_district distrcit ON distrcit.is_additional=region.region_id INNER JOIN care_tz_ward ward ON distrcit.district_id=ward.is_additional ORDER BY region_name";
			//$sql="SELECT region_id,region_name FROM view_care_region_district_ward GROUP BY reggion_id order by region_name ";
			$catchment_area_obj = $db->Execute($sql);
			echo '</td>
        <td  class="reg_input" colspan=1>
          <select name="region" id="regionid" size="1" onChange="getDistricts(this.value,\'dstr\');
          redirect(this.options.selectedIndex)">

            <option value="-1" id="-1">---select region--------</option>';
			print ' <option value="31" id="31" style="background:yellow; font-weight:bold;">--Unknown--</option>';

			while ($catchment_area_row = $catchment_area_obj->FetchRow()) {
				$selected = "";
				if (@$_POST['region'] && $_POST['region'] == $catchment_area_row['region_id']) {
					$selected = "selected";
				}

				if ($catchment_area_row['region_id'] != 31) {
					echo '<option ' . $selected . ' value="' . $catchment_area_row['region_id'] . '" >' . $catchment_area_row['region_name'] . '</option>';
				} else {
					echo '<option ' . $selected . ' value="' . $catchment_area_row['region_id'] . '" style="display:none;">' . $catchment_area_row['region_name'] . '</option>';
				}
...
echo '<SELECT name="addr_citytown_nr" onChange="list_popup(this,\'city\');">';
		...
		if (is_array($town_array) || is_object($town_array)) {
...
if (@$errorzip) {
		...
		?><input name="addr_zip" type="text" size="10" value="<?php echo $addr_zip; ?>" >
                            ...
                            <?php
$errorinsurancecoid = @$errorinsurancecoid ?? "";
		$errorinsuranceclass = @$errorinsuranceclass ?? "";
		$insurance_category = @$insurance_category ?? "";
		if (@$insurance_show) {
			if (empty($person_insurance_1_nr_hide)) {
				?>
                                    <!--<tr>
                                    <td class="reg_item">
		...
		if (empty($person_phone_1_nr_hide)) {
...
require_once $root_path . 'include/care_api_classes/class_multi.php';
        ...
        <?php if ($showPrescribeWithoutDiagnosis == 0): ?>
           <tr>
            <td class="reg_item">
                <FONT SIZE=-1  FACE="Arial">Prescribe Without Diagnosis</td>
            <td>
               &nbsp;&nbsp; <input name="prescribe_without_diagnosis" class="prescribe_without_diagnosis" type="radio" value="1"  <?php if (@$prescribe_without_diagnosis && $prescribe_without_diagnosis == "1") {
			echo "checked";
		}
		?>><?php echo $LDYes ?>&nbsp;&nbsp;
                <input name="prescribe_without_diagnosis" class="prescribe_without_diagnosis" type="radio" value="0"  <?php if (empty($prescribe_without_diagnosis) || $prescribe_without_diagnosis == "0") {
			echo "checked";
		}
		?>>
                <?php
echo $LDNo;
        ...
        <?php endif?>
         ...
         <tr>
            ...
            <td>
               &nbsp;&nbsp; <input name="allergic" class="allergic" type="radio" value="1"  <?php if (@$allergic && $allergic == "1") {
			echo "checked";
		}
		?>><?php echo $LDYes ?>&nbsp;&nbsp;
                <input name="allergic" class="allergic" type="radio" value="0"  <?php if (@$allergic && $allergic == "0") {
			echo "checked";
		}
