...
class Bill extends Encounter {
	...
	*/
	...
	function GetFirstPid() {
      ...
      FROM care_encounter, care_person, care_tz_billing_archive bi, care_tz_billing_archive_elem biel
      WHERE care_encounter.pid = care_person.pid
            AND bi.encounter_nr = care_encounter.encounter_nr
            AND biel.nr = bi.nr AND bi.is_printed=0
	...
	function getPrice($item_id, $pricelist_no) {
		...
		case 8:
			$this->sql = "select unit_price_7 from care_tz_drugsandservices WHERE item_id=" . $item_id;
			break;

		case 9:
			$this->sql = "select unit_price_8 from care_tz_drugsandservices WHERE item_id=" . $item_id;
			break;

		case 10:
			$this->sql = "select unit_price_9 from care_tz_drugsandservices WHERE item_id=" . $item_id;
			break;

		case 11:
			$this->sql = "select unit_price_10 from care_tz_drugsandservices WHERE item_id=" . $item_id;
			break;

		case 12:
			$this->sql = "select unit_price_11 from care_tz_drugsandservices WHERE item_id=" . $item_id;
			break;
		}
	...
	function getEncounterFromBillNo($billNo) {
		global $db;
		$this->debug = FALSE;

		$this->sql = "SELECT encounter_nr FROM $this->tbl_bill_archive WHERE nr=$billNo";
	...
	function CheckForPendingBill($encounter_nr) {
		...
		$this->sql = "select encounter_nr FROM care_tz_billing_archive where encounter_nr=$encounter_nr AND is_printed=0 LIMIT 0,1";
		echo $this->sql;
		$this->result = $db->Execute($this->sql);
	...
	function PendingBillObjects($encounter_nr) {
		...
		// We have nothing to do... no pending issues in the tables!
	}
...
//------------------------------------------------------------------------------
	...
	function GetBill($encounter_nr) {
		...
		$this->sql = "SELECT nr FROM $this->tbl_bill_archive WHERE encounter_nr=" . $encounter_nr . " limit 0,1 ";
		$this->request = $db->Execute($this->sql);
		//echo $this->sql;
	...
	function CreateNewBill($encounter_nr) {
		...
		$this->sql = "INSERT INTO care_tz_billing_archive (encounter_nr, first_date, create_id, creation_date) VALUES ('" . $encounter_nr . "','" . time() . "','','" . time() . "')";
		//echo $this->sql;
		$this->request = $db->Execute($this->sql);
		//echo $db->Insert_ID();
		return $db->Insert_ID();
	...
	function StorePrescriptionItemToBill($pid, $prescriptions_nr, $bill_number, $insurance, $pricelist_no, $bank) {
		global $db, $root_path;
		global $user_id, $sid, $local_user;
		$this->debug = FALSE;
		if ($this->debug) {
			echo "<b>class_tz_billing::StorePrescriptionItemToBill($pid, $prescriptions_nr, $bill_number, $insurance, $pricelist_no,$bank)</b><br>";
		}
		...
		$sqlTotalChart="SELECT SUM(qty) as total_dose FROM care_tz_nursing_chart WHERE nr=".$row['nr'];		
		$resutChart=$db->Execute($sqlTotalChart);
		$rowChartBill=$resutChart->Fetchrow();
		$rowChartBill['total_dose']+=0;
        if ($rowChartBill['total_dose']>0) {
            $row['total_dosage']=$rowChartBill['total_dose'];
		...
		if (strpos($_REQUEST['insrname'],'NHIF')!==false) {
            ...
            //nhif_item_code
            $sqlNhifPrice="SELECT UnitPrice FROM care_tz_drugsandservices_nhifschemes
            WHERE ItemCode='".$row['nhifItemCode']."' AND SchemeID='".$NhifScheme."'";
            $resultNhifPrice=$db->Execute($sqlNhifPrice);
            $nhifPrice=$resultNhifPrice->Fetchrow();
            $price=$nhifPrice['UnitPrice'];                                                   
		...
		$this->sql = "
		INSERT INTO $this->tbl_bill_archive_elements " .
		"	(" .
		"		nr, " .
		"		date_change, " .
		"		is_labtest, " .
		...
		$this->sql = "UPDATE $this->tbl_prescriptions SET bill_number=" . $bill_number . ", bill_status='archived' WHERE nr= " . $prescriptions_nr;
		if ($this->debug) {
			echo $this->sql;
		}
	...
	function StoreLaboratoryItemToBill($pid, $labtest_nr, $batch_nr, $bill_number, $insurance, $pricelist_no, $bank) {
		global $db, $root_path;
		global $user_id, $sid, $local_user;
		$this->debug = false;
		...
		care_test_request_chemlabor_sub.item_id AS p_item_id,care_test_request_chemlabor_sub.nhif_item_code AS nhifItemCode,care_test_request_chemlabor_sub.nhif_approval_number AS nhifApprovalNumber,
		care_tz_drugsandservices.item_description,care_encounter.current_dept_nr,care_encounter.current_ward_nr,care_encounter.encounter_class_nr,care_person.insurance_ID,care_tz_drugsandservices.nhif_item_code
		...
		ON care_encounter.encounter_nr=care_test_request_chemlabor.encounter_nr INNER JOIN care_person ON care_person.pid=care_encounter.pid
		...
		while ($row = $result->FetchRow()) {
            ...
            //nhif_item_code
            $sqlNhifPrice="SELECT UnitPrice FROM care_tz_drugsandservices_nhifschemes
            WHERE ItemCode='".$row['nhif_item_code']."' AND SchemeID='".$NhifScheme."'";
            $resultNhifPrice=$db->Execute($sqlNhifPrice);
            $nhifPrice=$resultNhifPrice->Fetchrow();
            $price=$nhifPrice['UnitPrice'];                                                             
	...
	function StoreRadiologyItemToBill($pid, $batch_nr, $bill_number, $insurance, $pricelist_no, $bank) {
		global $db, $root_path;
		global $user_id, $sid, $local_user;
		$this->debug = false;
		($this->debug) ? $db->debug = TRUE : $db->debug = FALSE;
		...
		care_tz_drugsandservices.item_id,care_tz_drugsandservices.item_description,care_encounter.current_dept_nr,care_encounter.current_ward_nr,care_encounter.encounter_class_nr,care_person.insurance_ID

		FROM $this->tbl_rad_requests
		INNER JOIN care_tz_drugsandservices
		ON care_tz_drugsandservices.item_id = care_test_request_radio.test_request
		INNER JOIN care_encounter
		ON care_encounter.encounter_nr=care_test_request_radio.encounter_nr INNER JOIN care_person
 ...
 '" . $row['item_description'] . "', " . $row['item_id'] . ", '" . $row['insurance_ID'] . "', '" . $contract['id'] . "', " . $row['batch_nr'] . ",'" . $bank . "'," . $row['current_dept_nr'] . ", " . $row['current_ward_nr'] . ", " . $row['encounter_class_nr'] . ",'" . $row['nhif_item_code'] . "','" . $row['nhif_approval_no'] . "','" . $user_id . "')";
			...
			$this->sql = "UPDATE $this->tbl_rad_requests SET bill_number=" . $bill_number . ", bill_status='archived'
	...
	function StoreAdvancePaymentToBill($pid, $bill_number, $insurance, $advance_payment, $notes, $description, $bank) {
		global $db, $root_path;
		global $user_id, $sid, $local_user;
		$this->debug = FALSE;
		($this->debug) ? $db->debug = TRUE : $db->debug = FALSE;
		...
		$sql_InOut_Patient = "SELECT b.encounter_nr,ce.current_dept_nr,ce.current_ward_nr,ce.encounter_class_nr
                    FROM care_tz_billing_archive b
		...
		"		User_Id)" .
		"
			VALUES (" . $bill_number . ", '" . time() . "', 0, 0, 0, 0, 1, 1, 1, 0, 0, " . $advance_payment . ", '" . $description . "', '" . $notes . "',0 , '" . $insurance . "', 0, 0,'" . $current_dept_nr . "','" . $bank . "','" . $current_ward_nr . "','" . $encounter_class_nr . "','" . $user_id . "')";
		//echo $this->sql;
		if ($this->debug) {
			echo $this->sql;
		}
	...
	function GetAdvanceTotalAmountQuotation($encounter_nr) {
		global $db, $root_path;
		$this->debug = FALSE;
		if ($this->debug) {
			echo "<b>class_tz_billing:: GetTotalAdvanceAmount($encounter_nr)</b><br>";
    ...
    WHERE description like 'Advance%'  AND close_deposit='0' AND care_tz_billing_archive.encounter_nr = " . $encounter_nr;
...
// end of function GetNameOfDrug($item_number)
	...
	function GetBillNumbersFromPID($pid) {
		...
		$debug = FALSE;
		($debug) ? $db->debug = TRUE : $db->debug = FALSE;
		$this->sql = "SELECT cb.nr FROM $this->tbl_bill_archive cb, $this->tbl_bill_archive_elements cbe, care_encounter ce, care_person cp
	...
	function VerifyBill($bill_nr) {
		global $db;
		$this->sql = "SELECT nr FROM $this->tbl_bill_archive WHERE nr=" . $bill_nr;
		//echo $this->sql;
		return $db->Execute($this->sql);
	}
...
//        $this->sql = "SELECT $this->tbl_bill_elements.*, care_tz_drugsandservices.purchasing_class FROM $this->tbl_bill_elements
		...
		//		echo $this->sql;
		/*
	         * Removed is_deposit_item!=1 by Mark Pat for the items to appear on the
	         * receipt in case of topup or refund
*/

		$this->sql = "SELECT $this->tbl_bill_archive_elements.*, care_tz_drugsandservices.purchasing_class FROM $this->tbl_bill_archive_elements
				INNER JOIN care_tz_drugsandservices ON $this->tbl_bill_archive_elements.item_number=care_tz_drugsandservices.item_id
					 WHERE nr = " . $nr . " " . $sql_where . "  AND nr IN(SELECT nr FROM $this->tbl_bill_archive WHERE is_printed=0 AND nr=" . $nr . ")
					 ORDER BY date_change ASC";
...
//------------------------------------------------------------------------------
	function GetRefundBillElem($nr) {
		...
		//lets get the receipt number for refund item
		$this->sql = "SELECT * from $this->tbl_bill_archive_elements
...
//------------------------------------------------------------------------------
	...
	function DisplayAdvanceForRefund($bill_nr, $edit_fields, $printout) {
		global $root_path, $billnr, $batch_nr, $db;
		...
		echo ($printout) ? '<table width="95%" border="0">' : '<table width="600" border="0" class="table_content">';
		...
		while ($bill_elems_row = $billelems->FetchRow()) {
			...
			$this->result = $db->Execute($this->sql);
			while ($elems_rows = $this->result->Fetchrow()) {
				if ($elems_rows['description'] == 'Refund') {
					$RefundPrice = abs($elems_rows['price']);

				}
				if ($elems_rows['price'] >= 0) {
					$part_sum = $elems_rows['price'] * $elems_rows['amount'];
					$sum_items += $part_sum;
			...
			$total_dep = $sum_items + $RefundPrice;

			// description of the item
			echo '<tr>';
			echo '<td colspan="2">Amt Deposited </td>';
			echo '</tr>';

			// Show the calculated sum of it now
			echo '<tr>';
			echo '<td><b>' . $LDpartSum . '</b></td>';
			echo '<td  style="text-align: right;" valign="bottom"><b>' . number_format($total_dep) . '</b></td>';
			echo '</tr>';
			// an emtpy row
			echo '<tr>';
	...
	//---------------------------------------------------------------------------
	function DisplayAdvanceForTopup($bill_nr, $edit_fields, $printout) {
		global $root_path, $billnr, $batch_nr, $db;
		global $LDPositionNr, $LDArticle, $LDPrice, $LDAmount, $LDpartSum, $LDPaidbyInsurance, $insurance_used,
			$LDPrescriptionOther;
		...
		while ($bill_elems_row = $billelems->FetchRow()) {
			...
			$this->result = $db->Execute($this->sql);
			while ($elems_rows = $this->result->Fetchrow()) {
				if ($elems_rows['description'] == 'Topup') {
					$TopupPrice = $elems_rows['price'];
			...
			$total_dep = $sum_items - $TopupPrice;
...
//---------------------------------------------------------------------------------
	...
	function GetBillCostSummaryInTimeframe($PID, $start, $end) {
		...
		*/
		...
		$this->sql = "SELECT nr FROM care_tz_billing_archive b, care_encounter e WHERE first_date>" . $start . " AND first_date <=" . $end . " AND e.encounter_nr = b.encounter_nr AND b.is_printed=0 AND e.pid=" . $PID;
		if ($this->result = $db->Execute($this->sql)) {
			while ($this->row = $this->result->FetchRow()) {
				$balanced_insurance = false;
	...
	function DisplayBillHeadline($bill_nr, $batch_nr) {
		...
		if ($enc_obj->EncounterExists($enc_number)) {
			...
			if ($enc_obj->GetHealthFundName($file_no)) {
				$insname = $enc_obj->GetHealthFundName($file_no);
			...
			$totalBillAmount = @$_GET['amount_for_efd'] ? $_GET['amount_for_efd'] : 0;
			echo "<tr bgcolor='#EEEEEE'><td><font color='white'>Amount For EFD Purpose: " . $totalBillAmount . "</font></td></tr>";
			echo '<tr><td align="center">' . $hosp_name . '<br>' . $hosp_address . '<br>' . $hosp_address_city . '</td></tr>
<tr><td style="text-align: right;"><img src="../../' . $bill_logo . '" border=0 align="absmiddle" alt=""></td></tr></table>';
			...
			echo '
  	  		<tr><td></td></tr>
  				...
  				<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2"><b>' . $LDBatchFileNr . '</b></font></span></span></td>
  					<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">' . $encoded_batch_number . '</font></span></span></b></td>

  				</tr>


  				 <tr>
  					<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">Admission-Nr.:</font></span></span></td>
  					<td bgcolor="#ffffee" class="vi_data"><span style="font-family:&quot;20 cpi&quot;"><font size="2">' . $enc_number . '</font></span></span></b></td>
  					</tr>

				<tr>
  					<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">File No.:</font></span></span></td>
  					<td bgcolor="#ffffee" class="vi_data"><span style="font-family:&quot;20 cpi&quot;"><font size="2">' . $file_no . '</font></span></span></b></td>
  					</tr>
  					<tr>
  					<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">PATIENT TYPE.:</font></span></span></td>
  					<td bgcolor="#ffffee" class="vi_data"><span style="font-family:&quot;20 cpi&quot;"><font size="2">' . $insname . '</font></span></span></b></td>
  					</tr>

  				<tr>
  					<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">Names:</font></span></span></td>
  					<td bgcolor="#ffffee" class="vi_data"><span style="font-family:&quot;20 cpi&quot;"><font size="2">' . $enc_obj->LastName($enc_number) . ', ' . ucwords($enc_obj->FirstName($enc_number)) . '</font></span></span></b></td>
  					</tr>';
			...
			*/
			...
			echo '<tr>
  					...
  					<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">RECEIPT DATE</font></span></span></td>
  					<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">' . date('d-m-Y h:i:s') . '</font></span></span></td>
  				</tr>
  				<tr>
  					<td class="headline"><b><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">' . $LDBillNumber . '</font></span></span></b></td>
  					<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2"><input type="text" name="bill_number" style="border: 0; background:none;" value="' . $bill_nr . '"></font></span></span></b></td>
  				</tr>
		<hr/>
...
<tr><td style="text-align: right;"><img src="../../' . $bill_logo . '" border=0 align="absmiddle" alt=""></td></tr></table>';
			...
			echo '
				...
				<tr>
  				...
  				<tr>
  					...
  					<td bgcolor="#ffffee" class="vi_data"><span style="font-family:&quot;20 cpi&quot;"><font size="2">' . $enc_obj->LastName($enc_number) . ', ' . $enc_obj->FirstName($enc_number) . '</font></span></span></b></td>
			...
			*/
			...
			echo '<tr>
  					...
  					<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">' . $bill_date_archived . '</font></span></span></td>
  				...
  				<tr>
  					...
  					<td class="headline"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">' . $bill_nr . '</font></span></span></b></td>
	...
	function DisplayLaboratoryBill($bill_nr, $edit_fields) {
		...
		while ($bill_elems_row = $billelems->FetchRow()) {
			...
			// headline
			echo '<tr>';
			echo '<td colspan="2" style="font-size: 0.9em;">' . $desc . '</td>';
			echo '</tr>';
			// price
			//            echo '<tr>';
			//            echo '<td>' . $LDPrice . '</td>';
			...
			echo '<tr>';
			echo '<td><b>' . $LDpartSum . '</b></td>';
			echo '<td style="text-align: right; font-size: 0.9em;" valign="bottom"><b>' . number_format(($price * $bill_elems_row['amount']) - $bill_elems_row['balanced_insurance'], 2, '.', ',') . '</b></td>';
			echo '</tr>';
...
//            // price
			...
			echo '<tr>';
			echo '<td><b>' . $LDpartSum . '</b></td>';
			echo '<td style="text-align: left; font-size: 0.9em" valign="bottom"><b>' . number_format(($price * $bill_elems_row['amount']) - $bill_elems_row['balanced_insurance'], 2, '.', ',') . '</b></td>';
			echo '</tr>';
...
//            echo '<td colspan="2"><b>' . $desc . '</b></td>';
			echo '<td colspan="2"  style="font-size: 0.9em;">' . $desc . ' &nbsp&nbsp'
				. '(' . $bill_elems_row['amount'] . 'x' . $price . '/-)</td>';
			echo '</tr>';
//            // price out of the list:
			...
			//            echo '<tr>';
			//            echo '<td>Quantity</td>';
			//            echo '<td style="text-align: right; font-size: 0.9em;" valign="bottom">(' . $bill_elems_row['amount'] . 'x' . $price . '/-)</td>';
			//            echo '</tr>';
			//            // Times per day:
			//            echo '<tr>';
			...
			//            echo '<tr>';
			//            echo '<td>Subtotal:</td>';
			//            echo '<td style="text-align: right;">' . number_format(($price * $bill_elems_row['amount']), 0, '.', ',') . '</td>';
			//            echo '</tr>';
			// How much is covered by insurance
			//            echo '<tr>';
			//            echo '<td>' . '- ' . $LDPaidbyInsurance . '</td>';
...
//------------------------------------------------------------------------------
	...
	function DisplayAdvanceBillItem($bill_nr, $edit_fields) {
		...
		global $root_path, $billnr, $batch_nr;
		global $LDPositionNr, $LDArticle, $LDPrice, $LDAmount, $LDpartSum, $LDPaidbyInsurance, $insurance_used,
		$LDPrescriptionOther, $LDadvanceDetails;
		...
		while ($bill_elems_row = $billelems->FetchRow()) {
			...
			echo '<tr>';
			echo '<td><b>' . $LDpartSum . '</b></td>';
			echo '<td style="text-align: left;" valign="bottom"><b>' . number_format(($price * $bill_elems_row['amount']) - $bill_elems_row['balanced_insurance'], 2, '.', ',') . '</b></td>';
			echo '</tr>';
			echo '<tr>';
			echo '<td><b>' . $LDadvanceDetails . '</b></td>';
			echo '<td style="text-align: left;" valign="bottom"><b>' . $bill_elems_row['notes'] . '</b></td>';
			echo '</tr>';
			// an emtpy row
			echo '<tr>';
			echo '<td colspan="3"><hr>&nbsp;</td>';
	...
	function DisplayRefundBillItem($bill_nr, $edit_fields) {
		...
		while ($bill_elems_row = $billelems->FetchRow()) {
			...
			echo '<tr>';
			echo '<td><b>' . $LDpartSum . '</b></td>';
			echo '<td  style="text-align: right;" valign="bottom"><b>' . number_format(($price * $bill_elems_row['amount']) - $bill_elems_row['balanced_insurance'], 2, '.', ',') . '</b></td>';
			echo '</tr>';
			// an emtpy row
			echo '<tr>';
			echo '<td colspan="3"><hr>&nbsp;</td>';
...
//------------------------------------------------------------------------------------------
	function DisplayArchivedRefundBill($bill_nr, $edit_fields) {
		...
		global $root_path, $billnr, $batch_nr;
		global $LDArticle, $LDPrice, $LDAmount, $LDPaidbyInsurance, $LDpartSum, $LDtotalamount, $LDAdvance, $db;
		$sum_price = 0;
  	...
  	<table width="100%" border="0" class="table_content">
		...
		while ($bill_elems_row = $billelems->FetchRow()) {
			...
			$sql_rf_items = "SELECT * FROM care_tz_billing_archive_elem WHERE nr='" . $bill_elems_row['nr'] . "' ORDER BY date_change ASC";

			$result_rf_items = $db->Execute($sql_rf_items);
			$bill_elems_row = null;
			while ($bill_elems_row = $result_rf_items->Fetchrow()) {
				$pos_nr += 1;
				$insurance_used += $bill_elems_row['balanced_insurance'];
				...
				$part_sum = ($this->price * $bill_elems_row['amount']) - $bill_elems_row['balanced_insurance'];
				if ($bill_elems_row['description'] != 'Refund') {
					$sum += 0.00;
				}
	...
	function DisplayArchivedTopupBill($bill_nr, $edit_fields) {
  	...
  	<table width="100%" border="0" class="table_content">
		...
		while ($bill_elems_row = $billelems->FetchRow()) {
			$sql_tp_items = "SELECT * FROM care_tz_billing_archive_elem WHERE nr='" . $bill_elems_row['nr'] . "' ORDER BY date_change ASC";

			$result_tp_items = $db->Execute($sql_tp_items);
			$bill_elems_row = null;

			while ($bill_elems_row = $result_tp_items->Fetchrow()) {
				$pos_nr += 1;
				$insurance_used += $bill_elems_row['balanced_insurance'];
	...
	function EditBillElement($nr) {
		...
		$this->sql = "SELECT
                ...
                WHERE care_tz_billing_elem.nr=" . $nr;
...
<tbody class="main">
	...
	function update_bill_element($bill_elem_number, $is_paid, $amount, $price, $description) {
		...
		$this->sql = "UPDATE care_tz_billing_archive_elem SET
                `is_paid` = '" . $is_paid . "',
                `amount` = '" . $amount . "',
                `price` = '" . $price . "',
	...
	function delete_bill_element($specific_bill) {
		global $db;
		$debug = FALSE;
		($debug) ? $db->debug = TRUE : $db->debug = FALSE;

		$this->sql = "DELETE FROM care_tz_billing_elem
             WHERE `nr` = " . $bill_elem_number;
		$db->Execute($this->sql);
		return TRUE;
	}
...
*/
				...
				if ($insurancebudget < 0) {
					echo '<tr><td><table border="0" ><tr><td ><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="-1">' . $LDInsurance . '</font></span></span></td><td><table  border="0" align="right"><tr>
	        			  ...
	        			  <tr>
	        				<td style="text-align: right;"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="-1">' . $LDUsedBudget . '</font></span></span></td>
	        				<td colspan="2"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="-1">
	        					' . number_format(($used_budget), 2, '.', ',') . '
	        				</font></span></span></td>
	        			  </tr>
	        			  <tr>
	        				<td style="text-align: right;"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="-1">' . $LDOverdrawnBudget . '</font></span></span></td>
	        				<td colspan="2">
					...
					if ($contract['gets_company_credit']) {
						echo '<tr>
	        				<td style="text-align: right;"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="-1">' . $LDCompanyCredit . '</font></span></span></td>
	        				<td colspan="2"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="-1">
						...
						echo '<tr>
	        				<td style="text-align: right;"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="-1">' . $LDOverdrawnPayment . '</font></span></span></td>
				...
				<tr>
					<td>
						<table  border="0" class="headline" align="right">
							<tr>
								<td ><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">' . $LDSummary . '</font></span></span></td>
								<td>
									<table border="0">
										<tr>
											<td style="text-align: right;"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">
												' . $LDLabTotal . '
											</font></span></span></td>
											<td ><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="-1">
										...
										<tr>
											...
											<td><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">
												';
				if ($insurancebudget < 0) {
				...
				echo '
										...
										<tr>
											<td style="text-align: right;">
												<b><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">' . $LDSumtopay . '</font></span></span></b>
											</td>
											<td><b><font color="#FF0000"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;">
                                            <font size="2">
                                            <input type="text" style="border: 0; background: none;" name="sum_to_pay" value="' . number_format(($sum_to_pay), 2) . '">
												</font><br>
												=======</b>
											</font></span></span></td>
										</tr>
...
//                    print_r($matchingContract);
				...
				if ($billelems) {
					...
					echo '
	  	  			<tr>
	  	  				<td valign="top"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="1.5">';
				...
				if ($billelems) {
                    ...
                    <tr>
                        <td valign="top"><span class=SpellE><span style="font-family;&quot;20 cpi&quot;"<font size="1.5">';
					$total_advace_for_refund = $this->DisplayAdvanceForRefund($bills['nr'], $edit_fields, $printout);
					//$total_refund = $this->DisplayRefundBillItem($bills['nr'], $edit_fields, $printout);
				...
				if ($billelems) {
                    ...
                    <tr>
                        <td valign="top"><span class=SpellE><span style="font-family;&quot;20 cpi&quot;"<font size="1.5">';
					$total_advace_for_topup = $this->DisplayAdvanceForTopup($bills['nr'], $edit_fields, $printout);
				...
				if ($billelems) {
					...
					<tr>
						<td valign="top"><span class=SpellE><span style="font-family;&quot;20 cpi&quot;"<font size="1.5">';
					$total_topup = $this->DisplayTopupBillItem($bills['nr'], $edit_fields, $printout);
					echo '
						</font></span></span></td>
					</tr>';
				}
				...
				if ($insurancebudget < 0) {
					...
					echo '<tr><td><table border="1" ><tr><td ><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="-1">' . $LDInsurance . '</font></span></span></td>
	        			  ...
	        			  <tr>
	        				<td style="text-align: right;"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="1.5">' . $LDOldBudget . '</font></span></span></td>
	        				<td colspan="2"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="1.5">
	        					' . number_format(($used_budget), 2, '.', ',') . '
	        				</font></span></span></td>
	        			  </tr>
	        			  <tr>
	        				<td style="text-align: right;"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="1.5">' . $LDOverdrawnBudget . '</font></span></span></td>
	        				<td colspan="2">
	        					<font color="FF0000"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="1.5">' . number_format(($insurancebudget), 2, '.', ',') . '</font></span></span></font>
	        				</td>
	        			  </tr>';
					$contract = $insurance_tz->CheckForValidContract($batch_nr, 0, $insurance_tz->GetCompanyFromPID($batch_nr));
					if ($contract['gets_company_credit']) {
						echo '<tr>
	        				<td style="text-align: right;"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="1.5">' . $LDCompanyCredit . '</font></span></span></td>
	        				<td colspan="2"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="1.5">
	        					' . number_format(($insurancebudget * -1), 2, '.', ',') . '
	        				</font></span></span></td>
	        			  </tr>';
						...
						echo '<tr>
	        				<td style="text-align: right;"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="1.5">' . $LDOverdrawnPayment . '</font></span></span></td>
					...
					echo '</table></td></tr>';
				} //end insurance burget <0

				//Get encounter no from bill for checking if there is any outstanding balance
				$encounter_nr = $this->getEncounterFromBillNo($bills['nr']);
				$balance = 0;
				//Get Quotation Balance
				if ($this->GetAdvanceTotalAmountQuotation($encounter_nr)) {
					$balance = $this->GetAdvanceTotalAmountQuotation($encounter_nr);
				}
...
//                $sum_to_pay += $total_advance;
				$rem_balance = 0;
				if ($balance < 0) {
					$sum_to_pay = $sum_to_pay + abs($balance);
				} else {
					if ($total_advance > 0) {
						$sum_to_pay += $total_advance;
						$rem_balance = $total_advance;
					} else {
						if ($balance > $sum_to_pay) {
							$rem_balance = $balance - $sum_to_pay;
							$sum_to_pay = 0.00;
						} else {
							$sum_to_pay = $sum_to_pay - $balance;
							$rem_balance = 0.00;
						}
					}
				}

				if (isset($total_topup) && $total_topup > 0) {
					$sum_to_pay = $total_topup;
				} else if (isset($total_refund) && abs($total_refund) > 0) {
					$sum_to_pay = 0.00;
				}
				...
				echo ($printout) ? '<table  width="100%" border="0">' : '<table width="600" border="0" class="table_content">';
				if ($total_laboratory || $total_prescription || $total_radiology || $total_services || $total_advance || $total_topup || $total_refund) {
					echo '<tr>
				...
				if ($balance != 0) {
					//Only show when balance is not zero
					if ($balance < 0) {
						echo '<tr>
											<td align="left"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">
												Outstanding Debt:
											</font></span></span></td>

											<td style="text-align: left;"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="2">
				...
				echo
					...
					'<tr>
                    <td><b>
                    Cashier:   ' . $_SESSION['sess_user_name'] . '</b></td>
		...
		if ($printout == FALSE) {
			...
			echo '<script language="javascript" >
                      <!--
                      function printOut_' . $billToPrint . '()
                      {
                        urlholder="show_bill.php?bill_number=' . $billToPrint . '&batch_nr=' . $batch_nr . '&printout=TRUE&amount_for_efd=' . $sum_to_pay . '";
                        testprintout=window.open(urlholder,"printout","width=380,height=600,menubar=no,resizable=yes,scrollbars=yes");

                      }
                      function printOut1_' . $billToPrint . '()
                      {
                        urlholder="show_company_bill.php?bill_number=' . $billToPrint . '&batch_nr=' . $batch_nr . '&printout=TRUE&amount_for_efd=' . $sum_to_pay . '";
                        testprintout=window.open(urlholder,"printout","width=380,height=600,menubar=no,resizable=yes,scrollbars=yes");
			...
			while ($bills = $billnumbers->FetchRow()) {
				...
				if ($insurancebudget < 0) {
					echo '<tr><td><table border="1" width="200"><tr><td width="105">#' . $LDInsurance . '</td><td><table width="200" border="0" align="right"><tr>
                                        ...
                                        <tr>
                                            <td style="text-align: right;">' . $LDOldBudget . '</td>
                                            <td colspan="2">
                                                ' . number_format(($used_budget), 2, '.', ',') . '
                                            </td>
                                        </tr>
                                        <tr>
                                            <td style="text-align: right;">' . $LDOverdrawnBudget . '</td>
                                            <td colspan="2">
					...
					if ($contract['gets_company_credit']) {
						echo '<tr>
                                            <td style="text-align: right;">' . $LDCompanyCredit . '</td>
                                            <td colspan="2">
                                                ' . number_format(($insurancebudget * -1), 2, '.', ',') . '
                                            </td>
                                        </tr>';
						...
						echo '<tr>
                                            <td style="text-align: right;">' . $LDOverdrawnPayment . '</td>
				...
				*/
				...
				echo '
                                                            ...
                                                            <tr>
                                                                    ...
                                                                    <td>
                                                                    <b>
                                                                    <font color="#FF0000"> <input type="text" name="sum_to_pay" style="border: 0; background: none;" value="' . number_format(($sum_to_pay), 2) . '"></font>
                                                                    <br>
                                                                            =======</b>
                                                                    </td>
                                                            </tr>
	...
	function ArchiveBill($bill_number) {
		...
		$this->sql = "UPDATE care_tz_billing_archive SET is_printed=1
        WHERE `nr`=" . $bill_number;
		...
		if ($CARE_TZ_BILLING_ARCHIVED) {
			$this->sql = "UPDATE care_encounter_prescription SET is_printed=1 WHERE bill_number=" . $bill_number;
			$db->Execute($this->sql);
			$this->sql = "UPDATE $this->tbl_lab_requests_sub SET is_printed=1 WHERE bill_number=" . $bill_number;
			$db->Execute($this->sql);
	...
	function FlagDepositItems($bill_number) {
		...
		$this->sql = "UPDATE care_tz_billing_archive_elem SET is_deposit_item=1 WHERE description NOT IN('Refund','Topup') AND  `nr`=" . $bill_number;
	...
	function close_deposit($encounter_nr) {
		global $db;
		$this->sql = "UPDATE care_tz_billing_archive_elem bare INNER JOIN care_tz_billing_archive
        bar ON bare.nr=bar.nr SET bare.close_deposit='1'  WHERE description='Advance' AND encounter_nr=" . $encounter_nr;
	...
	*/
	...
	function CreateTmpTableOfElementsToQuote($encounter_nr, $in_outpatient, $date_from, $date_to) {
		global $db, $tbl;

		if (!isset($date_from) && !isset($date_to)) {
			$date1 = date_create(date("Y-m-1"));
			$date2 = date_create(date("Y-m-t"));
			$date_from = date_format($date1, "Y-m-d 00:00:00");
			$date_to = date_format($date2, "Y-m-d 23:59:59");
		...
		$this->sql = "CREATE TEMPORARY TABLE quotation_temporary SELECT
                                    care_person.pid,
                                        ...
                                        care_encounter.nhif_approved,
                                     	...
                                     	CASE WHEN isnull(care_tz_drugsandservices.unit_price_4)=1 THEN 0 ELSE care_tz_drugsandservices.unit_price_4 END as unit_price_4,
                                     	CASE WHEN isnull(care_tz_drugsandservices.unit_price_5)=1 THEN 0 ELSE care_tz_drugsandservices.unit_price_5 END as unit_price_5,
                                     	CASE WHEN isnull(care_tz_drugsandservices.unit_price_6)=1 THEN 0 ELSE care_tz_drugsandservices.unit_price_6 END as unit_price_6,
                                        CASE WHEN isnull(care_tz_drugsandservices.unit_price_7)=1 THEN 0 ELSE care_tz_drugsandservices.unit_price_7 END as unit_price_7,
                                        CASE WHEN isnull(care_tz_drugsandservices.unit_price_8)=1 THEN 0 ELSE care_tz_drugsandservices.unit_price_8 END as unit_price_8,
                                        CASE WHEN isnull(care_tz_drugsandservices.unit_price_9)=1 THEN 0 ELSE care_tz_drugsandservices.unit_price_9 END as unit_price_9,
                                        CASE WHEN isnull(care_tz_drugsandservices.unit_price_10)=1 THEN 0 ELSE care_tz_drugsandservices.unit_price_10 END as unit_price_10,
                                        CASE WHEN isnull(care_tz_drugsandservices.unit_price_11)=1 THEN 0 ELSE care_tz_drugsandservices.unit_price_11 END as unit_price_11,
                                    ...
                                    ,       care_tz_drugsandservices.unit_price_7
                                    ,       care_tz_drugsandservices.unit_price_8
                                    ...
                                    WHERE care_test_request_radio.bill_number = 0 $and_in_outpatient  AND encounter_date BETWEEN '$date_from' AND '$date_to'
	...
	function GetNewQuotationOverview($date_from, $date_to) {
		global $db, $tbl;
	...
	function GetNewQuotations($encounter_nr, $in_outpatients) {
		...
		$sqlMod="SET GLOBAL sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''))";
		$db->Execute($sqlMod);

		$this->sql = "SELECT count(*) as anzahl, encounter_nr,pid, selian_pid, UPPER(name_last) as name_last, CONCAT(name_first,'  ', name_2) AS name_first, date_birth, encounter_date
	...
	function GetNewQuotation_Prescriptions($encounter_nr = 0, $in_outpatient = "", $id_array = "") {
		global $db;
		/**
		 * Returns all new prescriptions in a recordset. If no Encounter-Nr is given
		 * the result-list is grouped by encounters
	...
	//------------------------------------------------------------------------------
	function GetNewQuotation_Radiology($encounter_nr = 0, $in_outpatient = "", $id_array = "") {
		global $db;
		/**
		 * Returns all new prescriptions in a recordset. If no Encounter-Nr is given
		 * the result-list is grouped by encounters
	...
	function GetNewQuotation_Laboratory($encounter_nr = 0, $in_outpatient = "", $id_array = "") {
	...
	function DeleteNewPrescription($nr, $reason) {
		...
		$this->sql = "UPDATE $this->tbl_prescriptions SET
                                    is_disabled = 1, status='deleted', bill_status = 'dropped', disable_id='" . $_SESSION['sess_user_name'] . "',disable_date='" . date('Y-m-d') . "',history = CONCAT(history, '$reason')
                                    WHERE status<>'done' AND `nr`=" . $nr;
		$db->Execute($this->sql);
		return TRUE;
	}
	...
	function DeleteNewLaboratory($nr, $reason) {
		...
		$this->sql = "UPDATE $this->tbl_lab_requests_sub SET
                                    ...
                                    WHERE status<>'done' AND  `sub_id`=" . $nr;
		$db->Execute($this->sql);
		return TRUE;
	}
	...
	function DeleteNewRadiology($nr, $reason) {
		...
		$this->sql = "UPDATE $this->tbl_rad_requests SET
                                    is_disabled = 1, bill_status = 'dropped', disable_id='" . $_SESSION['sess_user_name'] . "',disable_date='" . date('Y-m-d') . "',history = CONCAT(history, '$reason')
                                    WHERE status<>'done' AND `batch_nr`=" . $nr;
		$db->Execute($this->sql);
		return TRUE;
	}
	...
	function ShowNewQuotations($in_outpatient, $sid, $date_from, $date_to) {
		...
		if ($this->CreateTmpTableOfElementsToQuote(0, $in_outpatient, $date_from, $date_to)) {
			$result = $this->GetNewQuotationOverview($date_from, $date_to);
		}
		...
		if ($result) {
			while ($row = $result->FetchRow()) {
				...
				if (@$row['encounter_nr'] && $this->GetAdvanceTotalAmountQuotation($row['encounter_nr']) > 0) {
					$BGCOLOR = 'bgcolor="#bfd7f0"';
				}
	...
	function ShowNewQuotationEncounter_Prescriptions($encounter_nr, &$id_array) {
		...
		# 4 lang
		global $LDNotes, $LDDosage, $LDPrice, $LDInsurance, $LDPricing, $LDTSH, $LDPricing, $LDNothingtodo, $test, $LDTimesPerDay, $LDDays,$NhifScheme;
		...
		if ($result) {
			...
			$unit_price_7_description = $this->GetMeaningOfPrices('unit_price_7');
			$unit_price_8_description = $this->GetMeaningOfPrices('unit_price_8');
			$unit_price_9_description = $this->GetMeaningOfPrices('unit_price_9');
			$unit_price_10_description = $this->GetMeaningOfPrices('unit_price_10');
			$unit_price_11_description = $this->GetMeaningOfPrices('unit_price_11');
			...
			while ($row = $result->FetchRow()) {
				 ...
				 //It seems I should touch some other areas as well

				if ($row['purchasing_class']=='drug_list') {
					//echo "<pre>"; print_r($row); "</pre>";
					$rowChartBill['chartTotal']=0;
					$sqlChartBill="SELECT SUM(qty) as chartTotal FROM care_tz_nursing_chart WHERE nr=".$row['nr'];
					$resultChartBill=$db->Execute($sqlChartBill);
					$rowChartBill=$resultChartBill->Fetchrow();
					$rowChartBill['chartTotal']+=0;
                    if ($rowChartBill['chartTotal']>0) {
                    	$row['total_dosage']=$rowChartBill['chartTotal'];
				...
				if (empty($row['unit_price_7'])) {
					$unit_price_7 = "0";
				} else {
					$unit_price_7 = $row['unit_price_7'];
				}

				// replace , to . if needed:
				$unit_price_7 = strtr($unit_price_7, ',', '.');

				if (empty($row['unit_price_8'])) {
					$unit_price_8 = "0";
				} else {
					$unit_price_8 = $row['unit_price_8'];
				}

				// replace , to . if needed:
				$unit_price_8 = strtr($unit_price_8, ',', '.');

				if (empty($row['unit_price_9'])) {
					$unit_price_9 = "0";
				} else {
					$unit_price_9 = $row['unit_price_9'];
				}

				// replace , to . if needed:
				$unit_price_9 = strtr($unit_price_9, ',', '.');

				if (empty($row['unit_price_10'])) {
					$unit_price_10 = "0";
				} else {
					$unit_price_10 = $row['unit_price_10'];
				}

				// replace , to . if needed:
				$unit_price_10 = strtr($unit_price_10, ',', '.');

				if (empty($row['unit_price_11'])) {
					$unit_price_11 = "0";
				} else {
					$unit_price_11 = $row['unit_price_11'];
				}

				// replace , to . if needed:
				$unit_price_11 = strtr($unit_price_11, ',', '.');
				...
				if ($NhifScheme) {
					//$nhifUnitPrice=0;
					//Ds: care_tz_drugsandservices
					//echo "<pre>"; print_r($row); echo "</pre>";
					$sqlNhifPrice="SELECT UnitPrice FROM care_tz_drugsandservices_nhifschemes WHERE ItemCode='".$row['nhifItemCode']."' AND SchemeID='".$NhifScheme."'";				
					$resultNhifPrice=$db->Execute($sqlNhifPrice);
					$nhifUnitPrice=$resultNhifPrice->Fetchrow();
					$nhifUnitPrice=$nhifUnitPrice['UnitPrice'];
					echo '<font size="1"><input type="hidden" name="unit_price_' . $row['nr'] . '" value="' . $nhifUnitPrice . '">';	
				...
				case 8:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['nr'] . '" value="' . $unit_price_7 . '">';
					$unitPrice = $unit_price_7;
					break;

				case 9:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['nr'] . '" value="' . $unit_price_8 . '">';
					$unitPrice = $unit_price_8;
					break;

				case 10:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['nr'] . '" value="' . $unit_price_9 . '">';
					$unitPrice = $unit_price_9;
					break;

				case 11:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['nr'] . '" value="' . $unit_price_10 . '">';
					$unitPrice = $unit_price_10;
					break;

				case 12:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['nr'] . '" value="' . $unit_price_11 . '">';
					$unitPrice = $unit_price_11;
					break;
				}//not nhif
			...
			if ($NhifScheme) {
				...
				echo '</td>
                                                                ...
                                                                <input onkeyup="javascript:calc_article(\'' . $row['nr'] . '\');" type="hidden" name="showprice_' . $row['nr'] . '" id="showprice_' . $row['nr'] . '" value="' . $nhifUnitPrice . '">SHOWPRICE: ' . $nhifUnitPrice;
				...
				echo '	</table>
                                                    ...
                                                    <td valign="top" id="div_' . $row['nr'] . '"><div>
                                                            <table border="0" cellpadding="0" cellspacing="0" width="200" >
                                                                ...
                                                                <input type="hidden" name="unit_price_' . $row['nr'] . '" id="unit_price_' . $row['nr'] . '" value="' . $nhifUnitPrice . '">';
               }else{
               	echo '	</table>
                                                        </div></td>
                                                    <td valign="top" id="div_' . $row['nr'] . '"><div>
				...
				case 8:echo '<br><font size="1">' . $unit_price_7_description . ':' . $unit_price_7 . ' ' . $LDTSH . '<input type="radio" name="unit_price_' . $row['nr'] . '" value="' . $unit_price_7 . '" checked>';
					break;

				case 9:echo '<br><font size="1">' . $unit_price_8_description . ':' . $unit_price_8 . ' ' . $LDTSH . '<input type="radio" name="unit_price_' . $row['nr'] . '" value="' . $unit_price_8 . '" checked>';
					break;

				case 10:echo '<br><font size="1">' . $unit_price_9_description . ':' . $unit_price_9 . ' ' . $LDTSH . '<input type="radio" name="unit_price_' . $row['nr'] . '" value="' . $unit_price_9 . '" checked>';
					break;

				case 11:echo '<br><font size="1">' . $unit_price_10_description . ':' . $unit_price_10 . ' ' . $LDTSH . '<input type="radio" name="unit_price_' . $row['nr'] . '" value="' . $unit_price_10 . '" checked>';
					break;

				case 12:echo '<br><font size="1">' . $unit_price_11_description . ':' . $unit_price_11 . ' ' . $LDTSH . '<input type="radio" name="unit_price_' . $row['nr'] . '" value="' . $unit_price_11 . '" checked>';
					break;
				}
				echo '</td>
                                                                </tr>
	...
	function ShowNewQuotationEncounter_Laboratory($encounter_nr) {
		...
		global $LDInsurance, $LDPricing, $LDDosage, $LDTSH, $LDPrice, $LDNothingtodo,$NhifScheme;
		...
		if ($result) {
			...
			while ($row = $result->FetchRow()) {
				...
				// Check if unit_price_7 is NULL -> replace it with 0 as number
				if (empty($row['unit_price_7'])) {
					$unit_price_7 = 0;
				} else {
					$unit_price_7 = $row['unit_price_7'];
				...
				// Check if unit_price_8 is NULL -> replace it with 0 as number
				if (empty($row['unit_price_8'])) {
					$unit_price_8 = 0;
				} else {
					$unit_price_8 = $row['unit_price_8'];
				...
				// Check if unit_price_9 is NULL -> replace it with 0 as number
				if (empty($row['unit_price_9'])) {
					$unit_price_9 = 0;
				} else {
					$unit_price_9 = $row['unit_price_9'];
				...
				// Check if unit_price_10 is NULL -> replace it with 0 as number
				if (empty($row['unit_price_10'])) {
					$unit_price_10 = 0;
				} else {
					$unit_price_10 = $row['unit_price_10'];
				...
				// Check if unit_price_11 is NULL -> replace it with 0 as number
				if (empty($row['unit_price_11'])) {
					$unit_price_11 = 0;
				} else {
					$unit_price_11 = $row['unit_price_11'];
                ...
                if ($NhifScheme) {
					...
					$nhifUnitPrice=$resultNhifPrice->Fetchrow();
					$nhifUnitPrice=$nhifUnitPrice['UnitPrice'];
                    echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['sub_id'] . '" value="' . $nhifUnitPrice . '">';					
				}else{
				...
				case 8:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['sub_id'] . '" value="' . $unit_price_7 . '">';
					$unitPrice = $unit_price_7;
					break;

				case 9:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['sub_id'] . '" value="' . $unit_price_8 . '">';
					$unitPrice = $unit_price_8;
					break;

				case 10:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['sub_id'] . '" value="' . $unit_price_9 . '">';
					$unitPrice = $unit_price_9;
					break;

				case 11:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['sub_id'] . '" value="' . $unit_price_10 . '">';
					$unitPrice = $unit_price_10;
					break;

				case 12:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['sub_id'] . '" value="' . $unit_price_11 . '">';
					$unitPrice = $unit_price_11;
					break;
				}//not nhif
			}// nhif
			...
			if ($NhifScheme) {
				echo '</td>
                                                            <input onkeyup="javascript:calc_article(\'' . $row['sub_id'] . '\');" type="hidden" name="showprice_' . $row['sub_id'] . '" id="showprice_' . $row['sub_id'] . '" value="' . $nhifUnitPrice . '">SHOWPRICE: ' . $nhifUnitPrice;
				echo '</tr>';
                                        ...
                                        <td valign="top" id="div_' . $row['sub_id'] . '">
                                            <table border="0" cellpadding="0" width="200">
                                                <div>
                                                    <tr>
                                                        <td>Price:</td>
                                                    <input onkeyup="javascript:calc_article(\'' . $row['sub_id'] . '\');" type="hidden" id="unit_price_' . $row['sub_id'] . '" name="unit_price_' . $row['sub_id'] . '" value="' . $nhifUnitPrice . '">
	...
	/*
		      ...
		      <td style="text-align: right;"><input type="hidden" id="showprice_'.$row['nr'].'" name="showprice_'.$row['batch_nr'].'" value="'.$sum.'">'.$sum.' TSH<input onChange="calc_article(\''.$row['batch_nr'].'\');" type="hidden" value="1" name="dosage_'.$row['batch_nr'].'"></td>
		      </tr>';

		      if ($IS_PATIENT_INSURED) echo '
		      <tr>
		      <td>'.$LDInsurance.'</td>
		      <td style="text-align: right;"><input onkeyup="calc_article(\''.$row['batch_nr'].'\')" type="text" size="4" value='.$sum.' name="insurance_'.$row['batch_nr'].'"> TSH</td>
		      </tr>';
		      echo '
		      </table>
		      </td>
...
*/
		...
		if ($result) {
			...
			while ($row = $result->FetchRow()) {
                ...
                if ($NhifScheme) {
					...
					$nhifUnitPrice=$resultNhifPrice->Fetchrow();
					$nhifUnitPrice=$nhifUnitPrice['UnitPrice'];
					echo '<font size="1"><input type="hidden" name="unit_price_' . $row['batch_nr'] . '" value="' . $nhifUnitPrice . '">';
				...
				case 8:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['batch_nr'] . '" value="' . $unit_price_7 . '">';
					$unitPrice = $unit_price_7;
					break;

				case 9:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['batch_nr'] . '" value="' . $unit_price_8 . '">';
					$unitPrice = $unit_price_8;
					break;

				case 10:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['batch_nr'] . '" value="' . $unit_price_9 . '">';
					$unitPrice = $unit_price_9;
					break;

				case 11:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['batch_nr'] . '" value="' . $unit_price_10 . '">';
					$unitPrice = $unit_price_10;
					break;

				case 12:echo '<br><font size="1"><input type="hidden" name="unit_price_' . $row['batch_nr'] . '" value="' . $unit_price_11 . '">';
					$unitPrice = $unit_price_11;
					break;
			...
			if ($NhifScheme) {
				echo '</td>
                                                                <input onkeyup="javascript:calc_article(\'' . $row['batch_nr'] . '\');" type="hidden" name="showprice_' . $row['batch_nr'] . '" id="showprice_' . $row['batch_nr'] . '" value="' . $nhifUnitPrice . '">SHOWPRICE: ' . $nhifUnitPrice;
				echo '</tr>';
			}else{
				echo '</td>
					...
					echo '	</table>
                                                    ...
                                                    <td valign="top" id="div_' . $row['batch_nr'] . '"><div>
                                                            <table border="0" cellpadding="0" cellspacing="0" width="200" >
                                                                ...
                                                                <input type="hidden" name="unit_price_' . $row['batch_nr'] . '" id="unit_price_' . $row['batch_nr'] . '" value="' . $nhifUnitPrice. '">';
				...
				case 8:echo '<br><font size="1">' . $unit_price_7_description . ':' . $unit_price_7 . ' ' . $LDTSH . '<input type="radio" name="unit_price_' . $row['batch_nr'] . '" value="' . $unit_price_7 . '" checked>';
					break;

				case 9:echo '<br><font size="1">' . $unit_price_8_description . ':' . $unit_price_8 . ' ' . $LDTSH . '<input type="radio" name="unit_price_' . $row['batch_nr'] . '" value="' . $unit_price_8 . '" checked>';
					break;

				case 10:echo '<br><font size="1">' . $unit_price_9_description . ':' . $unit_price_9 . ' ' . $LDTSH . '<input type="radio" name="unit_price_' . $row['batch_nr'] . '" value="' . $unit_price_9 . '" checked>';
					break;

				case 11:echo '<br><font size="1">' . $unit_price_10_description . ':' . $unit_price_10 . ' ' . $LDTSH . '<input type="radio" name="unit_price_' . $row['batch_nr'] . '" value="' . $unit_price_10 . '" checked>';
					break;

				case 12:echo '<br><font size="1">' . $unit_price_11_description . ':' . $unit_price_11 . ' ' . $LDTSH . '<input type="radio" name="unit_price_' . $row['batch_nr'] . '" value="' . $unit_price_11 . '" checked>';
					break;
	...
	function new_reg($encounter, $reg, $prescriber) {
		...
		while ($this->res = $this->request->FetchRow()) {
			...
			$is_transmit_to_weberp_enable = (isset($is_transmit_to_weberp_enable) ? $is_transmit_to_weberp_enable : null);
	...
	function cash_patient($filenumber) {
		global $db;
		...
		$this->sql = "SELECT DISTINCT company_id,care_tz_company.name FROM care_tz_insurance
            ...
            WHERE cancel_flag='0' AND name = 'CASH-PATIENT'
             order by name asc ";

		//echo $this->sql;

		$result = $db->Execute($this->sql);
		if ($row_id = $result->Fetchrow()) {
			$cash_id = $row_id['company_id'];
			$sql_cash = "SELECT insurance_ID FROM care_person WHERE selian_pid=" . $filenumber;
			$result = $db->Execute($sql_cash);
			if ($patient_insid = $result->Fetchrow()) {

				if ($cash_id == $patient_insid[0]) {
	...
	function cash_id($filenumber) {
		...
		$result = $db->Execute($this->sql);
		if ($cash_id = $result->Fetchrow()) {
			return $cash_id['company_id'];
		}
