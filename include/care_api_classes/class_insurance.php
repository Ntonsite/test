...
class Insurance extends Core {
	...
	function Firm_exists($firm_id = '') {
		global $db;
		if (!$this->_internResolveFirmID($firm_id)) {
			return FALSE;
		}
	...
	function updateFirmInfoFromArray($nr, $data) {
		...
		if (isset($this->data_array['firm_id'])) {
			unset($this->data_array['firm_id']);
		}
...
class PersonInsurance extends Insurance {
	...
	function updateDataFromArray($array, $item_nr = '', $isnum = TRUE) {
		...
		$v = '';
		$sql = '';
		if (!is_array($array) || empty($item_nr) || !is_numeric($item_nr)) {
			return FALSE;
		}
