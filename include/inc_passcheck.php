...
//    print_r($zeile2);
	if (preg_match('/System_Admin/i', $zeile2)) {
		// if System_admin return true
		return 1;
	} elseif (in_array('no_allow_type_all', $allowedarea)) {
		// check if the type "all" is blocked, if so return false
		return 0;
	} elseif ($permit_type_all && preg_match('/_a_0_all/i', $zeile2)) {
...
//                    print_r($childareas[$list]);
				if (is_array($childareas[$list])) {					
					if (sizeof($childareas[$list]) > 0) {
