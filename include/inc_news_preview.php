...
if (isset($news[$j])) {
    ...
    if (!empty($news[$j]['body'])) {
        ...
        if (isset($news_headline_title_font_bold))
            $sBuffer = $sBuffer . '<b>';

        //$sBuffer = $sBuffer.ucfirst(deactivateHotHtml(nl2br($news[$j]['title'])));
        $news_default = nl2br($news[$j]['title']);
        $sBuffer = $sBuffer . ucfirst(deactivateHotHtml($news_default));

        if ($news_headline_title_font_bold)
            $sBuffer = $sBuffer . '</b>';
...
if (!isset($news[$j]) || $nofile) {
    ...
    if (file_exists($root_path . "language/" . $lang . "/lang_" . $lang . "_newsdummy.php"))
        include ($root_path . "language/" . $lang . "/lang_" . $lang . "_newsdummy.php");
    else
        include($root_path . "language/en/lang_en_newsdummy.php");
    ...
    if (!isset($editor_path) || empty($editor_path))
        $editor_path = 'editor-pass.php?sid=' . $sid . '&lang=' . $lang . '&target=headline&title=' . $LDEditTitle;
