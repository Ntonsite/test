...
if (!isset($cfg['control_buttons']) || empty($cfg['control_buttons'])) {
	...
	if (!isset($GLOBAL_CONFIG['theme_control_buttons']) || empty($GLOBAL_CONFIG['theme_control_buttons'])) {
		$theme_control = $gc->getConfigValue('theme_control_buttons');
		if (empty($theme_control)) {
...
function createLDImgSrc($froot, $fn, $border = '', $align = '') {
	...
	} elseif (file_exists($froot . 'gui/img/control/' . $theme_control . '/' . $lang . '/' . $lang . '_' . $fn)) {
		$picfile_path = $froot . 'gui/img/control/' . $theme_control . '/' . $lang . '/' . $lang . '_' . $fn;
	} else {
		if (file_exists($froot . 'gui/img/control/' . $theme_control . '/' . LANG_DEFAULT . '/' . LANG_DEFAULT . '_' . $fn)) {
			$picfile_path = $froot . 'gui/img/control/' . $theme_control . '/' . LANG_DEFAULT . '/' . LANG_DEFAULT . '_' . $fn;
		} else {
	...
	if ($border != '') {
		$picfilesrc .= ' border=' . $border;
	}

	if ($align) {
		$picfilesrc .= ' align="' . $align . '"';
	}
...
function createLogo($froot, $fn, $border = '', $align = '') {
	...
	if (!file_exists($froot . $img_path_com_icon . $fn)) {
		$img_path_com_icon = 'gui/img/logos/default/';
	}
