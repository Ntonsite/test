...
if (!defined('NO_CHAIN') || NO_CHAIN != 1) {
    ...
    isset($clear_ck_sid)? $clear_ck_sid : "";
	$clear_ck_sid = $dec_hcemd5->DecodeMimeSelfRand($_COOKIE[$ck_sid_buffer]);

	$tnow = date('His');

	$_GET['refreshed'] = isset($_GET['refreshed']) ? $_GET['refreshed'] : $_GET['refreshed'] = TRUE;
			...
			if (!$TIME_OUT_INACTIVE) {
				...
				if (isset($_SESSION['sess_tos']) || $_SESSION['sess_tos']) {
					...
					if (($tnow - $_SESSION['sess_tos']) >= $TIME_OUT_TIME) {
						$time_out = TRUE;
					} else if (isset($_GET['refreshed']) && $_GET['refreshed'] != '') {
				...
				if ($time_out || !$_SESSION['sess_tos']) {
					...
					if (isset($_GET['refreshed']) && $_GET['refreshed'] != '') {
						$_SESSION['sess_tos'] = $tnow;
					}
...
if (defined('LANG_FILE') && LANG_FILE != '') {
	if (getLang(LANG_FILE)) {
		...
		if (file_exists($root_path . 'language/' . LANG_DEFAULT . '/lang_' . LANG_DEFAULT . '_' . LANG_FILE)) {
