...
while ($test_request_w = $requests_w->FetchRow()) {
	...
	// if (@$batch_nr_w && ($batch_nr_w != $test_request_w['batch_nr'])) {
	if ($batch_nr_w != $test_request_w['batch_nr']) {
		echo "<img src=\"" . $root_path . "gui/img/common/default/pixel.gif\" border=0 width=4 height=7>
        <a onmouseover=\"showBallon('" . $test_request_w['name_last'] . ", " . ucwords($test_request_w['name_first']) . " encounter: " . $test_request_w['encounter_nr'] . " Hospital file nr: " . $test_request_w['selian_pid'] . " Batch nr: " . $test_request_w['batch_nr'] . "',0,150,'#99ccff'); window.status='Care2x Tooltip'; return true;\"
